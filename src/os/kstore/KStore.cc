...
bool KStore::OnodeHashLRU::get_next(const ghobject_t &after,
...
int KStore::OnodeHashLRU::trim(int max) {
  std::lock_guard<std::mutex> l(lock);
  dout(20) << __func__ << " max " << max << " size " << onode_map.size()
           << dendl;
  int trimmed = 0;
...
#undef dout_prefix
#define dout_prefix                                                            \
  *_dout << "kstore(" << store->path << ").collection(" << cid << ") "

KStore::Collection::Collection(KStore *ns, coll_t cid)
    : CollectionImpl(ns->cct, cid), store(ns), osr(new OpSequencer()),
      onode_map(store->cct) {}

void KStore::Collection::flush() { osr->flush(); }

bool KStore::Collection::flush_commit(Context *c) {
  return osr->flush_commit(c);
}

KStore::OnodeRef KStore::Collection::get_onode(const ghobject_t &oid,
                                               bool create) {
...
KStore::KStore(CephContext *cct, const string &path)
    : ObjectStore(cct, path), db(NULL), basedir(path), path_fd(-1), fsid_fd(-1),
      mounted(false), nid_last(0), nid_max(0),
      throttle_ops(cct, "kstore_max_ops", cct->_conf->kstore_max_ops),
      throttle_bytes(cct, "kstore_max_bytes", cct->_conf->kstore_max_bytes),
      finisher(cct), kv_sync_thread(this), kv_stop(false), logger(nullptr) {
  _init_logger();
}
...
void KStore::_init_logger() {
  ...
  b.add_time_avg(l_kstore_state_prepare_lat, "state_prepare_lat",
                 "Average prepare state latency");
  b.add_time_avg(l_kstore_state_kv_queued_lat, "state_kv_queued_lat",
                 "Average kv_queued state latency");
  b.add_time_avg(l_kstore_state_kv_done_lat, "state_kv_done_lat",
                 "Average kv_done state latency");
  b.add_time_avg(l_kstore_state_finishing_lat, "state_finishing_lat",
                 "Average finishing state latency");
  b.add_time_avg(l_kstore_state_done_lat, "state_done_lat",
...
int KStore::stat(CollectionHandle &ch, const ghobject_t &oid, struct stat *st,
                 bool allow_eio) {
  dout(10) << __func__ << " " << ch->cid << " " << oid << dendl;
  Collection *c = static_cast<Collection *>(ch.get());
  std::shared_lock l{c->lock};
...
int KStore::read(CollectionHandle &ch, const ghobject_t &oid, uint64_t offset,
                 size_t length, bufferlist &bl, uint32_t op_flags) {
  dout(15) << __func__ << " " << ch->cid << " " << oid << " " << offset << "~"
           << length << dendl;
  bl.clear();
  Collection *c = static_cast<Collection *>(ch.get());
  std::shared_lock l{c->lock};
...
int KStore::_do_read(OnodeRef o, uint64_t offset, size_t length, bufferlist &bl,
                     uint32_t op_flags) {
  int r = 0;
...
int KStore::fiemap(CollectionHandle &ch, const ghobject_t &oid, uint64_t offset,
...
int KStore::getattr(CollectionHandle &ch, const ghobject_t &oid,
...
int KStore::getattrs(CollectionHandle &ch, const ghobject_t &oid,
...
int KStore::collection_list(CollectionHandle &c_, const ghobject_t &start,
...
int KStore::_collection_list(Collection *c, const ghobject_t &start,
                             const ghobject_t &end, int max,
                             vector<ghobject_t> *ls, ghobject_t *pnext) {
  int r = 0;
...
KStore::OmapIteratorImpl::OmapIteratorImpl(CollectionRef c, OnodeRef o,
                                           KeyValueDB::Iterator it)
    : c(c), o(o), it(it) {
  std::shared_lock l{c->lock};
...
out:
  dout(10) << __func__ << " " << ch->cid << " oid " << oid << " = " << r
           << dendl;
  return r;
}
...
KStore::get_omap_iterator(CollectionHandle &ch, ///< [in] collection
...
void KStore::_txc_finalize(OpSequencer *osr, TransContext *txc) {
  dout(20) << __func__ << " osr " << osr << " txc " << txc << " onodes "
           << txc->onodes << dendl;

  // finalize onodes
  for (set<OnodeRef>::iterator p = txc->onodes.begin(); p != txc->onodes.end();
       ++p) {
    bufferlist bl;
    encode((*p)->onode, bl);
...
void KStore::_kv_sync_thread() {
  ...
  while (true) {
    ...
    if (kv_queue.empty()) {
      ...
      dout(20) << __func__ << " committed " << kv_committing.size() << " in "
               << dur << dendl;
...
int KStore::queue_transactions(CollectionHandle &ch, vector<Transaction> &tls,
  ...
  *_dout << dendl;
  ...
  for (int pos = 0; i.have_op(); ++pos) {
    ...
    if (!o) {
      ...
      if (!create) {
        if (!o || !o->exists) {
          dout(10) << __func__ << " op " << op->op << " got ENOENT on " << oid
                   << dendl;
          r = -ENOENT;
          goto endop;
        }
    ...
    case Transaction::OP_SETALLOCHINT: {
      ...
      r = _setallochint(txc, c, o, expected_object_size, expected_write_size,
                        flags);
    } break;
...
void KStore::_dump_onode(OnodeRef o) {
  dout(30) << __func__ << " " << o << " nid " << o->onode.nid << " size "
           << o->onode.size << " expected_object_size "
           << o->onode.expected_object_size << " expected_write_size "
           << o->onode.expected_write_size << dendl;
  for (map<string, bufferptr>::iterator p = o->onode.attrs.begin();
       p != o->onode.attrs.end(); ++p) {
    dout(30) << __func__ << "  attr " << p->first << " len "
...
void KStore::_do_write_stripe(TransContext *txc, OnodeRef o, uint64_t offset,
                              bufferlist &bl) {
...
int KStore::_do_write(TransContext *txc, OnodeRef o, uint64_t offset,
                      uint64_t length, bufferlist &orig_bl,
                      uint32_t fadvise_flags) {
  int r = 0;

  dout(20) << __func__ << " " << o->oid << " " << offset << "~" << length
           << " - have " << o->onode.size << " bytes, nid " << o->onode.nid
           << dendl;
  _dump_onode(o);
  o->exists = true;
  ...
  while (length > 0) {
    ...
    dout(20) << __func__ << " read previous stripe " << stripe_off << ", got "
             << prev.length() << dendl;
    bufferlist bl;
    if (offset_rem) {
      ...
      if (p < offset_rem) {
        dout(20) << __func__ << " add leading " << offset_rem - p << " zeros"
                 << dendl;
...
int KStore::_write(TransContext *txc, CollectionRef &c, OnodeRef &o,
                   uint64_t offset, size_t length, bufferlist &bl,
                   uint32_t fadvise_flags) {
  dout(15) << __func__ << " " << c->cid << " " << o->oid << " " << offset << "~"
           << length << dendl;
  _assign_nid(txc, o);
  int r = _do_write(txc, o, offset, length, bl, fadvise_flags);
  txc->write_onode(o);

  dout(10) << __func__ << " " << c->cid << " " << o->oid << " " << offset << "~"
           << length << " = " << r << dendl;
  return r;
}

int KStore::_zero(TransContext *txc, CollectionRef &c, OnodeRef &o,
    ...
    while (pos < offset + length) {
      if (stripe_off || end - pos < stripe_size) {
        ...
        dout(30) << __func__ << " stripe " << pos - stripe_off << " got "
                 << stripe.length() << dendl;
        bufferlist bl;
        bl.substr_of(stripe, 0,
                     std::min<uint64_t>(stripe.length(), stripe_off));
        if (end >= pos - stripe_off + stripe_size || end >= o->onode.size) {
          ...
          if (stripe.length() > bl.length()) {
            ...
            dout(20) << __func__ << " keeping tail " << l << " of stripe"
                     << dendl;
            bl.claim_append(t);
          }
        }
...
int KStore::_truncate(TransContext *txc, CollectionRef &c, OnodeRef &o,
                      uint64_t offset) {
  dout(15) << __func__ << " " << c->cid << " " << o->oid << " " << offset
           << dendl;
...
int KStore::_setattr(TransContext *txc, CollectionRef &c, OnodeRef &o,
...
int KStore::_setattrs(TransContext *txc, CollectionRef &c, OnodeRef &o,
...
int KStore::_rmattr(TransContext *txc, CollectionRef &c, OnodeRef &o,
                    const string &name) {
  dout(15) << __func__ << " " << c->cid << " " << o->oid << " " << name
           << dendl;
  int r = 0;
...
int KStore::_omap_setkeys(TransContext *txc, CollectionRef &c, OnodeRef &o,
                          bufferlist &bl) {
  dout(15) << __func__ << " " << c->cid << " " << o->oid << dendl;
  int r;
  auto p = bl.cbegin();
...
int KStore::_omap_setheader(TransContext *txc, CollectionRef &c, OnodeRef &o,
                            bufferlist &bl) {
  dout(15) << __func__ << " " << c->cid << " " << o->oid << dendl;
  int r;
  string key;
...
int KStore::_omap_rmkeys(TransContext *txc, CollectionRef &c, OnodeRef &o,
                         const bufferlist &bl) {
  dout(15) << __func__ << " " << c->cid << " " << o->oid << dendl;
  int r = 0;
  auto p = bl.cbegin();
...
int KStore::_omap_rmkey_range(TransContext *txc, CollectionRef &c, OnodeRef &o,
...
int KStore::_setallochint(TransContext *txc, CollectionRef &c, OnodeRef &o,
...
int KStore::_clone(TransContext *txc, CollectionRef &c, OnodeRef &oldo,
                   OnodeRef &newo) {
...
int KStore::_clone_range(TransContext *txc, CollectionRef &c, OnodeRef &oldo,
                         OnodeRef &newo, uint64_t srcoff, uint64_t length,
                         uint64_t dstoff) {
  dout(15) << __func__ << " " << c->cid << " " << oldo->oid << " -> "
           << newo->oid << " from " << srcoff << "~" << length << " to offset "
           << dstoff << dendl;
  int r = 0;
...
int KStore::_rename(TransContext *txc, CollectionRef &c, OnodeRef &oldo,
...
int KStore::_create_collection(TransContext *txc, coll_t cid, unsigned bits,
                               CollectionRef *c) {
...
int KStore::_split_collection(TransContext *txc, CollectionRef &c,
...
int KStore::_merge_collection(TransContext *txc, CollectionRef *c,
