...
int aio_queue_t::submit_batch(aio_iter begin, aio_iter end, uint16_t aios_size,
  ...
  while (left > 0) {
    ...
    if (piocb[done]->n_aiocb == 1) {
      ...
      r = lio_listio(LIO_NOWAIT, &piocb[done]->aio.aiocbp, piocb[done]->n_aiocb,
                     &sev);
    }
#endif
    if (r < 0) {
