...
StupidAllocator::StupidAllocator(CephContext *cct, const std::string &name,
                                 int64_t _block_size)
    : Allocator(name), cct(cct), num_free(0), block_size(_block_size),
      free(10) {}
...
uint64_t
StupidAllocator::_aligned_len(StupidAllocator::interval_set_t::iterator p,
                              uint64_t alloc_unit) {
...
found:
  ...
  *length = std::min(std::max(alloc_unit, want_size),
                     p2align((p.get_len() - skew), alloc_unit));
