  ...
  explicit SocketHook(Allocator *alloc, const std::string &_name)
    ...
    if (admin_socket) {
      int r = admin_socket->register_command(
          ("bluestore allocator dump " + name).c_str(), this,
          "dump allocator free regions");
      if (r != 0)
        alloc = nullptr; // some collision, disable
      if (alloc) {
        r = admin_socket->register_command(
            ("bluestore allocator score " + name).c_str(), this,
            "give score on allocator fragmentation (0-no fragmentation, "
            "1-absolute fragmentation)");
        ceph_assert(r == 0);
        r = admin_socket->register_command(
            ("bluestore allocator fragmentation " + name).c_str(), this,
            "give allocator fragmentation (0-no fragmentation, 1-absolute "
            "fragmentation)");
        ceph_assert(r == 0);
      }
    }
...
Allocator::~Allocator() { delete asok_hook; }

Allocator *Allocator::create(CephContext *cct, string type, int64_t size,
  ...
  if (alloc == nullptr) {
    lderr(cct) << "Allocator::" << __func__ << " unknown alloc type " << type
               << dendl;
  }
  return alloc;
}
 ...
 * Assume block size X has value per-byte p; then block size 2*X will have
 * per-byte value 1.1*p.
 *
 * This could be expressed in logarithms, but for speed this is interpolated
 * inside ranges. [1]  [2..3] [4..7] [8..15] ... ^    ^      ^      ^ 1.1  1.1^2
 * 1.1^3  1.1^4 ...
 *
 * Final score is obtained by proportion between score that would have been
 * obtained in condition of absolute fragmentation and score in no fragmentation
 * at all.
 */
double Allocator::get_fragmentation_score() {
  // this value represents how much worth is 2X bytes in one chunk then in X + X
  // bytes
  ...
  auto get_score = [&](size_t v) -> double {
    size_t sc =
        sizeof(v) * 8 - clz(v) - 1; // assign to grade depending on log2(len)
    ...
    double x =
        double(v - sc_shifted) / sc_shifted; // x is <0,1) in its scale grade
