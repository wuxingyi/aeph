...
void BlueStore::SharedBlob::put() {
  if (--nref == 0) {
    ldout(coll->store->cct, 20)
        << __func__ << " " << this << " removing self from set " << get_parent()
        << dendl;
  again:
    auto coll_snap = coll;
    if (coll_snap) {
...
void BlueStore::_fsck_check_pool_statfs(
  ...
  if (!per_pool_stat_collection &&
      cct->_conf->bluestore_fsck_error_on_no_per_pool_stats && repairer) {
    ...
    for (it->lower_bound(string()); it->valid(); it->next()) {
      ...
      if (depth != FSCK_SHALLOW) {
        ...
        if (o->onode.has_omap()) {
          ...
          if (repairer && o->onode.has_omap() && !o->onode.is_perpool_omap() &&
              !o->oid.is_pgmeta()) {
...
int BlueStore::_fsck_on_open(BlueStore::FSCKDepth depth, bool repair) {
  ...
  // walk PREFIX_OBJ
  {
    ...
    BlueStore::FSCK_ObjectCtx ctx(
        ...
        &used_per_pool_omap_head, &used_pgmeta_omap_head, &sb_info_lock,
        sb_info, expected_store_statfs, expected_pool_statfs,
    ...
    if (it) {
      ...
      for (it->lower_bound(string()); it->valid(); it->next()) {
        ...
        if (used_omap_head.count(omap_head) == 0 &&
          ...
          derr << "fsck error: found stray omap data on omap_head " << omap_head
               << dendl;
          ++errors;
  ...
  if (repair) {
    if (!per_pool_omap && depth != FSCK_SHALLOW) {
