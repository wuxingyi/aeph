...
struct Task {
  ...
  ~Task() { ceph_assert(!io_request.nseg); }
...
int NVMEManager::try_get(const spdk_nvme_transport_id &trid,
  ...
  try {
    ...
    derr << __func__ << " invalid bluestore_spdk_coremask: " << coremask_arg
         << dendl;
    return -EINVAL;
  }
...
int NVMEDevice::read(uint64_t off, uint64_t len, bufferlist *pbl,
  ...
  t->ctx = ioc;
  char *buf = p.c_str();
  t->fill_cb = [buf, t]() { t->copy_to_buf(buf, 0, t->len); };
...
int NVMEDevice::read_random(uint64_t off, uint64_t len, char *buf,
  ...
  Task *t =
      new Task(this, IOCommand::READ_COMMAND, aligned_off, aligned_len, 1);
  int r = 0;
  t->ctx = &ioc;
