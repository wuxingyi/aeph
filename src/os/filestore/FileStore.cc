...
int FileStore::lfn_find(const ghobject_t &oid, const Index &index,
                        IndexedPath *path) {
  IndexedPath path2;
  if (!path)
    path = &path2;
...
int FileStore::lfn_truncate(const coll_t &cid, const ghobject_t &oid,
                            off_t length) {
  FDRef fd;
...
int FileStore::lfn_stat(const coll_t &cid, const ghobject_t &oid,
                        struct stat *buf) {
  IndexedPath path;
  Index index;
...
int FileStore::lfn_open(const coll_t &cid, const ghobject_t &oid, bool create,
                        FDRef *outfd, Index *index) {
  ceph_assert(outfd);
  int r = 0;
  bool need_lock = true;
  int flags = O_RDWR;
  ...
  if (create && (!exist)) {
    ...
    r = chain_fsetxattr<true, true>(fd, XATTR_SPILL_OUT_NAME,
                                    XATTR_NO_SPILL_OUT,
                                    sizeof(XATTR_NO_SPILL_OUT));
    if (r < 0) {
      VOID_TEMP_FAILURE_RETRY(::close(fd));
      derr << "error setting spillout xattr for oid " << oid << " ("
...
int FileStore::lfn_link(const coll_t &c, const coll_t &newcid,
                        const ghobject_t &o, const ghobject_t &newoid) {
...
int FileStore::lfn_unlink(const coll_t &cid, const ghobject_t &o,
...

  {
    ...
    if (force_clear_omap) {
      dout(20) << __FUNC__ << ": clearing omap on " << o << " in cid " << cid
               << dendl;
      r = object_map->clear(o, &spos);
      if (r < 0 && r != -ENOENT) {
        dout(25) << __FUNC__ << ": omap clear failed " << cpp_strerror(r)
                 << dendl;
        if (r == -EIO && m_filestore_fail_eio)
          handle_eio();
        return r;
...
FileStore::FileStore(CephContext *cct, const std::string &base,
    ...
    : JournalingObjectStore(cct, base), internal_name(name), basedir(base),
      journalpath(jdev), generic_flags(flags), blk_size(0), fsid_fd(-1),
      op_fd(-1), basedir_fd(-1), current_fd(-1), backend(nullptr),
      index_manager(cct, do_update), force_sync(false),
      timer(cct, sync_entry_timeo_lock), stop(false), sync_thread(this),
      fdcache(cct), wbthrottle(cct), next_osr_id(0),
      ...
      logger(nullptr), trace_endpoint("0.0.0.0", 0, "FileStore"),
      ...
      m_filestore_fadvise(cct->_conf->filestore_fadvise), do_update(do_update),
      ...
      m_filestore_do_dump(false), m_filestore_dump_fmt(true),
      ...
      m_fs_type(0), m_filestore_max_inline_xattr_size(0),
      m_filestore_max_inline_xattrs(0), m_filestore_max_xattr_value_size(0) {
...
int FileStore::mkfs() {
  ...
  if (ret < 0) {
    derr << __FUNC__ << ": failed to create "
         << cct->_conf->filestore_omap_backend << dendl;
    goto close_fsid_fd;
  }
  ...
  if (omap_fsid_fd < 0) {
    ret = -errno;
    derr << __FUNC__ << ": failed to open " << omap_fsid_fn << ": "
         << cpp_strerror(ret) << dendl;
    goto close_fsid_fd;
  }
  ...
  if (read_fsid(omap_fsid_fd, &old_omap_fsid) < 0 || old_omap_fsid.is_zero()) {
    ...
    dout(10) << __FUNC__ << ": write success, fsid:" << fsid_str
             << ", ret:" << ret << dendl;
    ...
    if (fsid != old_omap_fsid) {
      derr << __FUNC__ << ": " << omap_fsid_fn << " has existed omap fsid "
           << old_omap_fsid << " != expected osd fsid " << fsid << dendl;
      ret = -EINVAL;
      goto close_omap_fsid_fd;
    }
  ...
  dout(1) << cct->_conf->filestore_omap_backend << " db exists/created"
          << dendl;
...
int FileStore::_detect_fs() {
  ...
  if (tmpfd < 0) {
    int ret = -errno;
    derr << __FUNC__ << ": unable to create " << fn << ": " << cpp_strerror(ret)
         << dendl;
    return ret;
  }
...
int FileStore::update_version_stamp() { return write_version_stamp(); }
...
int FileStore::mount() {
  ...
  snprintf(omap_fsid_buf, sizeof(omap_fsid_buf), "%s/osd_uuid",
           omap_dir.c_str());
  ...
  if (::stat(omap_fsid_buf, &omap_fsid_stat) != 0) {
    ...
    if (fsid != omap_fsid) {
      derr << __FUNC__ << ": " << omap_fsid_buf << " has existed omap fsid "
           << omap_fsid << " != expected osd fsid " << fsid << dendl;
      ret = -EINVAL;
      goto close_current_fd;
    }
  }
  ...
  if (!(generic_flags & SKIP_MOUNT_OMAP)) {
    KeyValueDB *omap_store =
        KeyValueDB::create(cct, superblock.omap_backend, omap_dir);
    if (!omap_store) {
      derr << __FUNC__ << ": Error creating " << superblock.omap_backend
           << dendl;
      ret = -1;
      goto close_current_fd;
    }
    ...
    if (ret < 0) {
      derr << __FUNC__
           << ": Error initializing omap_store: " << cpp_strerror(ret) << dendl;
      goto close_current_fd;
    }
...
FileStore::create_new_collection(const coll_t &c) {
  std::lock_guard l{coll_lock};
  auto p = coll_map.find(c);
  if (p == coll_map.end()) {
...
int FileStore::queue_transactions(CollectionHandle &ch,
  ...
  // start on_readable finisher after we queue journal item, as on_readable
  // callback is allowed to delete the Transaction
  if (onreadable_sync) {
...
void FileStore::_journaled_ahead(OpSequencer *osr, Op *o, Context *ondisk) {
  ...
  // this should queue in order because the journal does it's completions in
  // order.
  queue_op(osr, o);
...
void FileStore::_set_global_replay_guard(const coll_t &cid,
  ...
  bufferlist v;
  encode(spos, v);
  int r = chain_fsetxattr<true, true>(fd, GLOBAL_REPLAY_GUARD_XATTR, v.c_str(),
                                      v.length());
  if (r < 0) {
...
int FileStore::_check_global_replay_guard(const coll_t &cid,
  ...
  if (fd < 0) {
    ...
    return 1; // if collection does not exist, there is no guard, and we can
              // replay.
  }
...
void FileStore::_set_replay_guard(int fd, const SequencerPosition &spos,
                                  const ghobject_t *hoid, bool in_progress) {
  if (backend->can_checkpoint())
    return;
  ...
  r = chain_fsetxattr<true, true>(fd, REPLAY_GUARD_XATTR, v.c_str(),
                                  v.length());
  if (r < 0) {
...
void FileStore::_close_replay_guard(int fd, const SequencerPosition &spos,
  ...
  bool in_progress = false;
  encode(in_progress, v);
  int r = chain_fsetxattr<true, true>(fd, REPLAY_GUARD_XATTR, v.c_str(),
                                      v.length());
  if (r < 0) {
    derr << "fsetxattr " << REPLAY_GUARD_XATTR << " got " << cpp_strerror(r)
         << dendl;
...
int FileStore::_check_replay_guard(const coll_t &cid,
                                   const SequencerPosition &spos) {
  if (!replaying || backend->can_checkpoint())
    return 1;
...
void FileStore::_do_transaction(Transaction &t, uint64_t op_seq, int trans_num,
  ...
  while (i.have_op()) {
    ...
    case Transaction::OP_CREATE: {
      ...
      const coll_t &cid =
          !_need_temp_object_collection(_cid, oid) ? _cid : _cid.get_temp();
    ...
    case Transaction::OP_SETATTR: {
      ...
      if (_check_replay_guard(cid, oid, spos) > 0) {
        ...
        if (r == -ENOSPC)
          dout(0) << " ENOSPC on setxattr on " << cid << "/" << oid << " name "
                  << name << " size " << bl.length() << dendl;
      }
    ...
    case Transaction::OP_CLONERANGE: {
      ...
      const coll_t &cid =
          !_need_temp_object_collection(_cid, oid) ? _cid : _cid.get_temp();
      const coll_t &ncid =
          !_need_temp_object_collection(_cid, noid) ? _cid : _cid.get_temp();
      uint64_t off = op->off;
      uint64_t len = op->len;
    ...
    case Transaction::OP_COLL_MOVE: {
      // WARNING: this is deprecated and buggy; only here to replay old
      // journals.
      const coll_t &ocid = i.get_cid(op->cid);
      const coll_t &ncid = i.get_cid(op->dest_cid);
      const ghobject_t &oid = i.get_oid(op->oid);
    ...
    case Transaction::OP_TRY_RENAME: {
      ...
      const ghobject_t &newoid = i.get_oid(op->dest_oid);
      const coll_t &oldcid =
          !_need_temp_object_collection(_cid, oldoid) ? _cid : _cid.get_temp();
      const coll_t &newcid =
          !_need_temp_object_collection(_cid, newoid) ? _cid : _cid.get_temp();
    ...
    if (r < 0) {
      ...
      if (!ok) {
        ...
        if (r == -ENOENT && (op->op == Transaction::OP_CLONERANGE ||
          ...
          msg = "EPERM suggests file(s) in osd data dir not owned by ceph "
                "user, or leveldb corruption";
        }
...
int FileStore::stat(CollectionHandle &ch, const ghobject_t &oid,
                    struct stat *st, bool allow_eio) {
  tracepoint(objectstore, stat_enter, ch->cid.c_str());
  auto osr = static_cast<OpSequencer *>(ch.get());
  osr->wait_for_apply(oid);
  const coll_t &cid = !_need_temp_object_collection(ch->cid, oid)
                          ? ch->cid
                          : ch->cid.get_temp();
...
int FileStore::set_collection_opts(CollectionHandle &ch,
                                   const pool_opts_t &opts) {
  return -EOPNOTSUPP;
}

int FileStore::read(CollectionHandle &ch, const ghobject_t &oid,
...
int FileStore::_truncate(const coll_t &cid, const ghobject_t &oid,
                         uint64_t size) {
...
int FileStore::_write(const coll_t &cid, const ghobject_t &oid, uint64_t offset,
...
int FileStore::_zero(const coll_t &cid, const ghobject_t &oid, uint64_t offset,
                     size_t len) {
  dout(15) << __FUNC__ << ": " << cid << "/" << oid << " " << offset << "~"
           << len << dendl;
  int ret = 0;
...
int FileStore::_clone(const coll_t &cid, const ghobject_t &oldoid,
                      const ghobject_t &newoid, const SequencerPosition &spos) {
...

  {
    ...
    if (r >= 0 &&
      ...
      r = chain_fsetxattr<true, true>(**n, XATTR_SPILL_OUT_NAME,
                                      XATTR_NO_SPILL_OUT,
                                      sizeof(XATTR_NO_SPILL_OUT));
    } else {
      r = chain_fsetxattr<true, true>(**n, XATTR_SPILL_OUT_NAME,
...
int FileStore::_clone_range(const coll_t &oldcid, const ghobject_t &oldoid,
                            const coll_t &newcid, const ghobject_t &newoid,
...
int FileStore::_fgetattrs(int fd, map<string, bufferptr> &aset) {
  ...
  while (name < end) {
    ...
    if (parse_attrname(&name)) {
      if (*name) {
        dout(20) << __FUNC__ << ": " << fd << " getting '" << name << "'"
                 << dendl;
...
int FileStore::getattr(CollectionHandle &ch, const ghobject_t &oid,
...
int FileStore::getattrs(CollectionHandle &ch, const ghobject_t &oid,
  ...
  if (!spill_out) {
    dout(10) << __FUNC__ << ": no xattr exists in object_map r = " << r
             << dendl;
    goto out;
  }
...
int FileStore::_setattrs(const coll_t &cid, const ghobject_t &oid,
                         map<string, bufferptr> &aset,
                         const SequencerPosition &spos) {
...
int FileStore::_rmattr(const coll_t &cid, const ghobject_t &oid,
  ...
  if (r == -ENODATA && spill_out) {
    ...
    if (r < 0 && r != -ENOENT) {
      dout(10) << __FUNC__ << ": could not remove_xattrs index r = " << r
               << dendl;
      if (r == -EIO && m_filestore_fail_eio)
        handle_eio();
      goto out_close;
...
int FileStore::_rmattrs(const coll_t &cid, const ghobject_t &oid,
  ...
  }
  {
    ...
    if (r < 0 && r != -ENOENT) {
      dout(10) << __FUNC__ << ": could not remove omap_attrs r = " << r
               << dendl;
      goto out_close;
    }
    if (r == -ENOENT)
...
int FileStore::_collection_remove_recursive(const coll_t &cid,
  ...
  while (!max.is_max()) {
    ...
    if (r < 0)
      return r;
    for (vector<ghobject_t>::iterator i = objects.begin(); i != objects.end();
         ++i) {
...
int FileStore::collection_empty(const coll_t &cid, bool *empty) {
  ...
  r = index->collection_list_partial(ghobject_t(), ghobject_t::get_max(), 1,
                                     &ls, nullptr);
  if (r < 0) {
    derr << __FUNC__
...
int FileStore::collection_list(const coll_t &c, const ghobject_t &orig_start,
...
int FileStore::omap_get(CollectionHandle &ch, const ghobject_t &hoid,
  ...
  const coll_t &c = !_need_temp_object_collection(ch->cid, hoid)
                        ? ch->cid
                        : ch->cid.get_temp();
...
int FileStore::omap_get_header(CollectionHandle &ch, const ghobject_t &hoid,
...
int FileStore::omap_get_keys(CollectionHandle &ch, const ghobject_t &hoid,
                             set<string> *keys) {
...
out:
  ...
  dout(15) << __FUNC__ << ": " << c << "/" << hoid << " = " << r << where
           << dendl;
  return r;
}
...
FileStore::get_omap_iterator(CollectionHandle &ch, const ghobject_t &oid) {
  auto osr = static_cast<OpSequencer *>(ch.get());
  osr->wait_for_apply(oid);
...
ObjectMap::ObjectMapIterator
FileStore::get_omap_iterator(const coll_t &_c, const ghobject_t &hoid) {
  tracepoint(objectstore, get_omap_iterator, _c.c_str());
  const coll_t &c =
      !_need_temp_object_collection(_c, hoid) ? _c : _c.get_temp();
  dout(15) << __FUNC__ << ": " << c << "/" << hoid << dendl;
  Index index;
  int r = get_index(c, &index);
  if (r < 0) {
...
int FileStore::_collection_hint_expected_num_objs(
    const coll_t &c, uint32_t pg_num, uint64_t expected_num_objs,
    const SequencerPosition &spos) {
...
int FileStore::_create_collection(const coll_t &c, int bits,
                                  const SequencerPosition &spos) {
  char fn[PATH_MAX];
  get_cdir(c, fn, sizeof(fn));
  dout(15) << __FUNC__ << ": " << fn << dendl;
...
int FileStore::_collection_add(const coll_t &c, const coll_t &oldcid,
                               const ghobject_t &o,
                               const SequencerPosition &spos) {
...
int FileStore::_collection_move_rename(const coll_t &oldcid,
                                       const ghobject_t &oldoid, coll_t c,
                                       const ghobject_t &o,
                                       const SequencerPosition &spos,
                                       bool allow_enoent) {
  dout(15) << __FUNC__ << ": " << c << "/" << o << " from " << oldcid << "/"
...
int FileStore::_merge_collection(const coll_t &cid, uint32_t bits, coll_t dest,
                                 const SequencerPosition &spos) {
  ...
  if (!r && cct->_conf->filestore_debug_verify_split) {
    ...
    ghobject_t next;
    while (1) {
      collection_list(dest, next, ghobject_t::get_max(), get_ideal_list_max(),
                      &objects, &next);
      if (objects.empty())
        break;
      for (vector<ghobject_t>::iterator i = objects.begin(); i != objects.end();
        ...
        if (!i->match(bits, pgid.pgid.ps())) {
          dout(20) << __FUNC__ << ": " << *i << " does not belong in " << cid
                   << dendl;
...
int FileStore::_split_collection(const coll_t &cid, uint32_t bits, uint32_t rem,
                                 coll_t dest, const SequencerPosition &spos) {
  int r;
  {
  ...
  if (!r && cct->_conf->filestore_debug_verify_split) {
    ...
    ghobject_t next;
    while (1) {
      collection_list(cid, next, ghobject_t::get_max(), get_ideal_list_max(),
                      &objects, &next);
      if (objects.empty())
        break;
      for (vector<ghobject_t>::iterator i = objects.begin(); i != objects.end();
...
uint64_t FileStore::estimate_objects_overhead(uint64_t num_objects) {
  uint64_t res = num_objects * blk_size /
                 2; // assumes that each object uses ( in average ) additional
                    // 1/2 block due to FS allocation granularity.
  return res;
}
