...
void PG::put(const char *tag) {
  ...
  auto local_cct = cct;
  int after = --ref;
  lgeneric_subdout(local_cct, refs, 5)
      << "PG::put " << this << " "
      << "tag " << (tag ? tag : "(none") << " " << (after + 1) << " -> "
      << after << dendl;
  if (after == 0)
    delete this;
}
...
uint64_t PG::get_with_id() {
  ...
  stringstream ss;
  bt.print(ss);
  lgeneric_subdout(cct, refs, 5)
      << "PG::get " << this << " " << info.pgid << " got id " << id << " "
      << (ref - 1) << " -> " << ref << dendl;
...
void PG::put_with_id(uint64_t id) {
  int newref = --ref;
  lgeneric_subdout(cct, refs, 5)
      << "PG::put " << this << " " << info.pgid << " put id " << id << " "
      << (newref + 1) << " -> " << newref << dendl;
  {
    std::lock_guard l(_ref_id_lock);
    ceph_assert(_live_ids.count(id));
    _live_ids.erase(id);
...
PG::PG(OSDService *o, OSDMapRef curmap, const PGPool &_pool, spg_t p)
    : pg_whoami(o->whoami, p.shard), pg_id(p), coll(p), osd(o), cct(o->cct),
      osdriver(osd->store, coll_t(), OSD::make_snapmapper_oid()),
      snap_mapper(cct, &osdriver, p.ps(),
                  p.get_split_bits(_pool.info.get_pg_num()), _pool.id, p.shard),
      trace_endpoint("0.0.0.0", 0, "PG"), info_struct_v(0),
      pgmeta_oid(p.make_pgmeta_oid()), stat_queue_item(this),
      scrub_queued(false), recovery_queued(false), recovery_ops_active(0),
      backfill_reserving(false), pg_stats_publish_valid(false),
      finish_sync_event(NULL), scrub_after_recovery(false), active_pushes(0),
      recovery_state(o->cct, pg_whoami, p, _pool, curmap, this, this),
      pool(recovery_state.get_pool()), info(recovery_state.get_info()) {
#ifdef PG_DEBUG_REFS
  osd->add_pgid(p, this);
#endif
#ifdef WITH_BLKIN
...
bool PG::is_locked() const { return ceph_mutex_is_locked(_lock); }
...
#endif
    out << "osd." << osd->whoami
        << " pg_epoch: " << (mapref ? mapref->get_epoch() : 0) << " " << *this
        << " ";
  } else {
    out << "osd." << osd->whoami
        << " pg_epoch: " << (mapref ? mapref->get_epoch() : 0) << " pg["
        << pg_id.pgid << "(unlocked)] ";
  }
  return out;
}

PerfCounters &PG::get_peering_perf() { return *(osd->recoverystate_perf); }

PerfCounters &PG::get_perf_logger() { return *(osd->logger); }

void PG::log_state_enter(const char *state) {
  osd->pg_recovery_stats.log_enter(state);
}

void PG::log_state_exit(const char *state_name, utime_t enter_time,
                        uint64_t events, utime_t event_dur) {
  osd->pg_recovery_stats.log_exit(state_name, ceph_clock_now() - enter_time,
                                  events, event_dur);
}

/********* PG **********/

void PG::remove_snap_mapped_object(ObjectStore::Transaction &t,
                                   const hobject_t &soid) {
  t.remove(coll, ghobject_t(soid, ghobject_t::NO_GEN, pg_whoami.shard));
  clear_object_snap_mapping(&t, soid);
}

void PG::clear_object_snap_mapping(ObjectStore::Transaction *t,
                                   const hobject_t &soid) {
...
void PG::update_object_snap_mapping(ObjectStore::Transaction *t,
                                    const hobject_t &soid,
...
PG::Scrubber::Scrubber()
    : local_reserved(false), remote_reserved(false), reserve_failed(false),
      epoch_start(0), active(false), shallow_errors(0), deep_errors(0),
      fixed(0), must_scrub(false), must_deep_scrub(false), must_repair(false),
      need_auto(false), time_for_deep(false), auto_repair(false),
      check_repair(false), deep_scrub_on_error(false),
      num_digest_updates_pending(0), state(INACTIVE), deep(false) {}
...
bool PG::op_has_sufficient_caps(OpRequestRef &op) {
  ...
  bool cap = caps.is_capable(pool.name, req->get_hobj().nspace,
                             pool.info.application_metadata, key,
                             op->need_read_cap(), op->need_write_cap(),
                             op->classes(), session->get_peer_socket_addr());

  dout(20) << "op_has_sufficient_caps "
           << "session=" << session << " pool=" << pool.id << " (" << pool.name
           << " " << req->get_hobj().nspace << ")"
           << " pool_app_metadata=" << pool.info.application_metadata
           << " need_read_cap=" << op->need_read_cap()
           << " need_write_cap=" << op->need_write_cap()
           << " classes=" << op->classes() << " -> " << (cap ? "yes" : "NO")
           << dendl;
  return cap;
}
...
bool PG::queue_scrub() {
  ...
  scrubber.priority = scrubber.must_scrub
                          ? cct->_conf->osd_requested_scrub_priority
                          : get_scrub_priority();
...
void PG::start_split_stats(const set<spg_t> &childpgs,
                           vector<object_stat_sum_t> *out) {
  recovery_state.start_split_stats(childpgs, out);
}

void PG::finish_split_stats(const object_stat_sum_t &stats,
...
void PG::add_backoff(const ceph::ref_t<Session> &s, const hobject_t &begin,
                     const hobject_t &end) {
  auto con = s->con;
  ...
  con->send_message(new MOSDBackoff(info.pgid, get_osdmap_epoch(),
                                    CEPH_OSD_BACKOFF_OP_BLOCK, b->id, begin,
                                    end));
}
...
void PG::release_backoffs(const hobject_t &begin, const hobject_t &end) {
  ...
  vector<ceph::ref_t<Backoff>> bv;
  {
    ...
    while (p != backoffs.end()) {
      ...
      dout(20) << __func__ << " ? " << r << " " << p->first << " " << p->second
               << dendl;
      // note: must still examine begin=end=p->first case
      if (r > 0 || (r == 0 && begin < end)) {
        break;
      }
      dout(20) << __func__ << " checking " << p->first << " " << p->second
               << dendl;
  ...
  for (auto b : bv) {
    ...
    if (b->session) {
      ...
      if (con) { // OSD::ms_handle_reset clears s->con without a lock
        con->send_message(new MOSDBackoff(info.pgid, get_osdmap_epoch(),
                                          CEPH_OSD_BACKOFF_OP_UNBLOCK, b->id,
                                          b->begin, b->end));
      }
      if (b->is_new()) {
...
void PG::set_probe_targets(const set<pg_shard_t> &probe_set) {
  ...
  probe_targets.clear();
  for (set<pg_shard_t>::iterator i = probe_set.begin(); i != probe_set.end();
       ++i) {
...
void PG::update_heartbeat_peers(set<int> new_peers) {
  ...
  if (new_peers == heartbeat_peers) {
    dout(10) << "update_heartbeat_peers " << heartbeat_peers << " unchanged"
             << dendl;
  } else {
    dout(10) << "update_heartbeat_peers " << heartbeat_peers << " -> "
             << new_peers << dendl;
...
bool PG::check_in_progress_op(
    const osd_reqid_t &r, eversion_t *version, version_t *user_version,
    int *return_code, vector<pg_log_op_return_item_t> *op_returns) const {
  return (projected_log.get_request(r, version, user_version, return_code,
                                    op_returns) ||
...
void PG::publish_stats_to_osd() {
  ...
  auto stats = recovery_state.prepare_stats_for_publish(
      pg_stats_publish_valid, pg_stats_publish, unstable_stats);
  if (stats) {
 ...
 */
void PG::init(int role, const vector<int> &newup, int new_up_primary,
              const vector<int> &newacting, int new_acting_primary,
              const pg_history_t &history, const PastIntervals &pi,
              bool backfill, ObjectStore::Transaction &t) {
  recovery_state.init(role, newup, new_up_primary, newacting,
...
void PG::upgrade(ObjectStore *store) {
  ...
  if (r != 0) {
    derr << __func__ << ": queue_transaction returned " << cpp_strerror(r)
         << dendl;
    ceph_abort();
  }
  ceph_assert(r == 0);
...
void PG::prepare_write(pg_info_t &info, pg_info_t &last_written_info,
                       PastIntervals &past_intervals, PGLog &pglog,
                       bool dirty_info, bool dirty_big_info,
                       bool need_write_epoch, ObjectStore::Transaction &t) {
  info.stats.stats.add(unstable_stats);
  unstable_stats.clear();
  map<string, bufferlist> km;
  if (dirty_big_info || dirty_info) {
    int ret = prepare_info_keymap(cct, &km, get_osdmap_epoch(), info,
                                  last_written_info, past_intervals,
                                  dirty_big_info, need_write_epoch,
                                  cct->_conf->osd_fast_info, osd->logger, this);
    ceph_assert(ret == 0);
  }
  pglog.write_log_and_missing(t, &km, coll, pgmeta_oid,
                              pool.info.require_rollback());
  if (!km.empty())
...
void PG::read_state(ObjectStore *store) {
  ...
  recovery_state.init_from_disk_state(
      std::move(info_from_disk), std::move(past_intervals_from_disk),
      [this, store](PGLog &pglog) {
        ostringstream oss;
        pglog.read_log_and_missing(
            store, ch, pgmeta_oid, info, oss,
  ...
  // initialize current mapping
  {
    ...
    get_osdmap()->pg_to_up_acting_osds(pg_id.pgid, &up, &up_primary, &acting,
                                       &primary);
...
void PG::update_snap_map(const vector<pg_log_entry_t> &log_entries,
                         ObjectStore::Transaction &t) {
  for (vector<pg_log_entry_t>::const_iterator i = log_entries.begin();
    ...
    if (i->soid.snap < CEPH_MAXSNAP) {
      if (i->is_delete()) {
        ...
        if (r != 0)
          derr << __func__ << " remove_oid " << i->soid << " failed with " << r
               << dendl;
...
void PG::requeue_ops(list<OpRequestRef> &ls) {
  for (list<OpRequestRef>::reverse_iterator i = ls.rbegin(); i != ls.rend();
       ++i) {
    requeue_op(*i);
  }
  ls.clear();
      ...
      for (auto q = p->second.rbegin(); q != p->second.rend(); ++q) {
        ...
        osd->enqueue_front(OpSchedulerItem(
            unique_ptr<OpSchedulerItem::OpQueueable>(
                new PGOpItem(info.pgid, req)),
            req->get_req()->get_cost(), req->get_req()->get_priority(),
            req->get_req()->get_recv_stamp(),
            req->get_req()->get_source().num(), epoch));
      }
    ...
    if (!scrubber.must_scrub) {
      ...
      if (allow_deep_scrub) {
        ...
        if (scrubber.need_auto) {
          ...
          pool.info.opts.get(pool_opts_t::DEEP_SCRUB_INTERVAL,
                             &deep_scrub_interval);
          if (deep_scrub_interval <= 0) {
            deep_scrub_interval = cct->_conf->osd_deep_scrub_interval;
          }
          scrubber.time_for_deep =
              ceph_clock_now() >=
              info.history.last_deep_scrub_stamp + deep_scrub_interval;

          bool deep_coin_flip = false;
          // If we randomize when !allow_scrub && allow_deep_scrub, then it
          // guarantees we will deep scrub because this function is called
          // often.
          if (!scrubber.time_for_deep && allow_scrub)
            deep_coin_flip = (rand() % 100) <
                             cct->_conf->osd_deep_scrub_randomize_ratio * 100;
          dout(20) << __func__ << ": time_for_deep=" << scrubber.time_for_deep
        ...
        if (try_to_auto_repair) {
          if (scrubber.time_for_deep) {
            dout(20) << __func__ << ": auto repair with deep scrubbing"
                     << dendl;
            scrubber.auto_repair = true;
          } else if (allow_scrub) {
            dout(20) << __func__
        ...
        if (has_deep_errors) {
          osd->clog->error() << "osd." << osd->whoami << " pg " << info.pgid
                             << " Regular scrub skipped due to deep-scrub "
                                "errors and nodeep-scrub set";
          return false;
        }
      }
...
bool PG::is_scrub_registered() { return !scrubber.scrub_reg_stamp.is_zero(); }
...
void PG::reg_next_scrub() {
  ...
  scrubber.scrub_reg_stamp = osd->reg_pg_scrub(
      info.pgid, reg_stamp, scrub_min_interval, scrub_max_interval, must);
...
void PG::clear_ready_to_merge() { osd->clear_ready_to_merge(this); }

void PG::queue_want_pg_temp(const vector<int> &wanted) {
  osd->queue_want_pg_temp(get_pgid().pgid, wanted);
}

void PG::clear_want_pg_temp() { osd->remove_want_pg_temp(get_pgid().pgid); }
...
epoch_t PG::oldest_stored_osdmap() { return osd->get_superblock().oldest_map; }

OstreamTemp PG::get_clog_info() { return osd->clog->info(); }

OstreamTemp PG::get_clog_debug() { return osd->clog->debug(); }

OstreamTemp PG::get_clog_error() { return osd->clog->error(); }
...
void PG::schedule_event_after(PGPeeringEventRef event, float delay) {
  ...
  osd->recovery_request_timer.add_event_after(
      delay, new QueuePeeringEvt(this, std::move(event)));
}
...
void PG::request_local_background_io_reservation(unsigned priority,
  ...
  osd->local_reserver.request_reservation(
      pg_id, on_grant ? new QueuePeeringEvt(this, on_grant) : nullptr, priority,
      on_preempt ? new QueuePeeringEvt(this, on_preempt) : nullptr);
}
...
void PG::request_remote_recovery_reservation(unsigned priority,
                                             PGPeeringEventRef on_grant,
                                             PGPeeringEventRef on_preempt) {
...
void PG::schedule_event_on_commit(ObjectStore::Transaction &t,
  ...
  if (i != new_removed_snaps.end()) {
    ...
    for (auto j : i->second) {
      if (snap_trimq.intersects(j.first, j.second)) {
        ...
        derr << __func__ << " removed_snaps already contains " << overlap
             << dendl;
        bad = true;
  ...
  if (j != new_purged_snaps.end()) {
    ...
    for (auto k : j->second) {
      if (!recovery_state.get_info().purged_snaps.contains(k.first, k.second)) {
        ...
        derr << __func__ << " purged_snaps does not contain " << rm << ", only "
             << overlap << dendl;
        recovery_state.adjust_purged_snaps(
            [&overlap](auto &purged_snaps) { purged_snaps.subtract(overlap); });
        ...
        recovery_state.adjust_purged_snaps([&k](auto &purged_snaps) {
          purged_snaps.erase(k.first, k.second);
        });
      }
    }
    dout(10) << __func__ << " new purged_snaps " << j->second << ", now "
             << recovery_state.get_info().purged_snaps << dendl;
    ceph_assert(!bad || !cct->_conf->osd_debug_verify_cached_snaps);
  }
}
...
void PG::queue_snap_retrim(snapid_t snap) {
  ...
  if (!snap_trimq.contains(snap)) {
    ...
    dout(20) << __func__ << " snap " << snap << ", trimq now " << snap_trimq
             << ", repeat " << snap_trimq_repeat << dendl;
    kick_snap_trim();
  } else {
    dout(20) << __func__ << " snap " << snap << " already in trimq "
             << snap_trimq << dendl;
  }
}
...
void PG::on_recovery_reserved() { queue_recovery(); }
...
void PG::send_pg_created(pg_t pgid) { osd->send_pg_created(pgid); }

ceph::signedspan PG::get_mnow() { return osd->get_mnow(); }
...
void PG::schedule_renew_lease(epoch_t lpr, ceph::timespan delay) {
  ...
  osd->mono_timer.add_event(
      delay, [o, lpr, spgid]() { o->queue_renew_lease(lpr, spgid); });
}
...
void PG::rebuild_missing_set_with_deletes(PGLog &pglog) {
  pglog.rebuild_missing_set_with_deletes(osd->store, ch,
...
void PG::do_replica_scrub_map(OpRequestRef op) {
  ...
  if (m->map_epoch < info.history.same_interval_since) {
    dout(10) << __func__ << " discarding old from " << m->map_epoch << " < "
             << info.history.same_interval_since << dendl;
    return;
  }
  ...
  dout(10) << "map version is " << scrubber.received_maps[m->from].valid_through
           << dendl;
...
void PG::_request_scrub_map(pg_shard_t replica, eversion_t version,
                            hobject_t start, hobject_t end, bool deep,
                            bool allow_preemption) {
  ceph_assert(replica != pg_whoami);
  dout(10) << "scrub  requesting scrubmap from osd." << replica << " deep "
           << (int)deep << dendl;
  MOSDRepScrub *repscrubop = new MOSDRepScrub(
      spg_t(info.pgid.pgid, replica.shard), version, get_osdmap_epoch(),
      get_last_peering_reset(), start, end, deep, allow_preemption,
      scrubber.priority, ops_blocked_by_scrub());
  // default priority, we want the rep scrub processed prior to any recovery
  // or client io messages (we are holding a lock!)
  osd->send_message_osd_cluster(replica.osd, repscrubop, get_osdmap_epoch());
}
...
void PG::handle_scrub_reserve_request(OpRequestRef op) {
  ...
  Message *reply = new MOSDScrubReserve(
      spg_t(info.pgid.pgid, get_primary().shard), m->map_epoch,
      scrubber.remote_reserved ? MOSDScrubReserve::GRANT
...
static int64_t pending_backfill(CephContext *cct, int64_t bf_bytes,
                                int64_t local_bytes) {
  lgeneric_dout(cct, 20) << __func__ << " Adjust local usage "
                         << (local_bytes >> 10) << "KiB"
                         << " primary usage " << (bf_bytes >> 10) << "KiB"
                         << dendl;
...
bool PG::try_reserve_recovery_space(int64_t primary_bytes,
                                    int64_t local_bytes) {
  ...
  if (cct->_conf->osd_debug_reject_backfill_probability > 0 &&
      (rand() % 1000 <
       (cct->_conf->osd_debug_reject_backfill_probability * 1000.0))) {
...
void PG::_scan_rollback_obs(const vector<ghobject_t> &rollback_obs) {
  ...
  for (vector<ghobject_t>::const_iterator i = rollback_obs.begin();
       i != rollback_obs.end(); ++i) {
    if (i->generation < trimmed_to.version) {
      dout(10) << __func__ << "osd." << osd->whoami << " pg " << info.pgid
               << " found obsolete rollback obj " << *i
               << " generation < trimmed_to " << trimmed_to << "...repaired"
               << dendl;
      t.remove(coll, *i);
    }
  }
...
void PG::_scan_snaps(ScrubMap &smap) {
  ...
  for (map<hobject_t, ScrubMap::object>::reverse_iterator i =
           smap.objects.rbegin();
       i != smap.objects.rend(); ++i) {
    ...
    if (hoid.snap < CEPH_MAXSNAP) {
      ...
      if (r == -ENOENT || cur_snaps != obj_snaps) {
        ...
        if (r == 0) {
          ...
          osd->clog->error()
              << "osd." << osd->whoami << " found snap mapper error on pg "
              << info.pgid << " oid " << hoid
          ...
          osd->clog->error()
              << "osd." << osd->whoami << " found snap mapper error on pg "
              << info.pgid << " oid " << hoid << " snaps missing in mapper"
  ...
  for (map<hobject_t, ScrubMap::object>::reverse_iterator i =
    ...
    if (oi.soid != hoid) {
      ...
      osd->clog->error() << "osd." << osd->whoami
                         << " found object info error on pg " << info.pgid
                         << " oid " << hoid
...
int PG::build_scrub_map_chunk(ScrubMap &map, ScrubMapBuilder &pos,
                              hobject_t start, hobject_t end, bool deep,
  ...
  while (pos.empty()) {
    ...
    pos.ret =
        get_pgbackend()->objects_list_range(start, end, &pos.ls, &rollback_obs);
    if (pos.ret < 0) {
...
void PG::repair_object(const hobject_t &soid,
...
void PG::scrub(epoch_t queued, ThreadPool::TPHandle &handle) {
  ...
  if (scrub_sleep > 0 &&
    ...
    auto scrub_requeue_callback = new LambdaContext([osds, pgid, state](int r) {
    ...
    osd->sleep_timer.add_event_after(scrub_sleep, scrub_requeue_callback);
  ...
  // for the replica
  if (!is_primary() && scrubber.state == PG::Scrubber::BUILD_MAP_REPLICA) {
    chunky_scrub(handle);
    return;
  }
  ...
  while (!done) {
    ...
    case PG::Scrubber::INACTIVE:
...

      {
        ...
        scrubber.store.reset(
            Scrub::Store::create(osd->store, &t, info.pgid, coll));
      ...
      scrubber.preempt_left =
          cct->_conf.get_val<uint64_t>("osd_scrub_max_preemptions");
    ...
    case PG::Scrubber::NEW_CHUNK:
...

      {
        ...
        ret = get_pgbackend()->objects_list_partial(start, min, max, &objects,
                                                    &candidate_end);
        ceph_assert(ret >= 0);
    ...
    case PG::Scrubber::WAIT_LAST_UPDATE:
      ...
      for (set<pg_shard_t>::iterator i = get_acting_recovery_backfill().begin();
        ...
        if (*i == pg_whoami)
          continue;
        _request_scrub_map(*i, scrubber.subset_last_update, scrubber.start,
                           scrubber.end, scrubber.deep,
    ...
    case PG::Scrubber::BUILD_MAP:
      ...
      ret = build_scrub_map_chunk(scrubber.primary_scrubmap,
                                  scrubber.primary_scrubmap_pos, scrubber.start,
                                  scrubber.end, scrubber.deep, handle);
      if (ret == -EINPROGRESS) {
        requeue_scrub();
        done = true;
        break;
    ...
    case PG::Scrubber::WAIT_DIGEST_UPDATES:
      if (scrubber.num_digest_updates_pending) {
        dout(10) << __func__ << " waiting on "
                 << scrubber.num_digest_updates_pending << " digest updates"
                 << dendl;
        done = true;
        break;
      }
    ...
    case PG::Scrubber::BUILD_MAP_REPLICA:
      ...
      if (scrub_preempted) {
        ...
        ret = build_scrub_map_chunk(
            scrubber.replica_scrubmap, scrubber.replica_scrubmap_pos,
            scrubber.start, scrubber.end, scrubber.deep, handle);
      }
      if (ret == -EINPROGRESS) {
        requeue_scrub();
        done = true;
      ...
      // reply
      {
        MOSDRepScrubMap *reply =
            new MOSDRepScrubMap(spg_t(info.pgid.pgid, get_primary().shard),
                                scrubber.replica_scrub_start, pg_whoami);
...
bool PG::range_intersects_scrub(const hobject_t &start, const hobject_t &end) {
  ...
  return (start < scrubber.max_end && end >= scrubber.start);
}
...
void PG::scrub_compare_maps() {
  ...
  map<hobject_t, pair<std::optional<uint32_t>, std::optional<uint32_t>>>
      missing_digest;
  ...
  stringstream ss;
  get_pgbackend()->be_omap_checks(maps, master_set, scrubber.omap_stats, ss);
  ...
  if (recovery_state.get_acting().size() > 1) {
    ...
    get_pgbackend()->be_compare_scrubmaps(
        maps, master_set, state_test(PG_STATE_REPAIR), scrubber.missing,
        scrubber.inconsistent, authoritative, missing_digest,
        scrubber.shallow_errors, scrubber.deep_errors, scrubber.store.get(),
        info.pgid, recovery_state.get_acting(), ss);
    dout(2) << ss.str() << dendl;
...
bool PG::scrub_process_inconsistent() {
  ...
  if (!scrubber.authoritative.empty()) {
    stringstream ss;
    ss << info.pgid << " " << mode << " " << scrubber.missing.size()
       << " missing, " << scrubber.inconsistent.size()
       << " inconsistent objects";
    dout(2) << ss.str() << dendl;
    osd->clog->error(ss);
    if (repair) {
      state_clear(PG_STATE_CLEAN);
      for (map<hobject_t, list<pair<ScrubMap::object, pg_shard_t>>>::iterator
               i = scrubber.authoritative.begin();
           i != scrubber.authoritative.end(); ++i) {
        auto missing_entry = scrubber.missing.find(i->first);
        if (missing_entry != scrubber.missing.end()) {
          repair_object(i->first, i->second, missing_entry->second);
          scrubber.fixed += missing_entry->second.size();
        }
        if (scrubber.inconsistent.count(i->first)) {
          repair_object(i->first, i->second, scrubber.inconsistent[i->first]);
          scrubber.fixed += missing_entry->second.size();
        }
      }
    }
...
void PG::scrub_finish() {
  ...
  if (repair && scrubber.auto_repair &&
      scrubber.authoritative.size() >
          cct->_conf->osd_scrub_auto_repair_num_errors) {
    state_clear(PG_STATE_REPAIR);
    repair = false;
  }
  bool deep_scrub = state_test(PG_STATE_DEEP_SCRUB);
  ...
  // repair.
  if (scrubber.deep_scrub_on_error && scrubber.authoritative.size() &&
      scrubber.authoritative.size() <=
          cct->_conf->osd_scrub_auto_repair_num_errors) {
  ...
  if (repair) {
    if (scrubber.fixed == scrubber.shallow_errors + scrubber.deep_errors) {
      ...
      dout(10) << __func__ << " "
               << (scrubber.shallow_errors + scrubber.deep_errors)
...
bool PG::old_peering_msg(epoch_t reply_epoch, epoch_t query_epoch) {
  if (get_last_peering_reset() > reply_epoch ||
    ...
    dout(10) << "old_peering_msg reply_epoch " << reply_epoch << " query_epoch "
             << query_epoch << " last_peering_reset "
             << get_last_peering_reset() << dendl;
    return true;
  }
  return false;
}
...
void PG::start_flush_on_transaction(ObjectStore::Transaction &t) {
  ...
  FlushStateRef flush_trigger(
      std::make_shared<FlushState>(this, get_osdmap_epoch()));
...
bool PG::try_flush_or_schedule_async() {
  ...
  Context *c = new QueuePeeringEvt(this, get_osdmap_epoch(),
...
bool PG::can_discard_op(OpRequestRef &op) {
  ...
  if (m->get_map_epoch() < info.history.same_primary_since) {
    dout(7) << " changed after " << m->get_map_epoch() << ", dropping " << *m
            << dendl;
    return true;
  }
  ...
  if (m->get_connection()->has_feature(CEPH_FEATURE_RESEND_ON_SPLIT)) {
    ...
    if (m->get_connection()->has_feature(CEPH_FEATURE_SERVER_NAUTILUS)) {
      ...
      if (m->get_map_epoch() < pool.info.get_last_force_op_resend()) {
        dout(7) << __func__ << " sent before last_force_op_resend "
                << pool.info.last_force_op_resend << ", dropping" << *m
                << dendl;
        return true;
      }
    } else {
      ...
      if (m->get_map_epoch() <
          pool.info.get_last_force_op_resend_prenautilus()) {
        dout(7) << __func__ << " sent before last_force_op_resend_prenautilus "
                << pool.info.last_force_op_resend_prenautilus << ", dropping"
                << *m << dendl;
        return true;
      }
    }
    if (m->get_map_epoch() < info.history.last_epoch_split) {
      dout(7) << __func__ << " pg split in " << info.history.last_epoch_split
              << ", dropping" << dendl;
      return true;
    }
    ...
    if (m->get_map_epoch() < pool.info.get_last_force_op_resend_preluminous()) {
      dout(7) << __func__ << " sent before last_force_op_resend_preluminous "
              << pool.info.last_force_op_resend_preluminous << ", dropping"
              << *m << dendl;
      return true;
    }
  }
...
bool PG::can_discard_replica_op(OpRequestRef &op) {
  ...
  if (old_peering_msg(m->map_epoch, m->map_epoch)) {
    dout(10) << "can_discard_replica_op pg changed " << info.history
             << " after " << m->map_epoch << ", dropping" << dendl;
    return true;
  }
  return false;
}
...
bool PG::can_discard_request(OpRequestRef &op) {
  ...
  case MSG_OSD_PG_RECOVERY_DELETE:
    return can_discard_replica_op<MOSDPGRecoveryDelete,
                                  MSG_OSD_PG_RECOVERY_DELETE>(op);

  case MSG_OSD_PG_RECOVERY_DELETE_REPLY:
    return can_discard_replica_op<MOSDPGRecoveryDeleteReply,
                                  MSG_OSD_PG_RECOVERY_DELETE_REPLY>(op);

  case MSG_OSD_EC_WRITE:
    return can_discard_replica_op<MOSDECSubOpWrite, MSG_OSD_EC_WRITE>(op);
  case MSG_OSD_EC_WRITE_REPLY:
    return can_discard_replica_op<MOSDECSubOpWriteReply,
                                  MSG_OSD_EC_WRITE_REPLY>(op);
  case MSG_OSD_EC_READ:
    return can_discard_replica_op<MOSDECSubOpRead, MSG_OSD_EC_READ>(op);
  case MSG_OSD_EC_READ_REPLY:
    return can_discard_replica_op<MOSDECSubOpReadReply, MSG_OSD_EC_READ_REPLY>(
        op);
  ...
  case MSG_OSD_PG_UPDATE_LOG_MISSING:
    return can_discard_replica_op<MOSDPGUpdateLogMissing,
                                  MSG_OSD_PG_UPDATE_LOG_MISSING>(op);
  case MSG_OSD_PG_UPDATE_LOG_MISSING_REPLY:
    return can_discard_replica_op<MOSDPGUpdateLogMissingReply,
                                  MSG_OSD_PG_UPDATE_LOG_MISSING_REPLY>(op);
...
void PG::queue_null(epoch_t msg_epoch, epoch_t query_epoch) {
  ...
  queue_peering_event(PGPeeringEventRef(
      std::make_shared<PGPeeringEvent>(msg_epoch, query_epoch, NullEvt())));
}
  ...
  if (!recovery_state.discover_all_missing(rctx)) {
    ...
    if (state_test(PG_STATE_BACKFILLING)) {
      auto evt = PGPeeringEventRef(
          new PGPeeringEvent(queued, queued, PeeringState::UnfoundBackfill()));
      queue_peering_event(evt);
      action = "in backfill";
    } else if (state_test(PG_STATE_RECOVERING)) {
      auto evt = PGPeeringEventRef(
          new PGPeeringEvent(queued, queued, PeeringState::UnfoundRecovery()));
      queue_peering_event(evt);
      action = "in recovery";
    } else {
      action = "already out of recovery/backfill";
    }
    dout(10) << __func__ << ": no luck, giving up on this pg for now ("
             << action << ")" << dendl;
  } else {
    dout(10) << __func__
             << ": no luck, giving up on this pg for now (queue_recovery)"
             << dendl;
    queue_recovery();
  }
}
...
void PG::handle_advance_map(OSDMapRef osdmap, OSDMapRef lastmap,
  ...
  recovery_state.advance_map(osdmap, lastmap, newup, up_primary, newacting,
                             acting_primary, rctx);
}
...
void PG::handle_query_state(Formatter *f) {
  ...
  if (is_primary() && is_active()) {
    ...
    f->dump_stream("scrubber.subset_last_update")
        << scrubber.subset_last_update;
    f->dump_bool("scrubber.deep", scrubber.deep);
    {
      ...
      for (set<pg_shard_t>::iterator p = scrubber.waiting_on_whom.begin();
           p != scrubber.waiting_on_whom.end(); ++p) {
...
void PG::do_delete_work(ObjectStore::Transaction &t) {
...

  {
    ...
    if (osd_delete_sleep > 0 && delete_needs_sleep) {
      ...
      auto delete_requeue_callback = new LambdaContext([this, pgref, e](int r) {
        dout(20) << __func__ << " wake up at " << ceph_clock_now()
      ...
      dout(20) << __func__ << " Delete scheduled at " << delete_schedule_time
               << dendl;
      return;
    }
  }
  ...
  ghobject_t next;
  osd->store->collection_list(ch, next, ghobject_t::get_max(), max, &olist,
                              &next);
  ...
  if (num) {
    ...
    if (!osd->try_finish_pg_delete(this, pool.info.get_pg_num())) {
      ...
      create_pg_collection(t, info.pgid,
...
int PG::pg_stat_adjust(osd_stat_t *ns) {
  ...
  if (reserved_num_bytes > 0) {
    ...
    dout(20) << __func__ << " reserved_num_bytes " << (reserved_num_bytes >> 10)
             << "KiB"
             << " Before kb_used " << new_stat.statfs.kb_used() << "KiB"
             << dendl;
