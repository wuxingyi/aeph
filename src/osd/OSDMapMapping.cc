...
void OSDMapMapping::_init_mappings(const OSDMap &osdmap) {
  ...
  for (auto &p : osdmap.get_pools()) {
    ...
    pools.emplace(p.first,
                  PoolMapping(p.second.get_size(), p.second.get_pg_num(),
                              p.second.is_erasure()));
  }
...
void OSDMapMapping::_update_range(const OSDMap &osdmap, int64_t pool,
  ...
  for (unsigned ps = pg_begin; ps < pg_end; ++ps) {
    ...
    osdmap.pg_to_up_acting_osds(pg_t(ps, pool), &up, &up_primary, &acting,
                                &acting_primary);
    i->second.set(ps, std::move(up), up_primary, std::move(acting),
                  acting_primary);
  }
}
...
void ParallelPGMapper::queue(Job *job, unsigned pgs_per_item,
