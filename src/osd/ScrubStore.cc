...
Store *Store::create(ObjectStore *store, ObjectStore::Transaction *t,
                     const spg_t &pgid, const coll_t &coll) {
  ceph_assert(store);
  ceph_assert(t);
...
Store::~Store() { ceph_assert(results.empty()); }
...
void Store::add_snap_error(int64_t pool,
                           const inconsistent_snapset_wrapper &e) {
  bufferlist bl;
  e.encode(bl);
  results[to_snap_key(pool, e.object)] = bl;
}

bool Store::empty() const { return results.empty(); }
...
void Store::cleanup(ObjectStore::Transaction *t) { t->remove(coll, hoid); }

std::vector<bufferlist>
Store::get_snap_errors(ObjectStore *store, int64_t pool,
...
std::vector<bufferlist>
Store::get_object_errors(ObjectStore *store, int64_t pool,
                         const librados::object_id_t &start,
                         uint64_t max_return) {
  const string begin = (start.name.empty() ? first_object_key(pool)
                                           : to_object_key(pool, start));
  const string end = last_object_key(pool);
  return get_errors(store, begin, end, max_return);
}

std::vector<bufferlist> Store::get_errors(ObjectStore *store,
                                          const string &begin,
                                          const string &end,
                                          uint64_t max_return) {
