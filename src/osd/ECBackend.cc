...
static ostream &operator<<(ostream &lhs,
                           const map<pg_shard_t, bufferlist> &rhs) {
  lhs << "[";
...
static ostream &operator<<(ostream &lhs, const map<int, bufferlist> &rhs) {
  lhs << "[";
  for (map<int, bufferlist>::const_iterator i = rhs.begin(); i != rhs.end();
       ++i) {
    if (i != rhs.begin())
      lhs << ", ";
    lhs << make_pair(i->first, i->second.length());
...
static ostream &operator<<(
  ...
  return lhs << "(" << rhs.get<0>() << ", " << rhs.get<1>() << ", "
             << rhs.get<2>() << ")";
}
...
ostream &operator<<(ostream &lhs, const ECBackend::ReadOp &rhs) {
  ...
  return lhs << ", to_read=" << rhs.to_read << ", complete=" << rhs.complete
...
ostream &operator<<(ostream &lhs, const ECBackend::Op &rhs) {
  lhs << "Op(" << rhs.hoid << " v=" << rhs.version << " tt=" << rhs.trim_to
      << " tid=" << rhs.tid << " reqid=" << rhs.reqid;
...
ECBackend::ECBackend(PGBackend::Listener *pg, const coll_t &coll,
                     ObjectStore::CollectionHandle &ch, ObjectStore *store,
                     CephContext *cct, ErasureCodeInterfaceRef ec_impl,
                     uint64_t stripe_width)
    : PGBackend(cct, pg, store, coll, ch), ec_impl(ec_impl),
...
void ECBackend::_failed_push(
    const hobject_t &hoid,
    pair<RecoveryMessages *, ECBackend::read_result_t &> &in) {
  ECBackend::read_result_t &res = in.second;
  dout(10) << __func__ << ": Read error " << hoid << " r=" << res.r
           << " errors=" << res.errors << dendl;
...
struct OnRecoveryReadComplete
    : public GenContext<
          pair<RecoveryMessages *, ECBackend::read_result_t &> &> {
  ECBackend *pg;
  hobject_t hoid;
  OnRecoveryReadComplete(ECBackend *pg, const hobject_t &hoid)
      : pg(pg), hoid(hoid) {}
  void
  finish(pair<RecoveryMessages *, ECBackend::read_result_t &> &in) override {
    ECBackend::read_result_t &res = in.second;
    ...
    pg->handle_recovery_read_complete(hoid, res.returned.back(), res.attrs,
                                      in.first);
  }
};
...
struct RecoveryMessages {
  ...
  void read(ECBackend *ec, const hobject_t &hoid, uint64_t off, uint64_t len,
    ...
    reads.insert(make_pair(
        hoid, ECBackend::read_request_t(to_read, need, attrs,
                                        new OnRecoveryReadComplete(ec, hoid))));
  }
...
void ECBackend::handle_recovery_push(const PushOp &op, RecoveryMessages *m,
                                     bool is_repair) {
  ...
  if (oneshot) {
    ...
    tobj =
        ghobject_t(get_parent()->get_temp_recovery_object(op.soid, op.version),
                   ghobject_t::NO_GEN, get_parent()->whoami_shard().shard);
    if (op.before_progress.first) {
      dout(10) << __func__ << ": Adding oid " << tobj.hobj
  ...
  if (get_parent()->pg_is_remote_backfilling()) {
    ...
    get_parent()->pg_add_num_bytes(op.data.length() *
                                   get_ec_data_chunk_count());
    dout(10) << __func__ << " " << op.soid << " add new actual data by "
             << op.data.length() << " add new num_bytes by "
             << op.data.length() * get_ec_data_chunk_count() << dendl;
  }
  ...
  if (op.after_progress.data_complete && !oneshot) {
    dout(10) << __func__ << ": Removing oid " << tobj.hobj
             << " from the temp collection" << dendl;
    clear_temp_obj(tobj.hobj);
    m->t.remove(coll, ghobject_t(op.soid, ghobject_t::NO_GEN,
                                 get_parent()->whoami_shard().shard));
    m->t.collection_move_rename(coll, tobj, coll,
                                ghobject_t(op.soid, ghobject_t::NO_GEN,
                                           get_parent()->whoami_shard().shard));
  }
  if (op.after_progress.data_complete) {
    if ((get_parent()->pgb_is_primary())) {
      ...
      get_parent()->on_local_recover(op.soid, op.recovery_info,
                                     recovery_ops[op.soid].obc, false, &m->t);
    } else {
      // If primary told us this is a repair, bump
      // osd_stat_t::num_objects_repaired
      if (is_repair)
        get_parent()->inc_osd_stat_repaired();
      get_parent()->on_local_recover(op.soid, op.recovery_info,
                                     ObjectContextRef(), false, &m->t);
      if (get_parent()->pg_is_remote_backfilling()) {
        struct stat st;
        int r = store->stat(ch,
                            ghobject_t(op.soid, ghobject_t::NO_GEN,
        ...
        if (r == 0) {
          ...
          get_parent()->pg_sub_num_bytes(st.st_size *
                                         get_ec_data_chunk_count());
          dout(10) << __func__ << " " << op.soid << " sub actual data by "
                   << st.st_size << " sub num_bytes by "
                   << st.st_size * get_ec_data_chunk_count() << dendl;
        }
      }
    }
  }
...
void ECBackend::handle_recovery_push_reply(const PushReplyOp &op,
                                           pg_shard_t from,
                                           RecoveryMessages *m) {
...
struct SendPushReplies : public Context {
  ...
  SendPushReplies(PGBackend::Listener *l, epoch_t epoch,
                  map<int, MOSDPGPushReply *> &in)
      : l(l), epoch(epoch) {
    replies.swap(in);
  }
...
void ECBackend::dispatch_recovery_messages(RecoveryMessages &m, int priority) {
  for (map<pg_shard_t, vector<PushOp>>::iterator i = m.pushes.begin();
       i != m.pushes.end(); m.pushes.erase(i++)) {
  ...
  for (map<pg_shard_t, vector<PushReplyOp>>::iterator i =
       ...
       i != m.push_replies.end(); m.push_replies.erase(i++)) {
  ...
  if (!replies.empty()) {
    (m.t).register_on_complete(get_parent()->bless_context(
        new SendPushReplies(get_parent(), get_osdmap_epoch(), replies)));
...
void ECBackend::continue_recovery_op(RecoveryOp &op, RecoveryMessages *m) {
  ...
  while (1) {
    ...
    case RecoveryOp::IDLE: {
      ...
      int r =
          get_min_avail_to_read_shards(op.hoid, want, true, false, &to_read);
      if (r != 0) {
      ...
      m->read(this, op.hoid, op.recovery_progress.data_recovered_to, amount,
              std::move(want), to_read, op.recovery_progress.first && !op.obc);
    ...
    case RecoveryOp::READING: {
      ...
      if (after_progress.data_recovered_to >= op.obc->obs.oi.size) {
        after_progress.data_recovered_to =
            sinfo.logical_to_next_stripe_offset(op.obc->obs.oi.size);
        after_progress.data_complete = true;
      }
      for (set<pg_shard_t>::iterator mi = op.missing_on.begin();
           mi != op.missing_on.end(); ++mi) {
        ...
        if (pop.data.length())
          pop.data_included.insert(sinfo.aligned_logical_offset_to_chunk_offset(
                                       op.recovery_progress.data_recovered_to),
                                   pop.data.length());
        ...
        if (*mi != get_parent()->primary_shard())
          get_parent()->begin_peer_recover(*mi, op.hoid);
      }
      ...
      if (op.waiting_on_pushes.empty()) {
        if (op.recovery_progress.data_complete) {
          ...
          for (set<pg_shard_t>::iterator i = op.missing_on.begin();
               i != op.missing_on.end(); ++i) {
            if (*i != get_parent()->primary_shard()) {
              dout(10) << __func__ << ": on_peer_recover on " << *i << ", obj "
                       << op.hoid << dendl;
...
int ECBackend::recover_object(const hobject_t &hoid, eversion_t v,
  ...
  for (set<pg_shard_t>::const_iterator i =
       ...
       i != get_parent()->get_acting_recovery_backfill_shards().end(); ++i) {
...
bool ECBackend::can_handle_while_inactive(OpRequestRef _op) { return false; }
...
bool ECBackend::_handle_message(OpRequestRef _op) {
  ...
  case MSG_OSD_EC_WRITE: {
    ...
    MOSDECSubOpWrite *op =
        static_cast<MOSDECSubOpWrite *>(_op->get_nonconst_req());
    parent->maybe_preempt_replica_scrub(op->op.soid);
    handle_sub_write(op->op.from, _op, op->op, _op->pg_trace);
    return true;
  }
  case MSG_OSD_EC_WRITE_REPLY: {
    const MOSDECSubOpWriteReply *op =
        static_cast<const MOSDECSubOpWriteReply *>(_op->get_req());
  ...
  case MSG_OSD_EC_READ: {
    ...
    get_parent()->send_message_osd_cluster(reply,
                                           _op->get_req()->get_connection());
    return true;
  }
  case MSG_OSD_EC_READ_REPLY: {
    ...
    MOSDECSubOpReadReply *op =
        static_cast<MOSDECSubOpReadReply *>(_op->get_nonconst_req());
    RecoveryMessages rm;
  ...
  case MSG_OSD_PG_PUSH_REPLY: {
    const MOSDPGPushReply *op =
        static_cast<const MOSDPGPushReply *>(_op->get_req());
    RecoveryMessages rm;
...
struct SubWriteCommitted : public Context {
  ...
  SubWriteCommitted(ECBackend *pg, OpRequestRef msg, ceph_tid_t tid,
                    eversion_t version, eversion_t last_complete,
                    const ZTracer::Trace &trace)
      : pg(pg), msg(msg), tid(tid), version(version),
        last_complete(last_complete), trace(trace) {}
  void finish(int) override {
    if (msg)
...
void ECBackend::sub_write_committed(ceph_tid_t tid, eversion_t version,
                                    eversion_t last_complete,
                                    const ZTracer::Trace &trace) {
  if (get_parent()->pgb_is_primary()) {
    ...
    get_parent()->send_message_osd_cluster(get_parent()->primary_shard().osd, r,
                                           get_osdmap_epoch());
  }
}

void ECBackend::handle_sub_write(pg_shard_t from, OpRequestRef msg,
                                 ECSubWrite &op, const ZTracer::Trace &trace) {
  if (msg)
  ...
  if (op.backfill_or_async_recovery) {
    for (set<hobject_t>::iterator i = op.temp_removed.begin();
         i != op.temp_removed.end(); ++i) {
      dout(10) << __func__ << ": removing object " << *i
               << " since we won't get the transaction" << dendl;
      localt.remove(coll, ghobject_t(*i, ghobject_t::NO_GEN,
                                     get_parent()->whoami_shard().shard));
    }
  }
  ...
  if (pmissing.is_missing(op.soid)) {
    async = true;
    dout(30) << __func__ << " is_missing " << pmissing.is_missing(op.soid)
             << dendl;
  ...
  get_parent()->log_operation(op.log_entries, op.updated_hit_set_history,
                              op.trim_to, op.roll_forward_to,
                              !op.backfill_or_async_recovery, localt, async);
  ...
  dout(30) << __func__ << " missing after"
           << get_parent()->get_log().get_missing().get_items() << dendl;
  if (op.at_version != eversion_t()) {
    // dummy rollforward transaction doesn't get at_version (and doesn't advance
    // it)
    get_parent()->op_applied(op.at_version);
  }
}

void ECBackend::handle_sub_read(pg_shard_t from, const ECSubRead &op,
  ...
  for (auto i = op.to_read.begin(); i != op.to_read.end(); ++i) {
    ...
    for (auto j = i->second.begin(); j != i->second.end(); ++j) {
      ...
      if ((op.subchunks.find(i->first)->second.size() == 1) &&
        ...
        dout(25) << __func__ << " case1: reading the complete chunk/shard."
                 << dendl;
        r = store->read(ch, ghobject_t(i->first, ghobject_t::NO_GEN, shard),
                        j->get<0>(), j->get<1>(), bl,
        ...
        for (int m = 0; m < (int)j->get<1>() && !error;
          ...
          for (auto &&k : op.subchunks.find(i->first)->second) {
            ...
            r = store->read(ch, ghobject_t(i->first, ghobject_t::NO_GEN, shard),
                            ...
                            (k.second) * subchunk_size, bl0, j->get<2>());
            if (r < 0) {
              error = true;
              break;
            }
      ...
      if (r < 0) {
        ...
        if (r == -ENOENT && get_parent()->get_pool().fast_read) {
          dout(5) << __func__ << ": Error " << r << " reading " << i->first
                  << ", fast read, probably ok" << dendl;
        } else {
          get_parent()->clog_error()
              << "Error " << r << " reading object " << i->first;
          dout(5) << __func__ << ": Error " << r << " reading " << i->first
                  << dendl;
        }
        goto error;
      } else {
        dout(20) << __func__ << " read request=" << j->get<1>() << " r=" << r
                 << " len=" << bl.length() << dendl;
      ...
      if (!get_parent()->get_pool().allows_ecoverwrites()) {
        ...
        if ((bl.length() == hinfo->get_total_chunk_size()) &&
          ...
          if (h.digest() != hinfo->get_chunk_hash(shard)) {
            get_parent()->clog_error()
                << "Bad hash for " << i->first << " digest 0x" << hex
                << h.digest() << " expected 0x" << hinfo->get_chunk_hash(shard)
                << dec;
  ...
  for (set<hobject_t>::iterator i = op.attrs_to_read.begin();
       i != op.attrs_to_read.end(); ++i) {
    dout(10) << __func__ << ": fulfilling attr request on " << *i << dendl;
    if (reply->errors.count(*i))
      continue;
    int r = store->getattrs(ch, ghobject_t(*i, ghobject_t::NO_GEN, shard),
                            reply->attrs_read[*i]);
    if (r < 0) {
...
void ECBackend::handle_sub_write_reply(pg_shard_t from,
...
void ECBackend::handle_sub_read_reply(pg_shard_t from, ECSubReadReply &op,
                                      RecoveryMessages *m,
                                      const ZTracer::Trace &trace) {
  ...
  for (auto i = op.buffers_read.begin(); i != op.buffers_read.end(); ++i) {
    ceph_assert(!op.errors.count(
        i->first)); // If attribute error we better not have sent a buffer
    ...
    list<boost::tuple<uint64_t, uint64_t, map<pg_shard_t, bufferlist>>>::
        iterator riter = rop.complete[i->first].returned.begin();
    for (list<pair<uint64_t, bufferlist>>::iterator j = i->second.begin();
         j != i->second.end(); ++j, ++req_iter, ++riter) {
  ...
  for (auto i = op.attrs_read.begin(); i != op.attrs_read.end(); ++i) {
    ceph_assert(!op.errors.count(
        i->first)); // if read error better not have sent an attribute
    if (!rop.to_read.count(i->first)) {
      // We canceled this read! @see filter_read_op
      dout(20) << __func__ << " to_read skipping" << dendl;
      continue;
  ...
  for (auto i = op.errors.begin(); i != op.errors.end(); ++i) {
    ...
    dout(20) << __func__ << " shard=" << from << " error=" << i->second
             << dendl;
  }
      ...
      if ((err = ec_impl->minimum_to_decode(rop.want_to_read[iter->first], have,
        ...
        if (rop.in_progress.empty()) {
          ...
          if (!rop.do_redundant_reads) {
            ...
            // Couldn't read any additional shards so handle as completed with
            // errors
          }
        ...
        if (!rop.complete[iter->first].errors.empty()) {
          if (cct->_conf->osd_read_ec_check_for_errors) {
            ...
            get_parent()->clog_warn() << "Error(s) ignored for " << iter->first
...
void ECBackend::filter_read_op(const OSDMapRef &osdmap, ReadOp &op) {
  ...
  for (set<hobject_t>::iterator i = to_cancel.begin(); i != to_cancel.end();
       ++i) {
    get_parent()->cancel_pull(*i);

    ceph_assert(op.to_read.count(*i));
    read_request_t &req = op.to_read.find(*i)->second;
    dout(10) << __func__ << ": canceling " << req << "  for obj " << *i
             << dendl;
    ceph_assert(req.cb);
    delete req.cb;
    req.cb = nullptr;
...
void ECBackend::check_recovery_sources(const OSDMapRef &osdmap) {
  ...
  for (map<pg_shard_t, set<ceph_tid_t>>::iterator i = shard_to_read_map.begin();
...
void ECBackend::on_change() {
  ...
  for (map<ceph_tid_t, ReadOp>::iterator i = tid_to_read_map.begin();
       i != tid_to_read_map.end(); ++i) {
    dout(10) << __func__ << ": cancelling " << i->second << dendl;
    for (map<hobject_t, read_request_t>::iterator j = i->second.to_read.begin();
         j != i->second.to_read.end(); ++j) {
      delete j->second.cb;
      j->second.cb = nullptr;
    }
  }
...
void ECBackend::clear_recovery_state() { recovery_ops.clear(); }
...
void ECBackend::submit_transaction(
    const hobject_t &hoid, const object_stat_sum_t &delta_stats,
    const eversion_t &at_version, PGTransactionUPtr &&t,
...
void ECBackend::get_all_avail_shards(const hobject_t &hoid,
  ...
  for (set<pg_shard_t>::const_iterator i =
       ...
       i != get_parent()->get_acting_shards().end(); ++i) {
  ...
  if (for_recovery) {
    for (set<pg_shard_t>::const_iterator i =
         ...
         i != get_parent()->get_backfill_shards().end(); ++i) {
      if (error_shards.find(*i) != error_shards.end())
        continue;
...
int ECBackend::get_min_avail_to_read_shards(
    const hobject_t &hoid, const set<int> &want, bool for_recovery,
...
int ECBackend::get_remaining_shards(
    const hobject_t &hoid, const set<int> &avail, const set<int> &want,
    const read_result_t &result,
    map<pg_shard_t, vector<pair<int, int>>> *to_read, bool for_recovery) {
  ceph_assert(to_read);
  ...
  for (set<int>::iterator i = shards_left.begin(); i != shards_left.end();
       ++i) {
...
void ECBackend::start_read_op(int priority,
                              map<hobject_t, set<int>> &want_to_read,
                              map<hobject_t, read_request_t> &to_read,
                              OpRequestRef _op, bool do_redundant_reads,
                              bool for_recovery) {
  ...
  auto &op =
      tid_to_read_map
          .emplace(tid,
                   ReadOp(priority, tid, do_redundant_reads, for_recovery, _op,
                          std::move(want_to_read), std::move(to_read)))
          .first->second;
    ...
    for (list<boost::tuple<uint64_t, uint64_t, uint32_t>>::const_iterator j =
      ...
      pair<uint64_t, uint64_t> chunk_off_len =
          sinfo.aligned_offset_len_to_chunk(
              make_pair(j->get<0>(), j->get<1>()));
      for (auto k = i->second.need.begin(); k != i->second.need.end(); ++k) {
        messages[k->first].to_read[i->first].push_back(boost::make_tuple(
            chunk_off_len.first, chunk_off_len.second, j->get<2>()));
      }
...
ECUtil::HashInfoRef
ECBackend::get_hash_info(const hobject_t &hoid, bool checks,
  ...
  if (!ref) {
    ...
    struct stat st;
    int r = store->stat(ch,
                        ghobject_t(hoid, ghobject_t::NO_GEN,
                                   get_parent()->whoami_shard().shard),
                        &st);
    ECUtil::HashInfo hinfo(ec_impl->get_chunk_count());
    ...
    if (r >= 0) {
      ...
      bufferlist bl;
      if (attrs) {
        map<string, bufferptr>::const_iterator k =
            attrs->find(ECUtil::get_hinfo_key());
        if (k == attrs->end()) {
        ...
        r = store->getattr(ch,
                           ghobject_t(hoid, ghobject_t::NO_GEN,
                                      get_parent()->whoami_shard().shard),
                           ECUtil::get_hinfo_key(), bl);
        if (r < 0) {
          dout(5) << __func__ << ": getattr failed: " << cpp_strerror(r)
                  << dendl;
  ...
  if (op->using_cache) {
    ...
    for (auto &&hpair : op->plan.will_write) {
      ...
      extent_set remote_read = cache.reserve_extents_for_rmw(
          hpair.first, op->pin, hpair.second, to_read_plan);
...
bool ECBackend::try_reads_to_commit() {
  ...
  if (op->using_cache) {
    for (auto &&hpair : op->pending_read) {
      op->remote_read_result[hpair.first].insert(
          cache.get_remaining_extents_for_rmw(hpair.first, op->pin,
                                              hpair.second));
    }
  ...
  if (op->plan.t) {
    ECTransaction::generate_transactions(
        op->plan, ec_impl, get_parent()->get_info().pgid.pgid, sinfo,
        op->remote_read_result, op->log_entries, &written, &trans,
        &(op->temp_added), &(op->temp_cleared), get_parent()->get_dpp(),
  ...
  for (set<pg_shard_t>::const_iterator i =
    ...
    ECSubWrite sop(get_parent()->whoami_shard(), op->tid, op->reqid, op->hoid,
                   stats, should_send ? iter->second : empty, op->version,
                   op->trim_to, op->roll_forward_to, op->log_entries,
                   op->updated_hit_set_history, op->temp_added,
                   op->temp_cleared, !should_send);
  ...
  if (should_write_local) {
    handle_sub_write(get_parent()->whoami_shard(), op->client_op,
                     local_write_op, op->trace);
  }

  for (auto i = op->on_write.begin(); i != op->on_write.end();
       op->on_write.erase(i++)) {
    (*i)();
  }
...
bool ECBackend::try_finish_rmw() {
  ...
  if (waiting_reads.empty() && waiting_commit.empty()) {
    pipeline_state.clear();
    dout(20) << __func__ << ": clearing pipeline_state " << pipeline_state
             << dendl;
  }
  return true;
}
...
int ECBackend::objects_read_sync(const hobject_t &hoid, uint64_t off,
                                 uint64_t len, uint32_t op_flags,
                                 bufferlist *bl) {
  return -EOPNOTSUPP;
}
...
void ECBackend::objects_read_async(
  ...
  for (list<pair<boost::tuple<uint64_t, uint64_t, uint32_t>,
           ...
           to_read.begin();
       i != to_read.end(); ++i) {
    pair<uint64_t, uint64_t> tmp = sinfo.offset_len_to_stripe_bounds(
  ...
  struct cb {
    ...
    cb(ECBackend *ec, const hobject_t &hoid,
       const list<pair<boost::tuple<uint64_t, uint64_t, uint32_t>,
                       pair<bufferlist *, Context *>>> &to_read,
       Context *on_complete)
        : ec(ec), hoid(hoid), to_read(to_read), on_complete(on_complete) {}
    void operator()(map<hobject_t, pair<int, extent_map>> &&results) {
      ...
      for (auto &&read : to_read) {
        if (got.first < 0) {
          ...
          ldpp_dout(dpp, 30)
              << "range offset: " << range.first.get_off() << dendl;
          ldpp_dout(dpp, 30) << "length: " << length << dendl;
          ldpp_dout(dpp, 30)
              << "range length: " << range.first.get_len() << dendl;
          ceph_assert((offset + length) <=
                      (range.first.get_off() + range.first.get_len()));
          read.second.first->substr_of(range.first.get_val(),
                                       offset - range.first.get_off(), length);
          if (read.second.second) {
  ...
  objects_read_and_reconstruct(
      reads, fast_read,
      make_gen_lambda_context<map<hobject_t, pair<int, extent_map>> &&, cb>(
          cb(this, hoid, to_read, on_complete)));
}
...
struct CallClientContexts
  ...
  list<boost::tuple<uint64_t, uint64_t, uint32_t>> to_read;
  CallClientContexts(
      hobject_t hoid, ECBackend *ec, ECBackend::ClientAsyncReadStatus *status,
  ...
  finish(pair<RecoveryMessages *, ECBackend::read_result_t &> &in) override {
    ...
    for (auto &&read : to_read) {
      pair<uint64_t, uint64_t> adjusted = ec->sinfo.offset_len_to_stripe_bounds(
...
void ECBackend::objects_read_and_reconstruct(
  ...
  for (auto &&to_read : reads) {
    ...
    int r = get_min_avail_to_read_shards(to_read.first, want_to_read, false,
                                         fast_read, &shards);
    ceph_assert(r == 0);

    CallClientContexts *c = new CallClientContexts(
        to_read.first, this, &(in_progress_client_reads.back()),
        to_read.second);
    for_read_op.insert(make_pair(
        to_read.first, read_request_t(to_read.second, shards, false, c)));
    obj_want_to_read.insert(make_pair(to_read.first, want_to_read));
  }

  start_read_op(CEPH_MSG_PRIO_DEFAULT, obj_want_to_read, for_read_op,
                OpRequestRef(), fast_read, false);
  return;
}
...
int ECBackend::send_all_remaining_reads(const hobject_t &hoid, ReadOp &rop) {
  ...
  // (Note cuixf) If we need to read attrs and we read failed, try to read
  // again.
  bool want_attrs =
  ...
  rop.to_read.insert(
      make_pair(hoid, read_request_t(offsets, shards, want_attrs, c)));
  do_read_op(rop);
  return 0;
}

int ECBackend::objects_get_attrs(const hobject_t &hoid,
...
void ECBackend::rollback_append(const hobject_t &hoid, uint64_t old_size,
                                ObjectStore::Transaction *t) {
...
int ECBackend::be_deep_scrub(const hobject_t &poid, ScrubMap &map,
                             ScrubMapBuilder &pos, ScrubMap::object &o) {
  ...
  r = store->read(
      ...
      pos.data_pos, stride, bl, fadvise_flags);
  if (r < 0) {
  ...
  if (bl.length() % sinfo.get_chunk_size()) {
    dout(20) << __func__ << "  " << poid << " got " << r
             << " on read, not chunk size " << sinfo.get_chunk_size()
             << " aligned" << dendl;
    o.read_error = true;
    return 0;
  }
  ...
  if (!hinfo) {
    dout(0) << "_scan_list  " << poid << " could not retrieve hash info"
            << dendl;
    o.read_error = true;
    ...
    if (!get_parent()->get_pool().allows_ecoverwrites()) {
      ...
      if (hinfo->get_total_chunk_size() != (unsigned)pos.data_pos) {
        dout(0) << "_scan_list  " << poid << " got incorrect size on read 0x"
                << std::hex << pos << " expected 0x"
                << hinfo->get_total_chunk_size() << std::dec << dendl;
