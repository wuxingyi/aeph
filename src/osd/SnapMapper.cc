...
int OSDriver::get_next(const std::string &key,
...
string SnapMapper::get_prefix(int64_t pool, snapid_t snap) {
  char buf[100];
  int len = snprintf(buf, sizeof(buf), "%lld_%.*X_", (long long)pool,
...
string SnapMapper::to_raw_key(const pair<snapid_t, hobject_t> &in) {
  return get_prefix(in.second.pool, in.first) + shard_prefix +
         in.second.to_str();
}

pair<string, bufferlist>
SnapMapper::to_raw(const pair<snapid_t, hobject_t> &in) {
  bufferlist bl;
  encode(Mapping(in), bl);
  return make_pair(to_raw_key(in), bl);
}

pair<snapid_t, hobject_t>
SnapMapper::from_raw(const pair<std::string, bufferlist> &image) {
  using ceph::decode;
  Mapping map;
...
bool SnapMapper::check(const hobject_t &hoid) const {
  ...
  derr << __func__ << " " << hoid << " mask_bits " << mask_bits << " match 0x"
       << std::hex << match << std::dec << " is false" << dendl;
  return false;
}
...
void SnapMapper::clear_snaps(
    const hobject_t &oid, MapCacher::Transaction<std::string, bufferlist> *t) {
...
int SnapMapper::update_snaps(
    const hobject_t &oid, const set<snapid_t> &new_snaps,
  ...
  set<string> to_remove;
  for (set<snapid_t>::iterator i = out.snaps.begin(); i != out.snaps.end();
       ++i) {
...
int SnapMapper::get_next_objects_to_trim(snapid_t snap, unsigned max,
                                         vector<hobject_t> *out) {
  ceph_assert(out);
  ceph_assert(out->empty());
  int r = 0;
  for (set<string>::iterator i = prefixes.begin();
       i != prefixes.end() && out->size() < max && r == 0; ++i) {
...
int SnapMapper::remove_oid(const hobject_t &oid,
                           MapCacher::Transaction<std::string, bufferlist> *t) {
  dout(20) << __func__ << " " << oid << dendl;
  ceph_assert(check(oid));
...
void SnapMapper::make_purged_snap_key_value(int64_t pool, snapid_t begin,
                                            snapid_t end,
...
int SnapMapper::_lookup_purged_snap(CephContext *cct, ObjectStore *store,
                                    ObjectStore::CollectionHandle &ch,
                                    const ghobject_t &hoid, int64_t pool,
                                    snapid_t snap, snapid_t *begin,
                                    snapid_t *end) {
  string k = make_purged_snap_key(pool, snap);
  auto it = store->get_omap_iterator(ch, hoid);
  it->lower_bound(k);
  if (!it->valid()) {
    dout(20) << __func__ << " pool " << pool << " snap " << snap << " key '"
             << k << "' lower_bound not found" << dendl;
    return -ENOENT;
  }
  if (it->key().find(PURGED_SNAP_PREFIX) != 0) {
    dout(20) << __func__ << " pool " << pool << " snap " << snap << " key '"
             << k << "' lower_bound got mismatched prefix '" << it->key() << "'"
             << dendl;
    return -ENOENT;
  }
  ...
  if (snap < *begin || snap >= *end) {
    dout(20) << __func__ << " pool " << pool << " snap " << snap << " found ["
             << *begin << "," << *end << "), no overlap" << dendl;
    return -ENOENT;
  }
  return 0;
}
...
void SnapMapper::record_purged_snaps(
    CephContext *cct, ObjectStore *store, ObjectStore::CollectionHandle &ch,
    ghobject_t hoid, ObjectStore::Transaction *t,
    map<epoch_t, mempool::osdmap::map<int64_t, snap_interval_set_t>>
        purged_snaps) {
  ...
  for (auto &[epoch, bypool] : purged_snaps) {
    ...
    for (auto &[pool, snaps] : bypool) {
      for (auto i = snaps.begin(); i != snaps.end(); ++i) {
        ...
        int b = _lookup_purged_snap(cct, store, ch, hoid, pool, begin - 1,
                                    &before_begin, &before_end);
        int a = _lookup_purged_snap(cct, store, ch, hoid, pool, end,
                                    &after_begin, &after_end);
        if (!b && !a) {
          dout(10) << __func__ << " [" << begin << "," << end << ") - joins ["
          ...
          dout(10) << __func__ << " [" << begin << "," << end
                   << ") - join with earlier [" << before_begin << ","
                   << before_end << ")" << dendl;
          rm.insert(make_purged_snap_key(pool, before_end - 1));
          make_purged_snap_key_value(pool, before_begin, end, &m);
        } else if (!a) {
          dout(10) << __func__ << " [" << begin << "," << end
                   << ") - join with later [" << after_begin << "," << after_end
                   << ")" << dendl;
...
bool SnapMapper::Scrubber::_parse_p() {
  ...
  ceph::decode(begin, p);
  ceph::decode(end, p);
  dout(20) << __func__ << " purged_snaps pool " << pool << " [" << begin << ","
           << end << ")" << dendl;
  psit->next();
  return true;
}
...
bool SnapMapper::Scrubber::_parse_m() {
  ...
  dout(20) << __func__ << " mapping pool " << mapping.hoid.pool << " snap "
           << mapping.snap << " shard " << shard << " " << mapping.hoid
           << dendl;
  mapit->next();
  return true;
}
...
void SnapMapper::Scrubber::run() {
  ...
  while (_parse_m()) {
    ...
    while (pool >= 0 && (mapping.hoid.pool > pool ||
    ...
    if (mapping.hoid.pool < pool || mapping.snap < begin) {
      // ok
      dout(20) << __func__ << " ok " << mapping.hoid << " snap " << mapping.snap
               << " precedes pool " << pool << " purged_snaps [" << begin << ","
               << end << ")" << dendl;
    } else {
      assert(mapping.snap >= begin && mapping.snap < end &&
             mapping.hoid.pool == pool);
      // invalid
      dout(10) << __func__ << " stray " << mapping.hoid << " snap "
               << mapping.snap << " in pool " << pool << " shard " << shard
...
string SnapMapper::get_legacy_prefix(snapid_t snap) {
  char buf[100];
  int len = snprintf(buf, sizeof(buf), "%.*X_", (int)(sizeof(snap) * 2),
...
int SnapMapper::convert_legacy(CephContext *cct, ObjectStore *store,
                               ObjectStore::CollectionHandle &ch,
  ...
  while (iter->valid()) {
    ...
    if (valid) {
      ...
      auto bp = bl.cbegin();
      decode(m, bp);
      to_set.emplace(SnapMapper::get_prefix(m.hoid.pool, m.snap), bl);
      ++n;
      iter->next();
    }
