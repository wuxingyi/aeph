...
void object_locator_t::encode(ceph::buffer::list &bl) const {
  ...
  if (hash != -1)
    encode_compat =
        std::max<std::uint8_t>(encode_compat, 6); // need to interpret the hash
...
void request_redirect_t::generate_test_instances(
...
void objectstore_perf_stat_t::encode(ceph::buffer::list &bl,
...
void objectstore_perf_stat_t::generate_test_instances(
...
void osd_stat_t::dump(Formatter *f) const {
  ...
  for (auto &i : hb_pingtime) {
    ...
    f->dump_format_unquoted(
        "1min", "%s", fixed_u_to_string(i.second.back_pingtime[0], 3).c_str());
    f->dump_format_unquoted(
        "5min", "%s", fixed_u_to_string(i.second.back_pingtime[1], 3).c_str());
    f->dump_format_unquoted(
        "15min", "%s", fixed_u_to_string(i.second.back_pingtime[2], 3).c_str());
    f->close_section(); // average
    f->open_object_section("min");
    f->dump_format_unquoted("1min", "%s",
                            fixed_u_to_string(i.second.back_min[0], 3).c_str());
    f->dump_format_unquoted("5min", "%s",
                            fixed_u_to_string(i.second.back_min[1], 3).c_str());
    f->dump_format_unquoted("15min", "%s",
                            fixed_u_to_string(i.second.back_min[2], 3).c_str());
    f->close_section(); // min
    f->open_object_section("max");
    f->dump_format_unquoted("1min", "%s",
                            fixed_u_to_string(i.second.back_max[0], 3).c_str());
    f->dump_format_unquoted("5min", "%s",
                            fixed_u_to_string(i.second.back_max[1], 3).c_str());
    f->dump_format_unquoted("15min", "%s",
                            fixed_u_to_string(i.second.back_max[2], 3).c_str());
    f->close_section(); // max
    f->dump_format_unquoted("last", "%s",
                            fixed_u_to_string(i.second.back_last, 3).c_str());
    ...
    if (i.second.front_pingtime[0] != 0) {
      ...
      f->dump_format_unquoted(
          "1min", "%s",
          fixed_u_to_string(i.second.front_pingtime[0], 3).c_str());
      f->dump_format_unquoted(
          "5min", "%s",
          fixed_u_to_string(i.second.front_pingtime[1], 3).c_str());
      f->dump_format_unquoted(
          "15min", "%s",
          fixed_u_to_string(i.second.front_pingtime[2], 3).c_str());
      f->close_section(); // average
      f->open_object_section("min");
      f->dump_format_unquoted(
          "1min", "%s", fixed_u_to_string(i.second.front_min[0], 3).c_str());
      f->dump_format_unquoted(
          "5min", "%s", fixed_u_to_string(i.second.front_min[1], 3).c_str());
      f->dump_format_unquoted(
          "15min", "%s", fixed_u_to_string(i.second.front_min[2], 3).c_str());
      f->close_section(); // min
      f->open_object_section("max");
      f->dump_format_unquoted(
          "1min", "%s", fixed_u_to_string(i.second.front_max[0], 3).c_str());
      f->dump_format_unquoted(
          "5min", "%s", fixed_u_to_string(i.second.front_max[1], 3).c_str());
      f->dump_format_unquoted(
          "15min", "%s", fixed_u_to_string(i.second.front_max[2], 3).c_str());
      f->close_section(); // max
      f->dump_format_unquoted(
          "last", "%s", fixed_u_to_string(i.second.front_last, 3).c_str());
      f->close_section(); // interface
    }
...
void osd_stat_t::generate_test_instances(std::list<osd_stat_t *> &o) {
  ...
  struct Interfaces gen_interfaces = {
      123456789, {1000, 900, 800},  {990, 890, 790},  {1010, 910, 810},
      1001,      {1100, 1000, 900}, {1090, 990, 890}, {1110, 1010, 910},
      1101};
...
bool pg_t::is_split(unsigned old_pg_num, unsigned new_pg_num,
                    set<pg_t> *children) const {
...
bool pg_t::is_merge_source(unsigned old_pg_num, unsigned new_pg_num,
                           pg_t *parent) const {
...
hobject_t pg_t::get_hobj_end(unsigned pg_num) const {
  ...
  if (rev_end >= 0x100000000) {
    ...
    return hobject_t(object_t(), string(), CEPH_NOSNAP,
                     hobject_t::_reverse_bits(rev_end), m_pool, string());
  }
}
...
void coll_t::decode(ceph::buffer::list::const_iterator &bl) {
  ...
  default: {
    ostringstream oss;
    oss << "coll_t::decode(): don't know how to decode version " << struct_v;
...
static opt_mapping_t opt_mapping = boost::assign::map_list_of(
    ...
    pool_opts_t::opt_desc_t(pool_opts_t::RECOVERY_OP_PRIORITY,
                            pool_opts_t::INT))(
    "scrub_priority",
    pool_opts_t::opt_desc_t(pool_opts_t::SCRUB_PRIORITY, pool_opts_t::INT))(
    "compression_mode",
    pool_opts_t::opt_desc_t(pool_opts_t::COMPRESSION_MODE, pool_opts_t::STR))(
    "compression_algorithm",
    pool_opts_t::opt_desc_t(pool_opts_t::COMPRESSION_ALGORITHM,
                            pool_opts_t::STR))(
    "compression_required_ratio",
    pool_opts_t::opt_desc_t(pool_opts_t::COMPRESSION_REQUIRED_RATIO,
                            pool_opts_t::DOUBLE))(
    "compression_max_blob_size",
    pool_opts_t::opt_desc_t(pool_opts_t::COMPRESSION_MAX_BLOB_SIZE,
                            pool_opts_t::INT))(
    "compression_min_blob_size",
    pool_opts_t::opt_desc_t(pool_opts_t::COMPRESSION_MIN_BLOB_SIZE,
                            pool_opts_t::INT))(
    "csum_type",
    pool_opts_t::opt_desc_t(pool_opts_t::CSUM_TYPE, pool_opts_t::INT))(
    "csum_max_block",
    pool_opts_t::opt_desc_t(pool_opts_t::CSUM_MAX_BLOCK, pool_opts_t::INT))(
    "csum_min_block",
    pool_opts_t::opt_desc_t(pool_opts_t::CSUM_MIN_BLOCK, pool_opts_t::INT))(
    "fingerprint_algorithm",
    pool_opts_t::opt_desc_t(pool_opts_t::FINGERPRINT_ALGORITHM,
                            pool_opts_t::STR))(
    "pg_num_min",
    pool_opts_t::opt_desc_t(pool_opts_t::PG_NUM_MIN, pool_opts_t::INT))(
    "target_size_bytes",
    pool_opts_t::opt_desc_t(pool_opts_t::TARGET_SIZE_BYTES, pool_opts_t::INT))(
    "target_size_ratio", pool_opts_t::opt_desc_t(pool_opts_t::TARGET_SIZE_RATIO,
                                                 pool_opts_t::DOUBLE))(
    "pg_autoscale_bias", pool_opts_t::opt_desc_t(pool_opts_t::PG_AUTOSCALE_BIAS,
                                                 pool_opts_t::DOUBLE))(
    "read_lease_interval",
    pool_opts_t::opt_desc_t(pool_opts_t::READ_LEASE_INTERVAL,
                            pool_opts_t::DOUBLE));
...
void pg_pool_t::dump(Formatter *f) const {
  ...
  f->dump_unsigned("min_read_recency_for_promote",
                   min_read_recency_for_promote);
  f->dump_unsigned("min_write_recency_for_promote",
...
void pg_pool_t::convert_to_pg_shards(const vector<int> &from,
                                     set<pg_shard_t> *to) const {
  for (size_t i = 0; i < from.size(); ++i) {
    if (from[i] != CRUSH_ITEM_NONE) {
      to->insert(pg_shard_t(from[i], is_erasure() ? shard_id_t(i)
                                                  : shard_id_t::NO_SHARD));
    }
  }
}
...
void pg_pool_t::remove_unmanaged_snap(snapid_t s, bool preoctopus_compat) {
  ...
  if (preoctopus_compat) {
    ...
    // try to add in the new seq, just to try to keep the interval_set
    // contiguous
...
void pg_pool_t::encode(ceph::buffer::list &bl, uint64_t features) const {
  ...
  if ((features & CEPH_FEATURE_PGPOOL3) == 0) {
    ...
    encode(pgp_num, bl);
    __u32 lpg_num = 0,
          lpgp_num = 0; // tell old code that there are no localized pgs.
    encode(lpg_num, bl);
    encode(lpgp_num, bl);
...
void pg_pool_t::decode(ceph::buffer::list::const_iterator &bl) {
  ...
  if (snap_seq > 0 &&
      (flags & (FLAG_SELFMANAGED_SNAPS | FLAG_POOL_SNAPS)) == 0) {
...
ostream &operator<<(ostream &out, const pg_pool_t &p) {
  ...
  out << " size " << p.get_size() << " min_size " << p.get_min_size()
      << " crush_rule " << p.get_crush_rule() << " object_hash "
      << p.get_object_hash_name() << " pg_num " << p.get_pg_num() << " pgp_num "
      << p.get_pgp_num();
  ...
  if (p.pg_autoscale_mode != pg_pool_t::pg_autoscale_mode_t::UNKNOWN) {
    out << " autoscale_mode "
        << p.get_pg_autoscale_mode_name(p.pg_autoscale_mode);
  }
  ...
  if (p.hit_set_params.get_type() != HitSet::TYPE_NONE) {
    out << " hit_set " << p.hit_set_params << " " << p.hit_set_period << "s"
        ...
        << p.hit_set_grade_decay_rate << " search_last_n "
...
bool operator==(const object_stat_sum_t &l, const object_stat_sum_t &r) {
  return l.num_bytes == r.num_bytes && l.num_objects == r.num_objects &&
         ...
         l.num_rd == r.num_rd && l.num_rd_kb == r.num_rd_kb &&
         l.num_wr == r.num_wr && l.num_wr_kb == r.num_wr_kb &&
         ...
         l.num_flush == r.num_flush && l.num_flush_kb == r.num_flush_kb &&
         l.num_evict == r.num_evict && l.num_evict_kb == r.num_evict_kb &&
...
void object_stat_collection_t::generate_test_instances(
...
void pg_stat_t::dump(Formatter *f) const {
  ...
  for (auto p = object_location_counts.cbegin();
       p != object_location_counts.cend(); ++p) {
    f->open_object_section("entry");
...
bool operator==(const pg_stat_t &l, const pg_stat_t &r) {
  return l.version == r.version && l.reported_seq == r.reported_seq &&
         l.reported_epoch == r.reported_epoch && l.state == r.state &&
         l.last_fresh == r.last_fresh && l.last_change == r.last_change &&
         l.last_active == r.last_active && l.last_peered == r.last_peered &&
         l.last_clean == r.last_clean && l.last_unstale == r.last_unstale &&
         ...
         l.stats == r.stats && l.stats_invalid == r.stats_invalid &&
         l.log_size == r.log_size && l.ondisk_log_size == r.ondisk_log_size &&
         l.up == r.up && l.acting == r.acting &&
         ...
         l.up_primary == r.up_primary && l.acting_primary == r.acting_primary &&
         l.pin_stats_invalid == r.pin_stats_invalid &&
         l.manifest_stats_invalid == r.manifest_stats_invalid &&
         l.purged_snaps == r.purged_snaps && l.snaptrimq_len == r.snaptrimq_len;
}

// -- store_statfs_t --

bool store_statfs_t::operator==(const store_statfs_t &other) const {
  return total == other.total && available == other.available &&
...
ostream &operator<<(ostream &out, const store_statfs_t &s) {
  out << std::hex << "store_statfs(0x" << s.available << "/0x"
      << s.internally_reserved << "/0x" << s.total << ", data 0x"
      << s.data_stored << "/0x" << s.allocated << ", compress 0x"
      << s.data_compressed << "/0x" << s.data_compressed_allocated << "/0x"
      << s.data_compressed_original << ", omap 0x" << s.omap_allocated
...
void pg_history_t::dump(Formatter *f) const {
  ...
  f->dump_stream("last_deep_scrub_stamp") << last_deep_scrub_stamp;
  f->dump_stream("last_clean_scrub_stamp") << last_clean_scrub_stamp;
  f->dump_float("prior_readable_until_ub",
...
ostream &operator<<(ostream &lhs, const pg_notify_t &notify) {
  lhs << "(query:" << notify.query_epoch << " sent:" << notify.epoch_sent << " "
      << notify.info;
...
void PastIntervals::pg_interval_t::decode(
...
void PastIntervals::pg_interval_t::generate_test_instances(
...
public:
  ...
  set<pg_shard_t> get_all_participants(bool ec_pool) const override {
    return all_participants;
  }
  void add_interval(bool ec_pool,
    ...
    for (unsigned i = 0; i < interval.acting.size(); ++i) {
      if (interval.acting[i] == CRUSH_ITEM_NONE)
        continue;
      acting.insert(pg_shard_t(interval.acting[i],
  ...
  static void generate_test_instances(list<pi_compact_rep *> &o) {
    ...
    o.push_back(new pi_compact_rep(
        true, ivallst{ival{{0, 1, 2}, {0, 1, 2}, 10, 20, true, 0, 0},
                      ival{{1, 2}, {1, 2}, 21, 30, true, 1, 1},
                      ival{{2}, {2}, 31, 35, false, 2, 2},
                      ival{{0, 2}, {0, 2}, 36, 50, true, 0, 0}}));
    o.push_back(new pi_compact_rep(
        false, ivallst{ival{{0, 1, 2}, {0, 1, 2}, 10, 20, true, 0, 0},
                       ival{{1, 2}, {1, 2}, 21, 30, true, 1, 1},
                       ival{{2}, {2}, 31, 35, false, 2, 2},
                       ival{{0, 2}, {0, 2}, 36, 50, true, 0, 0}}));
    o.push_back(new pi_compact_rep(
        true, ivallst{ival{{2, 1, 0}, {2, 1, 0}, 10, 20, true, 1, 1},
                      ival{{0, 2}, {0, 2}, 21, 30, true, 0, 0},
                      ival{{0, 2}, {2, 0}, 31, 35, true, 2, 2},
                      ival{{0, 2}, {0, 2}, 36, 50, true, 0, 0}}));
  }
  void iterate_mayberw_back_to(
      epoch_t les, std::function<void(epoch_t, const set<pg_shard_t> &)> &&f)
      const override {
...
PastIntervals::PastIntervals(const PastIntervals &rhs)
    : past_intervals(rhs.past_intervals ? rhs.past_intervals->clone()
                                        : nullptr) {}
...
ostream &operator<<(ostream &out, const PastIntervals::PriorSet &i) {
  return out << "PriorSet("
             << "ec_pool: " << i.ec_pool << ", probe: " << i.probe
             << ", down: " << i.down << ", blocked_by: " << i.blocked_by
             << ", pg_down: " << i.pg_down << ")";
}
...
bool PastIntervals::is_new_interval(
    int old_acting_primary, int new_acting_primary,
    const vector<int> &old_acting, const vector<int> &new_acting,
    int old_up_primary, int new_up_primary, const vector<int> &old_up,
    const vector<int> &new_up, int old_size, int new_size, int old_min_size,
    int new_min_size, unsigned old_pg_num, unsigned new_pg_num,
    unsigned old_pg_num_pending, unsigned new_pg_num_pending,
    bool old_sort_bitwise, bool new_sort_bitwise, bool old_recovery_deletes,
    bool new_recovery_deletes, pg_t pgid) {
  return old_acting_primary != new_acting_primary || new_acting != old_acting ||
         old_up_primary != new_up_primary || new_up != old_up ||
         old_min_size != new_min_size || old_size != new_size ||
...
bool PastIntervals::is_new_interval(
    ...
    const vector<int> &old_acting, const vector<int> &new_acting,
    int old_up_primary, int new_up_primary, const vector<int> &old_up,
    const vector<int> &new_up, const OSDMap *osdmap, const OSDMap *lastmap,
    pg_t pgid) {
  ...
  return is_new_interval(old_acting_primary, new_acting_primary, old_acting,
                         new_acting, old_up_primary, new_up_primary, old_up,
                         new_up, plast->size, pi->size, plast->min_size,
                         pi->min_size, plast->get_pg_num(), pi->get_pg_num(),
                         plast->get_pg_num_pending(), pi->get_pg_num_pending(),
                         ...
                         osdmap->test_flag(CEPH_OSDMAP_RECOVERY_DELETES), pgid);
}
...
bool PastIntervals::check_new_interval(
    ...
    const vector<int> &old_acting, const vector<int> &new_acting,
    int old_up_primary, int new_up_primary, const vector<int> &old_up,
    const vector<int> &new_up, epoch_t same_interval_since,
    epoch_t last_epoch_clean, const OSDMap *osdmap, const OSDMap *lastmap,
    pg_t pgid, const IsPGRecoverablePredicate &could_have_gone_active,
  ...
  if (is_new_interval(old_acting_primary, new_acting_primary, old_acting,
                      new_acting, old_up_primary, new_up_primary, old_up,
                      new_up, osdmap, lastmap, pgid)) {
    pg_interval_t i;
    ...
    const pg_pool_t &old_pg_pool =
        lastmap->get_pools().find(pgid.pool())->second;
    ...
    if (num_acting && i.primary != -1 && num_acting >= old_pg_pool.min_size &&
      ...
      if (out)
        *out << __func__ << " " << i << " up_thru "
             << lastmap->get_up_thru(i.primary) << " up_from "
             << lastmap->get_up_from(i.primary) << " last_epoch_clean "
             << last_epoch_clean;
      if (lastmap->get_up_thru(i.primary) >= i.first &&
        ...
        if (out)
          *out << " " << i << " : primary up "
               << lastmap->get_up_from(i.primary) << "-"
               << lastmap->get_up_thru(i.primary) << " includes interval"
               << std::endl;
        ...
        i.maybe_went_rw = true;
        if (out)
          *out << " " << i << " : includes last_epoch_clean "
               << last_epoch_clean << " and presumed to have been rw"
               << std::endl;
      } else {
      ...
      i.maybe_went_rw = false;
      if (out)
        *out << __func__ << " " << i << " : acting set is too small"
             << std::endl;
    }
...
bool PastIntervals::PriorSet::affected_by_map(
    const OSDMap &osdmap, const DoutPrefixProvider *dpp) const {
  for (auto p = probe.begin(); p != probe.end(); ++p) {
    ...
    if (r != blocked_by.end()) {
      if (!osdmap.exists(o)) {
        ldpp_dout(dpp, 10) << "affected_by_map osd." << o << " no longer exists"
                           << dendl;
        return true;
      }
      if (osdmap.get_info(o).lost_at != r->second) {
        ldpp_dout(dpp, 10) << "affected_by_map osd." << o
...
ostream &operator<<(ostream &out, const PastIntervals::pg_interval_t &i) {
  out << "interval(" << i.first << "-" << i.last << " up " << i.up << "("
      << i.up_primary << ")"
...
void pg_query_t::generate_test_instances(list<pg_query_t *> &o) {
  ...
  list<pg_history_t *> h;
  pg_history_t::generate_test_instances(h);
  o.push_back(new pg_query_t(pg_query_t::INFO, shard_id_t(1), shard_id_t(2),
                             *h.back(), 4));
  o.push_back(new pg_query_t(pg_query_t::MISSING, shard_id_t(2), shard_id_t(3),
                             *h.back(), 4));
  o.push_back(new pg_query_t(pg_query_t::LOG, shard_id_t(0), shard_id_t(0),
                             eversion_t(4, 5), *h.back(), 4));
  o.push_back(new pg_query_t(pg_query_t::FULLLOG, shard_id_t::NO_SHARD,
                             shard_id_t::NO_SHARD, *h.back(), 5));
}
...
struct DumpVisitor : public ObjectModDesc::Visitor {
  ...
  void
  setattrs(map<string, std::optional<ceph::buffer::list>> &attrs) override {
    f->open_object_section("op");
  ...
  void
  rollback_extents(version_t gen,
...
void ObjectCleanRegions::generate_test_instances(
...
void pg_log_entry_t::decode_with_checksum(
...
void pg_log_entry_t::generate_test_instances(list<pg_log_entry_t *> &o) {
  ...
  o.push_back(new pg_log_entry_t(
      MODIFY, oid, eversion_t(1, 2), eversion_t(3, 4), 1,
      osd_reqid_t(entity_name_t::CLIENT(777), 8, 999), utime_t(8, 9), 0));
  o.push_back(new pg_log_entry_t(
      ERROR, oid, eversion_t(1, 2), eversion_t(3, 4), 1,
      osd_reqid_t(entity_name_t::CLIENT(777), 8, 999), utime_t(8, 9), -ENOENT));
}

ostream &operator<<(ostream &out, const pg_log_entry_t &e) {
  out << e.version << " (" << e.prior_version << ") " << std::left
      << std::setw(8) << e.get_op_name() << ' ' << e.soid << " by " << e.reqid
      << " " << e.mtime << " " << e.return_code;
...
void pg_log_dup_t::generate_test_instances(list<pg_log_dup_t *> &o) {
  ...
  o.push_back(new pg_log_dup_t(
      eversion_t(1, 2), 1, osd_reqid_t(entity_name_t::CLIENT(777), 8, 999), 0));
  o.push_back(new pg_log_dup_t(eversion_t(1, 2), 2,
                               osd_reqid_t(entity_name_t::CLIENT(777), 8, 999),
                               -ENOENT));
}

std::ostream &operator<<(std::ostream &out, const pg_log_dup_t &e) {
  out << "log_dup(reqid=" << e.reqid << " v=" << e.version
      << " uv=" << e.user_version << " rc=" << e.return_code;
  if (!e.op_returns.empty()) {
    out << " " << e.op_returns;
  }
  return out << ")";
...
// reject: Entries rejected from "in" are in the reject.log.  Other fields not
// set.
...
static void _handle_dups(CephContext *cct, pg_log_t &target,
                         const pg_log_t &other, unsigned maxdups) {
  auto earliest_dup_version =
      target.head.version < maxdups ? 0u : target.head.version - maxdups + 1;
  lgeneric_subdout(cct, osd, 20)
      << "copy_up_to/copy_after earliest_dup_version " << earliest_dup_version
      << dendl;

  for (auto d = other.dups.cbegin(); d != other.dups.cend(); ++d) {
    if (d->version.version >= earliest_dup_version) {
      lgeneric_subdout(cct, osd, 20)
          << "copy_up_to/copy_after copy dup version " << d->version << dendl;
  ...
  for (auto i = other.log.cbegin(); i != other.log.cend(); ++i) {
    ...
    if (i->version.version >= earliest_dup_version) {
      lgeneric_subdout(cct, osd, 20)
          << "copy_up_to/copy_after copy dup from log version " << i->version
          << dendl;
...
void pg_log_t::copy_after(CephContext *cct, const pg_log_t &other,
                          eversion_t v) {
  ...
  for (auto i = other.log.crbegin(); i != other.log.crend(); ++i) {
    ...
    lgeneric_subdout(cct, osd, 20)
        << __func__ << " copy log version " << i->version << dendl;
    log.push_front(*i);
  }
...
void object_copy_cursor_t::generate_test_instances(
...
void object_copy_data_t::encode(ceph::buffer::list &bl,
                                uint64_t features) const {
  ENCODE_START(8, 5, bl);
  encode(size, bl);
  encode(mtime, bl);
...
void object_copy_data_t::generate_test_instances(
...
void pg_hit_set_history_t::generate_test_instances(
...
void SnapSet::from_snap_set(const librados::snap_set_t &ss, bool legacy) {
  ...
  for (auto p = _snaps.rbegin(); p != _snaps.rend(); ++p)
    snaps.push_back(*p);
}
...
void object_info_t::decode(ceph::buffer::list::const_iterator &bl) {
  ...
  if (struct_v >= 11) {
    ...
    for (auto i = old_watchers.begin(); i != old_watchers.end(); ++i) {
      watchers.insert(
          make_pair(make_pair(i->second.cookie, i->first), i->second));
    }
  }
  if (struct_v >= 13) {
    __u32 _flags;
...
void ObjectRecoveryInfo::encode(ceph::buffer::list &bl,
                                uint64_t features) const {
  ENCODE_START(3, 1, bl);
  encode(soid, bl);
  encode(version, bl);
...
ostream &ObjectRecoveryInfo::print(ostream &out) const {
  return out << "ObjectRecoveryInfo(" << soid << "@" << version
             << ", size: " << size << ", copy_subset: " << copy_subset
             << ", clone_subset: " << clone_subset << ", snapset: " << ss
...
ostream &PushOp::print(ostream &out) const {
  return out << "PushOp(" << soid << ", version: " << version
...
ostream &operator<<(ostream &out, const OSDOp &op) {
  ...
  if (ceph_osd_op_type_data(op.op.op)) {
    ...
    case CEPH_OSD_OP_WATCH:
      out << " " << ceph_osd_watch_op_name(op.op.watch.op) << " cookie "
          << op.op.watch.cookie;
      if (op.op.watch.gen)
    ...
    if (op.op.op == CEPH_OSD_OP_CMPXATTR)
      out << " op " << (int)op.op.xattr.cmp_op << " mode "
          << (int)op.op.xattr.cmp_mode;
...
void OSDOp::split_osd_op_vector_in_data(vector<OSDOp> &ops,
                                        ceph::buffer::list &in) {
...
void OSDOp::merge_osd_op_vector_in_data(vector<OSDOp> &ops,
...
void OSDOp::split_osd_op_vector_out_data(vector<OSDOp> &ops,
                                         ceph::buffer::list &in) {
  auto datap = in.begin();
  for (unsigned i = 0; i < ops.size(); i++) {
    if (ops[i].op.payload_len) {
...
void OSDOp::merge_osd_op_vector_out_data(vector<OSDOp> &ops,
                                         ceph::buffer::list &out) {
  for (unsigned i = 0; i < ops.size(); i++) {
...
void OSDOp::clear_data(vector<OSDOp> &ops) {
  for (unsigned i = 0; i < ops.size(); i++) {
    OSDOp &op = ops[i];
    op.outdata.clear();
    if (ceph_osd_op_type_attr(op.op.op) && op.op.xattr.name_len &&
...
int prepare_info_keymap(CephContext *cct, map<string, bufferlist> *km,
                        epoch_t epoch, pg_info_t &info,
                        pg_info_t &last_written_info,
                        PastIntervals &past_intervals, bool dirty_big_info,
                        bool dirty_epoch, bool try_fast_info,
                        PerfCounters *logger, DoutPrefixProvider *dpp) {
  if (dirty_epoch) {
...
void init_pg_ondisk(ceph::os::Transaction &t, spg_t pgid,
