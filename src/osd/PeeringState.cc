...
BufferedRecoveryMessages::BufferedRecoveryMessages(ceph_release_t r,
                                                   PeeringCtx &ctx)
...
void BufferedRecoveryMessages::send_query(int to, spg_t to_spgid,
                                          const pg_query_t &q) {
  if (require_osd_release >= ceph_release_t::octopus) {
    send_osd_message(to, make_message<MOSDPGQuery2>(to_spgid, q));
  } else {
    auto m = make_message<MOSDPGQuery>(q.epoch_sent,
...
void BufferedRecoveryMessages::send_info(
    int to, spg_t to_spgid, epoch_t min_epoch, epoch_t cur_epoch,
    const pg_info_t &info, std::optional<pg_lease_t> lease,
    std::optional<pg_lease_ack_t> lease_ack) {
  if (require_osd_release >= ceph_release_t::octopus) {
    send_osd_message(to,
                     make_message<MOSDPGInfo2>(to_spgid, info, cur_epoch,
                                               min_epoch, lease, lease_ack));
  } else {
    send_osd_message(
        to, make_message<MOSDPGInfo>(
                cur_epoch,
                vector{pg_notify_t{to_spgid.shard, info.pgid.shard, min_epoch,
                                   cur_epoch, info, PastIntervals{}}}));
  }
}
...
PeeringState::PeeringState(CephContext *cct, pg_shard_t pg_whoami, spg_t spgid,
                           const PGPool &_pool, OSDMapRef curmap,
                           DoutPrefixProvider *dpp, PeeringListener *pl)
    : state_history(*pl), cct(cct), spgid(spgid), dpp(dpp), pl(pl), orig_ctx(0),
      osdmap_ref(curmap), pool(_pool), pg_whoami(pg_whoami), info(spgid),
      pg_log(cct), missing_loc(spgid, this, dpp, cct),
...
void PeeringState::begin_block_outgoing() {
  ...
  messages_pending_flush =
      BufferedRecoveryMessages(orig_ctx->require_osd_release);
...
void PeeringState::update_history(const pg_history_t &new_history) {
  ...
  if (info.history.merge(new_history)) {
    ...
    if (prior_readable_until_ub != ceph::signedspan::zero()) {
      dout(20) << __func__ << " prior_readable_until_ub "
...
void PeeringState::purge_strays() {
  ...
  bool removed = false;
  for (set<pg_shard_t>::iterator p = stray_set.begin(); p != stray_set.end();
       ++p) {
    ...
    if (get_osdmap()->is_up(p->osd)) {
      ...
      MOSDPGRemove *m = new MOSDPGRemove(get_osdmap_epoch(), to_remove);
...
bool PeeringState::proc_replica_info(pg_shard_t from, const pg_info_t &oinfo,
                                     epoch_t send_epoch) {
  map<pg_shard_t, pg_info_t>::iterator p = peer_info.find(from);
  if (p != peer_info.end() && p->second.last_update == oinfo.last_update) {
    psdout(10) << " got dup osd." << from << " info " << oinfo
...
void PeeringState::remove_down_peer_info(const OSDMapRef &osdmap) {
  ...
  while (p != peer_info.end()) {
    if (!osdmap->is_up(p->first.osd)) {
      psdout(10) << " dropping down osd." << p->first << " info " << p->second
                 << dendl;
...
void PeeringState::write_if_dirty(ObjectStore::Transaction &t) {
  pl->prepare_write(info, last_written_info, past_intervals, pg_log, dirty_info,
                    dirty_big_info, last_persisted_osdmap < get_osdmap_epoch(),
                    t);
...
void PeeringState::advance_map(OSDMapRef osdmap, OSDMapRef lastmap,
                               vector<int> &newup, int up_primary,
                               vector<int> &newacting, int acting_primary,
                               PeeringCtx &rctx) {
  ceph_assert(lastmap == osdmap_ref);
  psdout(10) << "handle_advance_map " << newup << "/" << newacting << " -- "
             << up_primary << "/" << acting_primary << dendl;
...
void PeeringState::activate_map(PeeringCtx &rctx) {
  ...
  if (osdmap_ref->get_epoch() - last_persisted_osdmap >
    ...
    psdout(20) << __func__ << ": Dirtying info: last_persisted is "
               << last_persisted_osdmap << " while current is "
...
bool PeeringState::should_restart_peering(int newupprimary,
                                          int newactingprimary,
                                          const vector<int> &newup,
                                          const vector<int> &newacting,
                                          OSDMapRef lastmap, OSDMapRef osdmap) {
  if (PastIntervals::is_new_interval(primary.osd, newactingprimary, acting,
                                     newacting, up_primary.osd, newupprimary,
                                     up, newup, osdmap.get(), lastmap.get(),
                                     info.pgid.pgid)) {
    psdout(20) << "new interval newup " << newup << " newacting " << newacting
               << dendl;
    return true;
  }
...
void PeeringState::start_peering_interval(const OSDMapRef lastmap,
                                          const vector<int> &newup,
                                          int new_up_primary,
                                          const vector<int> &newacting,
  ...
  if (!lastmap) {
    ...
    bool new_interval = PastIntervals::check_new_interval(
        old_acting_primary.osd, new_acting_primary, oldacting, newacting,
        old_up_primary.osd, new_up_primary, oldup, newup,
        info.history.same_interval_since, info.history.last_epoch_clean,
        osdmap.get(), lastmap.get(), info.pgid.pgid,
        missing_loc.get_recoverable_predicate(), &past_intervals, &debug);
    psdout(10) << __func__ << ": check_new_interval output: " << debug.str()
               << dendl;
    if (new_interval) {
  ...
  psdout(1) << __func__ << " up " << oldup << " -> " << up << ", acting "
            << oldacting << " -> " << acting << ", acting_primary "
            << old_acting_primary << " -> " << new_acting_primary
            << ", up_primary " << old_up_primary << " -> " << new_up_primary
            << ", role " << oldrole << " -> " << role << ", features acting "
            << acting_features << " upacting " << upacting_features << dendl;
  ...
  if (role != oldrole || was_old_primary != is_primary()) {
    ...
    if (get_primary() != old_acting_primary) {
      psdout(10) << oldacting << " -> " << acting << ", acting primary "
      ...
      if (is_primary()) {
        ...
        psdout(10) << oldacting << " -> " << acting << ", replicas changed"
                   << dendl;
      }
    }
  }
...
void PeeringState::on_new_interval() {
  ...
  ceph_assert(pg_log.get_missing().may_include_deletes ==
              !perform_deletes_during_peering());
  ...
  prior_readable_until_ub =
      std::max(prior_readable_until_ub, readable_until_ub);
  ...
  readable_until = readable_until_ub = readable_until_ub_sent =
...
void PeeringState::init_primary_up_acting(const vector<int> &newup,
                                          const vector<int> &newacting,
                                          int new_up_primary,
                                          int new_acting_primary) {
  actingset.clear();
  ...
  for (uint8_t i = 0; i < acting.size(); ++i) {
    if (acting[i] != CRUSH_ITEM_NONE)
      actingset.insert(pg_shard_t(acting[i], pool.info.is_erasure()
                                                 ? shard_id_t(i)
                                                 : shard_id_t::NO_SHARD));
  }
  upset.clear();
  ...
  for (uint8_t i = 0; i < up.size(); ++i) {
    if (up[i] != CRUSH_ITEM_NONE)
      upset.insert(pg_shard_t(up[i], pool.info.is_erasure()
                                         ? shard_id_t(i)
                                         : shard_id_t::NO_SHARD));
  }
...
static pair<epoch_t, epoch_t>
get_required_past_interval_bounds(const pg_info_t &info, epoch_t oldest_map) {
  epoch_t start =
      std::max(info.history.last_epoch_clean ? info.history.last_epoch_clean
                                             : info.history.epoch_pool_created,
               oldest_map);
  epoch_t end = std::max(info.history.same_interval_since,
                         info.history.epoch_pool_created);
  ...
  if (rpib.first >= rpib.second) {
    ...
    if (!past_intervals.empty() && rpib.first > oldest_epoch) {
      pl->get_clog_error() << info.pgid << " required past_interval bounds are"
                           << " empty [" << rpib
                           << ") but past_intervals is not: " << past_intervals;
      derr << info.pgid << " required past_interval bounds are"
           << " empty [" << rpib
           << ") but past_intervals is not: " << past_intervals << dendl;
    }
  } else {
    ...
    if (apib.second != rpib.second) {
      ...
      derr << info.pgid << " past_interal bound [" << apib
           << ") end does not match required [" << rpib << ") end" << dendl;
      ceph_abort_msg("past_interval end mismatch");
    }
  }
}

int PeeringState::clamp_recovery_priority(int priority,
                                          int pool_recovery_priority, int max) {
  static_assert(OSD_RECOVERY_PRIORITY_MIN < OSD_RECOVERY_PRIORITY_MAX,
                "Invalid priority range");
  static_assert(OSD_RECOVERY_PRIORITY_MIN >= 0,
                "Priority range must match unsigned type");
  ...
  ceph_assert(pool_recovery_priority >= 0 &&
              pool_recovery_priority <=
                  (OSD_POOL_PRIORITY_MAX - OSD_POOL_PRIORITY_MIN));
...
unsigned PeeringState::get_recovery_priority() {
  ...
  if (state & PG_STATE_FORCED_RECOVERY) {
    ...
    // XXX: This priority boost isn't so much about inactive, but about
    // data-at-risk
    if (is_degraded() &&
        info.stats.avail_no_missing.size() < pool.info.min_size) {
      base = OSD_RECOVERY_INACTIVE_PRIORITY_BASE;
      // inactive: no. of replicas < min_size, highest priority since it blocks
      // IO
...
bool PeeringState::set_force_recovery(bool b) {
  ...
  if (b) {
    if (!(state & PG_STATE_FORCED_RECOVERY) &&
        (state &
         (PG_STATE_DEGRADED | PG_STATE_RECOVERY_WAIT | PG_STATE_RECOVERING))) {
...
bool PeeringState::set_force_backfill(bool b) {
  ...
  if (b) {
    if (!(state & PG_STATE_FORCED_BACKFILL) &&
        (state &
         (PG_STATE_DEGRADED | PG_STATE_BACKFILL_WAIT | PG_STATE_BACKFILLING))) {
...
void PeeringState::send_lease() {
  ...
  for (auto peer : actingset) {
    ...
    pl->send_cluster_message(
        peer.osd,
        new MOSDPGLease(epoch, spg_t(spgid.pgid, peer.shard), get_lease()),
        epoch);
  }
}
...
void PeeringState::recalc_readable_until() {
  ...
  for (unsigned i = 0; i < acting.size(); ++i) {
    ...
    dout(20) << __func__ << " peer osd." << acting[i] << " ruub "
             << acting_readable_until_ub[i] << dendl;
  ...
  dout(20) << __func__ << " readable_until[_ub] " << readable_until << " (sent "
           << readable_until_ub_sent << ")" << dendl;
}
  ...
  while (p != prior_readable_down_osds.end()) {
    if (map->is_dead(*p)) {
      dout(10) << __func__ << " prior_readable_down_osds osd." << *p
               << " is dead as of epoch " << map->get_epoch() << dendl;
...
bool PeeringState::adjust_need_up_thru(const OSDMapRef osdmap) {
  ...
  if (need_up_thru && up_thru >= info.history.same_interval_since) {
    psdout(10) << "adjust_need_up_thru now " << up_thru
...
PastIntervals::PriorSet PeeringState::build_prior() {
  ...
  PastIntervals::PriorSet prior = past_intervals.get_prior_set(
      pool.info.is_erasure(), info.history.last_epoch_started,
...
bool PeeringState::needs_backfill() const {
  ...
  for (; a != end; ++a) {
    ...
    if (!pi->second.last_backfill.is_max()) {
      psdout(10) << __func__ << " osd." << peer << " has last_backfill "
                 << pi->second.last_backfill << dendl;
      return true;
    }
  }
...
bool PeeringState::all_unfound_are_queried_or_lost(
  ...
  psdout(10) << "all_unfound_are_queried_or_lost all of might_have_unfound "
             << might_have_unfound << " have been queried or are marked lost"
             << dendl;
  return true;
}
...
void PeeringState::reject_reservation() {
  ...
  pl->send_cluster_message(
      primary.osd,
      new MBackfillReserve(MBackfillReserve::REJECT_TOOFULL,
 ...
 */
map<pg_shard_t, pg_info_t>::const_iterator
PeeringState::find_best_info(const map<pg_shard_t, pg_info_t> &infos,
...
void PeeringState::calc_ec_acting(
    map<pg_shard_t, pg_info_t>::const_iterator auth_log_shard, unsigned size,
    const vector<int> &acting, const vector<int> &up,
    const map<pg_shard_t, pg_info_t> &all_info, bool restrict_to_up_acting,
    vector<int> *_want, set<pg_shard_t> *backfill,
    set<pg_shard_t> *acting_backfill, ostream &ss) {
  ...
  for (uint8_t i = 0; i < want.size(); ++i) {
    ...
    if (up.size() > (unsigned)i && up[i] != CRUSH_ITEM_NONE &&
        !all_info.find(pg_shard_t(up[i], shard_id_t(i)))
             ->second.is_incomplete() &&
    ...
    if (acting.size() > (unsigned)i && acting[i] != CRUSH_ITEM_NONE &&
        !all_info.find(pg_shard_t(acting[i], shard_id_t(i)))
             ->second.is_incomplete() &&
        all_info.find(pg_shard_t(acting[i], shard_id_t(i)))
                ->second.last_update >= auth_log_shard->second.log_tail) {
      ss << " selecting acting[i]: " << pg_shard_t(acting[i], shard_id_t(i))
         << std::endl;
      want[i] = acting[i];
    } else if (!restrict_to_up_acting) {
      for (set<pg_shard_t>::iterator j =
               all_info_by_shard[shard_id_t(i)].begin();
           j != all_info_by_shard[shard_id_t(i)].end(); ++j) {
...
void PeeringState::calc_replicated_acting(
    ...
    uint64_t force_auth_primary_missing_objects, unsigned size,
    const vector<int> &acting, const vector<int> &up, pg_shard_t up_primary,
    const map<pg_shard_t, pg_info_t> &all_info, bool restrict_to_up_acting,
    vector<int> *want, set<pg_shard_t> *backfill,
    set<pg_shard_t> *acting_backfill, const OSDMapRef osdmap, ostream &ss) {
  ...
  ss << __func__ << " primary is osd." << primary->first << " with "
     << primary->second << std::endl;
  ...
  for (auto i : acting) {
    ...
    if (cur_info.is_incomplete() ||
        cur_info.last_update < oldest_auth_log_entry) {
      ss << " shard " << acting_cand << " (acting) REJECTED " << cur_info
         << std::endl;
    } else {
  ...
  for (auto &p : candidate_by_last_update) {
    ...
    ss << " shard " << s << " (acting) accepted " << all_info.find(s)->second
       << std::endl;
    if (want->size() >= size) {
      return;
    }
  ...
  for (auto &i : all_info) {
    ...
    if (up_it != up.end())
      continue;
    vector<int>::const_iterator acting_it =
        find(acting.begin(), acting.end(), i.first.osd);
  ...
  for (auto &p : candidate_by_last_update) {
    ...
    pg_shard_t s = pg_shard_t(p.second, shard_id_t::NO_SHARD);
    acting_backfill->insert(s);
    ss << " shard " << s << " (stray) accepted " << all_info.find(s)->second
       << std::endl;
    if (want->size() >= size) {
      return;
    }
...
bool PeeringState::recoverable(const vector<int> &want) const {
  ...
  for (int i = 0; i < (int)want.size(); ++i) {
    if (want[i] != CRUSH_ITEM_NONE) {
      ++num_want_acting;
      have.insert(pg_shard_t(want[i], pool.info.is_erasure()
                                          ? shard_id_t(i)
                                          : shard_id_t::NO_SHARD));
    }
  }
  ...
  if (num_want_acting < pool.info.min_size) {
    ...
    if (pool.info.is_erasure() && !recovery_ec_pool_below_min_size) {
      psdout(10)
          << __func__
          << " failed, ec recovery below min size not supported by pre-octopus"
          << dendl;
      return false;
    } else if (!cct->_conf.get_val<bool>("osd_allow_recovery_below_min_size")) {
      psdout(10) << __func__ << " failed, recovery below min size not enabled"
                 << dendl;
      return false;
    }
  }
...
void PeeringState::choose_async_recovery_ec(
    const map<pg_shard_t, pg_info_t> &all_info, const pg_info_t &auth_info,
    ...
    if (HAVE_FEATURE(osdmap->get_up_osd_features(), SERVER_NAUTILUS)) {
      ...
      if (auth_version > candidate_version &&
          (auth_version - candidate_version) >
              cct->_conf.get_val<uint64_t>("osd_async_recovery_min_cost")) {
        candidates_by_cost.insert(
            make_pair(auth_version - candidate_version, shard_i));
      }
    }
  }
  ...
  for (auto rit = candidates_by_cost.rbegin(); rit != candidates_by_cost.rend();
       ++rit) {
...
bool PeeringState::choose_acting(pg_shard_t &auth_log_shard_id,
  ...
  if (cct->_conf->subsys.should_gather<dout_subsys, 10>()) {
    for (map<pg_shard_t, pg_info_t>::iterator p = all_info.begin();
         p != all_info.end(); ++p) {
      psdout(10) << __func__ << " all_info osd." << p->first << " " << p->second
                 << dendl;
    }
  }
  ...
  if (pool.info.is_replicated())
    calc_replicated_acting(
        ...
        get_osdmap()->get_pg_size(info.pgid.pgid), acting, up, up_primary,
        all_info, restrict_to_up_acting, &want, &want_backfill,
        &want_acting_backfill, get_osdmap(), ss);
  else
    calc_ec_acting(auth_log_shard, get_osdmap()->get_pg_size(info.pgid.pgid),
                   acting, up, all_info, restrict_to_up_acting, &want,
                   &want_backfill, &want_acting_backfill, ss);
  ...
  if (HAVE_FEATURE(get_osdmap()->get_up_osd_features(), SERVER_MIMIC)) {
    if (pool.info.is_erasure()) {
      choose_async_recovery_ec(all_info, auth_log_shard->second, &want,
                               &want_async_recovery, get_osdmap());
    } else {
      choose_async_recovery_replicated(all_info, auth_log_shard->second, &want,
                                       &want_async_recovery, get_osdmap());
    }
  }
  ...
  psdout(10) << "acting_recovery_backfill is " << acting_recovery_backfill
             << dendl;
  ...
  // Adding !needs_recovery() to let the async_recovery_targets reset after
  // recovery is complete
  ceph_assert(async_recovery_targets.empty() ||
  ...
  for (set<pg_shard_t>::iterator i = want_backfill.begin();
       i != want_backfill.end(); ++i) {
    ceph_assert(stray_set.find(*i) == stray_set.end());
  }
  psdout(10) << "choose_acting want=" << want
             << " backfill_targets=" << want_backfill
...
void PeeringState::log_weirdness() {
  if (pg_log.get_tail() != info.log_tail)
    pl->get_clog_error() << info.pgid << " info mismatch, log.tail "
                         << pg_log.get_tail() << " != info.log_tail "
                         << info.log_tail;
  if (pg_log.get_head() != info.last_update)
    pl->get_clog_error() << info.pgid << " info mismatch, log.head "
  ...
  if (pg_log.get_log().caller_ops.size() > pg_log.get_log().log.size()) {
    pl->get_clog_error() << info.pgid << " caller_ops.size "
                         << pg_log.get_log().caller_ops.size() << " > log size "
 ...
 */
bool PeeringState::search_for_missing(const pg_info_t &oinfo,
                                      const pg_missing_t &omissing,
                                      pg_shard_t from, PeeringCtxWrapper &ctx) {
  uint64_t num_unfound_before = missing_loc.num_unfound();
  bool found_missing =
      missing_loc.add_source_info(from, oinfo, omissing, ctx.handle);
  ...
  if (found_missing && oinfo.last_update != eversion_t()) {
    ...
    ctx.send_info(from.osd, spg_t(info.pgid.pgid, from.shard),
                  get_osdmap_epoch(), // fixme: use lower epoch?
                  get_osdmap_epoch(), tinfo);
  }
...
bool PeeringState::discover_all_missing(BufferedRecoveryMessages &rctx) {
  ...
  psdout(10) << __func__ << " " << missing.num_missing() << " missing, "
             << unfound << " unfound" << dendl;
  ...
  for (; m != mend; ++m) {
    ...
    if (peer_log_requested.find(peer) != peer_log_requested.end()) {
      psdout(20) << __func__ << ": osd." << peer << ": in peer_log_requested"
                 << dendl;
      continue;
    }
    ...
    rctx.send_query(peer.osd, spg_t(info.pgid.pgid, peer.shard),
                    pg_query_t(pg_query_t::FULLLOG, peer.shard, pg_whoami.shard,
...
void PeeringState::build_might_have_unfound() {
  ...
  might_have_unfound =
      past_intervals.get_might_have_unfound(pg_whoami, pool.info.is_erasure());
...
void PeeringState::activate(ObjectStore::Transaction &t,
                            epoch_t activation_epoch, PeeringCtxWrapper &ctx) {
  ...
  if (is_primary()) {
    ...
    for (set<pg_shard_t>::iterator i = acting_recovery_backfill.begin();
         i != acting_recovery_backfill.end(); ++i) {
      if (*i == pg_whoami)
        continue;
      pg_shard_t peer = *i;
      ...
      if (pi.last_update == info.last_update) {
        ...
        if (!pi.last_backfill.is_max())
          pl->get_clog_info()
              << info.pgid << " continuing backfill to osd." << peer
              << " from (" << pi.log_tail << "," << pi.last_update << "] "
              << pi.last_backfill << " to " << info.last_update;
        if (!pi.is_empty()) {
          psdout(10) << "activate peer osd." << peer
                     << " is up to date, queueing in pending_activators"
                     << dendl;
          ctx.send_info(peer.osd, spg_t(info.pgid.pgid, peer.shard),
                        get_osdmap_epoch(), // fixme: use lower epoch?
                        get_osdmap_epoch(), info, get_lease());
        } else {
          psdout(10) << "activate peer osd." << peer
                     << " is up to date, but sending pg_log anyway" << dendl;
          m = new MOSDPGLog(i->shard, pg_whoami.shard, get_osdmap_epoch(), info,
                            last_peering_reset);
        }
        ...
        // backfill
        pl->get_clog_debug()
            << info.pgid << " starting backfill to osd." << peer << " from ("
            << pi.log_tail << "," << pi.last_update << "] " << pi.last_backfill
            << " to " << info.last_update;
      ...
      if (pm.num_missing() == 0) {
        ...
        psdout(10) << "activate peer osd." << peer << " " << pi << " uptodate"
                   << dendl;
      } else {
        psdout(10) << "activate peer osd." << peer << " " << pi << " missing "
                   << pm << dendl;
      }
    }
    ...
    // If necessary, create might_have_unfound to help us find our unfound
    // objects. NOTE: It's important that we build might_have_unfound before
    // trimming the past intervals.
    might_have_unfound.clear();
    if (needs_recovery()) {
      // If only one shard has missing, we do a trick to add all others as
      // recovery source, this is considered safe since the PGLogs have been
      // merged locally, and covers vast majority of the use cases, like one
      // OSD/host is down for a while for hardware repairing
...
void PeeringState::share_pg_info() {
  ...
  for (auto pg_shard : acting_recovery_backfill) {
    ...
    if (last_require_osd_release >= ceph_release_t::octopus) {
      ...
      m = new MOSDPGInfo{
          get_osdmap_epoch(),
          {pg_notify_t{pg_shard.shard, pg_whoami.shard, get_osdmap_epoch(),
                       get_osdmap_epoch(), info, past_intervals}}};
    }
    pl->send_cluster_message(pg_shard.osd, m, get_osdmap_epoch());
  }
}

void PeeringState::merge_log(ObjectStore::Transaction &t, pg_info_t &oinfo,
                             pg_log_t &olog, pg_shard_t from) {
  PGLog::LogEntryHandlerRef rollbacker{pl->get_log_handler(t)};
  pg_log.merge_log(oinfo, olog, from, info, rollbacker.get(), dirty_info,
                   dirty_big_info);
}

void PeeringState::rewind_divergent_log(ObjectStore::Transaction &t,
                                        eversion_t newhead) {
  PGLog::LogEntryHandlerRef rollbacker{pl->get_log_handler(t)};
  pg_log.rewind_divergent_log(newhead, info, rollbacker.get(), dirty_info,
                              dirty_big_info);
}

void PeeringState::proc_primary_info(ObjectStore::Transaction &t,
  ...
  if (!(info.purged_snaps == oinfo.purged_snaps)) {
    psdout(10) << __func__ << " updating purged_snaps to " << oinfo.purged_snaps
               << dendl;
...
void PeeringState::proc_master_log(ObjectStore::Transaction &t,
                                   pg_info_t &oinfo, pg_log_t &olog,
                                   pg_missing_t &omissing, pg_shard_t from) {
  psdout(10) << "proc_master_log for osd." << from << ": " << olog << " "
             << omissing << dendl;
  ...
  peer_info[from] = oinfo;
  psdout(10) << " peer osd." << from << " now " << oinfo << " " << omissing
             << dendl;
  might_have_unfound.insert(from);
...
void PeeringState::proc_replica_log(pg_info_t &oinfo, const pg_log_t &olog,
                                    pg_missing_t &omissing, pg_shard_t from) {
  psdout(10) << "proc_replica_log for osd." << from << ": " << oinfo << " "
             << olog << " " << omissing << dendl;
  ...
  for (map<hobject_t, pg_missing_item>::const_iterator i =
       ...
       i != omissing.get_items().end(); ++i) {
    psdout(20) << " after missing " << i->first << " need " << i->second.need
               << " have " << i->second.have << dendl;
  }
  peer_missing[from].claim(omissing);
}

void PeeringState::fulfill_info(pg_shard_t from, const pg_query_t &query,
...
void PeeringState::fulfill_log(pg_shard_t from, const pg_query_t &query,
                               epoch_t query_epoch) {
  psdout(10) << "log request from " << from << dendl;
  ceph_assert(from == primary);
  ceph_assert(query.type != pg_query_t::INFO);

  MOSDPGLog *mlog = new MOSDPGLog(from.shard, pg_whoami.shard,
                                  get_osdmap_epoch(), info, query_epoch);
  ...
  if (query.type == pg_query_t::LOG) {
    ...
    if (query.since != eversion_t() && query.since < pg_log.get_tail()) {
      pl->get_clog_error() << info.pgid << " got broken pg_query_t::LOG since "
                           << query.since << " when my log.tail is "
                           << pg_log.get_tail() << ", sending full log instead";
...
void PeeringState::fulfill_query(const MQuery &query, PeeringCtxWrapper &rctx) {
  if (query.query.type == pg_query_t::INFO) {
    ...
    rctx.send_notify(notify_info.first.osd,
                     pg_notify_t(notify_info.first.shard, pg_whoami.shard,
                                 query.query_epoch, get_osdmap_epoch(),
                                 notify_info.second, past_intervals));
  } else {
    update_history(query.query.history);
    ...
    if (is_clean()) {
      ...
      if (pool.info.is_pending_merge(info.pgid.pgid, &target)) {
        if (target) {
          ...
          pl->set_ready_to_merge_target(info.last_update,
...
void PeeringState::split_into(pg_t child_pgid, PeeringState *child,
                              unsigned split_bits) {
  ...
  get_osdmap()->pg_to_up_acting_osds(child->info.pgid.pgid, &newup, &up_primary,
                                     &newacting, &primary);
...
void PeeringState::merge_from(map<spg_t, PeeringState *> &sources,
                              PeeringCtx &rctx, unsigned split_bits,
                              const pg_merge_meta_t &last_pg_merge_meta) {
  bool incomplete = false;
  if (info.last_complete != info.last_update || info.is_incomplete() ||
      info.dne()) {
  ...
  for (auto &i : sources) {
    ...
    if (source->info.last_complete != source->info.last_update ||
        source->info.is_incomplete() || source->info.dne()) {
      psdout(10) << __func__ << " source " << source->pg_whoami << " incomplete"
                 << dendl;
      incomplete = true;
    }
    if (last_pg_merge_meta.source_pgid != pg_t()) {
  ...
  if (info.history.epoch_created == 0) {
    ...
    psdout(10) << __func__ << " set les/c to "
    ...
    if (info.history.last_epoch_started < info.history.same_interval_since) {
      ...
      info.history.same_up_since = std::min(info.history.same_up_since,
                                            info.history.same_interval_since);
...
void PeeringState::start_split_stats(const set<spg_t> &childpgs,
                                     vector<object_stat_sum_t> *out) {
  out->resize(childpgs.size() + 1);
  info.stats.stats.sum.split(*out);
}

void PeeringState::finish_split_stats(const object_stat_sum_t &stats,
                                      ObjectStore::Transaction &t) {
...
void PeeringState::update_blocked_by() {
  ...
  unsigned keep =
      std::min<unsigned>(blocked_by.size(), cct->_conf->osd_max_pg_blocked_by);
...
static pg_shard_t get_another_shard(const set<pg_shard_t> &pgs, pg_shard_t skip,
                                    shard_id_t shard) {
  for (auto &p : pgs) {
    if (p == skip)
      continue;
...
void PeeringState::update_calc_stats() {
  ...
  if ((is_remapped() || is_undersized() || !is_clean()) &&
    ...
    psdout(20) << __func__ << " actingset " << actingset << " upset " << upset
    ...
    boost::container::flat_set<pair<int64_t, pg_shard_t>>
...

    {
      ...
      psdout(20) << __func__ << " shard " << pg_whoami << " primary objects "
                 << num_objects << " missing " << missing << dendl;
    }

    // All other peers
    for (auto &peer : peer_info) {
      // Primary should not be in the peer_info, skip if it is.
      if (peer.first == pg_whoami)
        continue;
      int64_t missing = 0;
      ...
      if (is_backfill_target(peer.first)) {
        ...
        if (peer_missing.count(peer.first)) {
          ...
          psdout(20) << __func__ << " no peer_missing found for " << peer.first
                     << dendl;
          if (is_recovering()) {
            estimate = true;
          }
      ...
      psdout(20) << __func__ << " shard " << peer.first << " objects "
                 << peer_num_objects << " missing " << missing << dendl;
    }
    ...
    if (not_missing) {
      ...
      // During backfill we know that all non-missing objects are in the
      // actingset
    ...
    int64_t misplaced = 0;
    // a degraded objects has fewer replicas or EC shards than the pool
    // specifies.
    int64_t degraded = 0;
    ...
    if (is_recovering()) {
      for (auto &sml : missing_loc.get_missing_by_count()) {
        for (auto &ml : sml.second) {
          ...
          if (sml.first == shard_id_t::NO_SHARD) {
            psdout(20) << __func__ << " ml " << ml.second << " upset size "
                       << upset.size() << " up " << ml.first.up << dendl;
            ...
            psdout(20) << __func__ << " shard " << sml.first << " ml "
                       << ml.second << " missing shards " << missing_shards
                       << dendl;
          }
      ...
      psdout(20) << __func__ << " missing based misplaced " << misplaced
                 << dendl;
    ...
    if (pool.info.is_replicated()) {
      ...
      if (needed)
        missing_target_objects.emplace(num_objects * needed,
      ...
      for (unsigned i = 0; i < num_shards; ++i) {
        ...
        if (!found)
          missing_target_objects.emplace(num_objects,
                                         pg_shard_t(pg_shard_t::NO_OSD, shard));
      }
    }
    ...
    for (auto m = missing_target_objects.rbegin();
      ...
      if (extra_missing >= 0 && std::get<0>(*m) >= extra_missing) {
        ...
        // 2. extra_missing > std::get<0>(m), so that we know that some
        // extra_missing
    ...
    for (const auto &a : acting_source_objects) {
      int64_t extra_misplaced =
          std::max((int64_t)0, num_objects - std::get<0>(a));
...
std::optional<pg_stat_t> PeeringState::prepare_stats_for_publish(
    bool pg_stats_publish_valid, const pg_stat_t &pg_stats_publish,
  ...
  if (info.stats.state != state) {
    ...
    if (!(state & PG_STATE_ACTIVE) && (info.stats.state & PG_STATE_ACTIVE))
...
void PeeringState::init(int role, const vector<int> &newup, int new_up_primary,
                        const vector<int> &newacting, int new_acting_primary,
                        const pg_history_t &history, const PastIntervals &pi,
                        bool backfill, ObjectStore::Transaction &t) {
  psdout(10) << "init role " << role << " up " << newup << " acting "
             << newacting << " history " << history << " past_intervals " << pi
             << dendl;
...
void PeeringState::dump_peering_state(Formatter *f) {
  ...
  if (!async_recovery_targets.empty()) {
    ...
    for (set<pg_shard_t>::iterator p = async_recovery_targets.begin();
         p != async_recovery_targets.end(); ++p)
      f->dump_stream("shard") << *p;
    f->close_section();
  }
  if (!acting_recovery_backfill.empty()) {
    ...
    for (set<pg_shard_t>::iterator p = acting_recovery_backfill.begin();
         p != acting_recovery_backfill.end(); ++p)
      f->dump_stream("shard") << *p;
    f->close_section();
  }
...
bool PeeringState::append_log_entries_update_missing(
  ...
  PGLog::LogEntryHandlerRef rollbacker{pl->get_log_handler(t)};
  bool invalidate_stats = pg_log.append_new_log_entries(
      info.last_backfill, entries, rollbacker.get());
...
void PeeringState::merge_new_log_entries(
  ...
  bool rebuild_missing =
      append_log_entries_update_missing(entries, t, trim_to, roll_forward_to);
  for (set<pg_shard_t>::const_iterator i = acting_recovery_backfill.begin();
    ...
    psdout(20) << __func__ << " peer_missing for " << peer << " = " << pmissing
               << dendl;
    pg_info_t &pinfo(peer_info[peer]);
    bool invalidate_stats = PGLog::append_log_entries_update_missing(
        pinfo.last_backfill, entries, true, NULL, pmissing, NULL, dpp);
  ...
  for (auto &&i : entries) {
    missing_loc.rebuild(i.soid, pg_whoami, acting_recovery_backfill, info,
                        pg_log.get_missing(), peer_missing, peer_info);
  }
}
...
void PeeringState::append_log(const vector<pg_log_entry_t> &logv,
                              eversion_t trim_to, eversion_t roll_forward_to,
                              ObjectStore::Transaction &t,
                              bool transaction_applied, bool async) {
  ...
  for (vector<pg_log_entry_t>::const_iterator p = logv.begin(); p != logv.end();
       ++p) {
  ...
  psdout(10) << __func__
             << " approx pg log length =  " << pg_log.get_log().approx_size()
             << dendl;
  psdout(10) << __func__ << " transaction_applied = " << transaction_applied
             << dendl;
...
void PeeringState::recover_got(const hobject_t &oid, eversion_t v,
                               bool is_delete, ObjectStore::Transaction &t) {
...
void PeeringState::update_backfill_progress(const hobject_t &updated_backfill,
...
void PeeringState::on_peer_recover(pg_shard_t peer, const hobject_t &soid,
...
void PeeringState::force_object_missing(const set<pg_shard_t> &peers,
  ...
  missing_loc.rebuild(soid, pg_whoami, acting_recovery_backfill, info,
                      pg_log.get_missing(), peer_missing, peer_info);
}

void PeeringState::pre_submit_op(const hobject_t &hoid,
  ...
  if (requires_missing_loc) {
    for (auto &&entry : logv) {
      ...
      missing_loc.add_missing(entry.soid, entry.version, eversion_t(),
                              entry.is_delete());
...
void PeeringState::recovery_committed_to(eversion_t version) {
  ...
  if (last_complete_ondisk == info.last_update) {
    if (!is_primary()) {
      ...
      pl->send_cluster_message(
          get_primary().osd,
          new MOSDPGTrim(get_osdmap_epoch(),
                         spg_t(info.pgid.pgid, primary.shard),
...
void PeeringState::calc_trim_to() {
  ...
  eversion_t limit =
      std::min(min_last_complete_ondisk, pg_log.get_can_rollback_to());
  if (limit != eversion_t() && limit != pg_trim_to &&
    ...
    for (size_t i = 0; i < num_to_trim; ++i) {
      ...
      if (new_trim_to > limit) {
        new_trim_to = limit;
        psdout(10) << "calc_trim_to trimming to min_last_complete_ondisk"
                   << dendl;
        break;
      }
    }
    psdout(10) << "calc_trim_to " << pg_trim_to << " -> " << new_trim_to
               << dendl;
...
void PeeringState::calc_trim_to_aggressive() {
  ...
  eversion_t limit = std::min(pg_log.get_head(), pg_log.get_can_rollback_to());
  ...
  if (limit != eversion_t() && limit != pg_trim_to &&
    ...
    uint64_t num_to_trim =
        std::min<uint64_t>(pg_log.get_log().approx_size() - target,
                           cct->_conf->osd_pg_log_trim_max);
...
void PeeringState::apply_op_stats(const hobject_t &soid,
...
void PeeringState::update_complete_backfill_object_stats(
    const hobject_t &hoid, const pg_stat_t &stats) {
...
void PeeringState::update_peer_last_backfill(
    pg_shard_t peer, const hobject_t &new_last_backfill) {
  pg_info_t &pinfo = peer_info[peer];
...
void PeeringState::set_revert_with_targets(const hobject_t &soid,
...
void PeeringState::prepare_backfill_for_missing(
    const hobject_t &soid, const eversion_t &version,
...
#undef dout_prefix
#define dout_prefix                                                            \
  (context<PeeringMachine>().dpp->gen_prefix(*_dout)                           \
...
PeeringState::Initial::react(const MNotifyRec &notify) {
  DECLARE_LOCALS;
  ps->proc_replica_info(notify.from, notify.notify.info,
                        notify.notify.epoch_sent);
...
boost::statechart::result PeeringState::Started::react(const AdvMap &advmap) {
  ...
  if (ps->should_restart_peering(advmap.up_primary, advmap.acting_primary,
                                 advmap.newup, advmap.newacting, advmap.lastmap,
                                 advmap.osdmap)) {
...
boost::statechart::result PeeringState::Reset::react(const AdvMap &advmap) {
  ...
  if (ps->should_restart_peering(advmap.up_primary, advmap.acting_primary,
    ...
    psdout(10) << "should restart peering, calling start_peering_interval again"
               << dendl;
    ps->start_peering_interval(advmap.lastmap, advmap.newup, advmap.up_primary,
...
boost::statechart::result PeeringState::Reset::react(const ActMap &) {
  ...
  if (ps->should_send_notify() && ps->get_primary().osd >= 0) {
    ...
    context<PeeringMachine>().send_notify(
        ps->get_primary().osd,
        pg_notify_t(ps->get_primary().shard, ps->pg_whoami.shard,
                    ps->get_osdmap_epoch(), ps->get_osdmap_epoch(), ps->info,
                    ps->past_intervals));
  }
...
boost::statechart::result
PeeringState::Primary::react(const MNotifyRec &notevt) {
  DECLARE_LOCALS;
  psdout(7) << "handle_pg_notify from osd." << notevt.from << dendl;
  ps->proc_replica_info(notevt.from, notevt.notify.info,
...
boost::statechart::result
PeeringState::Primary::react(const SetForceRecovery &) {
  DECLARE_LOCALS;
...
boost::statechart::result
PeeringState::Primary::react(const UnsetForceRecovery &) {
  DECLARE_LOCALS;
...
boost::statechart::result
PeeringState::Primary::react(const RequestScrub &evt) {
  DECLARE_LOCALS;
  if (ps->is_primary()) {
...
boost::statechart::result
PeeringState::Primary::react(const SetForceBackfill &) {
  DECLARE_LOCALS;
...
boost::statechart::result
PeeringState::Primary::react(const UnsetForceBackfill &) {
  DECLARE_LOCALS;
...
PeeringState::Peering::Peering(my_context ctx)
    : my_base(ctx), NamedState(context<PeeringMachine>().state_history,
...
boost::statechart::result PeeringState::Peering::react(const QueryState &q) {
  ...
  for (set<int>::iterator p = prior_set.down.begin(); p != prior_set.down.end();
       ++p)
    q.f->dump_int("osd", *p);
  q.f->close_section();
...
void PeeringState::Backfilling::backfill_release_reservations() {
  ...
  for (set<pg_shard_t>::iterator it = ps->backfill_targets.begin();
       it != ps->backfill_targets.end(); ++it) {
    ceph_assert(*it != ps->pg_whoami);
    pl->send_cluster_message(
        it->osd,
        new MBackfillReserve(MBackfillReserve::RELEASE,
...
PeeringState::Backfilling::react(const DeferBackfill &c) {
  ...
  pl->schedule_event_after(
      std::make_shared<PGPeeringEvent>(
          ps->get_osdmap_epoch(), ps->get_osdmap_epoch(), RequestBackfill()),
      c.delay);
...
PeeringState::WaitRemoteBackfillReserved::WaitRemoteBackfillReserved(
    my_context ctx)
    : my_base(ctx),
      NamedState(context<PeeringMachine>().state_history,
                 "Started/Primary/Active/WaitRemoteBackfillReserved"),
      backfill_osd_it(
          context<Active>().remote_shards_to_reserve_backfill.begin()) {
  context<PeeringMachine>().log_enter(state_name);
  DECLARE_LOCALS;
...
boost::statechart::result PeeringState::WaitRemoteBackfillReserved::react(
    const RemoteBackfillReserved &evt) {
  DECLARE_LOCALS;
  ...
  if (backfill_osd_it !=
    ...
    pl->send_cluster_message(
        ...
        new MBackfillReserve(
            ...
            ps->get_osdmap_epoch(), ps->get_backfill_priority(), num_bytes,
...
boost::statechart::result PeeringState::WaitRemoteBackfillReserved::react(
    const RemoteReservationRevoked &evt) {
  retry();
  return transit<NotBackfilling>();
}

/*--WaitLocalBackfillReserved--*/
PeeringState::WaitLocalBackfillReserved::WaitLocalBackfillReserved(
    my_context ctx)
    : my_base(ctx),
      NamedState(context<PeeringMachine>().state_history,
  ...
  pl->request_local_background_io_reservation(
      ...
      std::make_shared<PGPeeringEvent>(ps->get_osdmap_epoch(),
                                       ps->get_osdmap_epoch(),
                                       LocalBackfillReserved()),
      std::make_shared<PGPeeringEvent>(
          ps->get_osdmap_epoch(), ps->get_osdmap_epoch(), DeferBackfill(0.0)));
  pl->publish_stats_to_osd();
}
...
boost::statechart::result PeeringState::NotBackfilling::react(
    const RemoteReservationRejectedTooFull &evt) {
  return discard_event();
}
...
boost::statechart::result PeeringState::RepNotRecovering::react(
...
boost::statechart::result PeeringState::RepWaitRecoveryReserved::react(
    const RemoteRecoveryReserved &evt) {
  DECLARE_LOCALS;
  pl->send_cluster_message(
      ps->primary.osd,
      new MRecoveryReserve(MRecoveryReserve::GRANT,
...
PeeringState::RepNotRecovering::react(const RequestBackfillPrio &evt) {
  ...
  if (!pl->try_reserve_recovery_space(evt.primary_num_bytes,
                                      evt.local_num_bytes)) {
    ...
    if (HAVE_FEATURE(ps->upacting_features, RECOVERY_RESERVATION_2)) {
      ...
      preempt = std::make_shared<PGPeeringEvent>(pl->get_osdmap_epoch(),
                                                 pl->get_osdmap_epoch(),
                                                 RemoteBackfillPreempted());
    }
    pl->request_remote_recovery_reservation(
        evt.priority,
        std::make_shared<PGPeeringEvent>(pl->get_osdmap_epoch(),
                                         pl->get_osdmap_epoch(),
...
PeeringState::RepNotRecovering::react(const RequestRecoveryPrio &evt) {
  ...
  if (HAVE_FEATURE(ps->upacting_features, RECOVERY_RESERVATION_2)) {
    ...
    preempt = std::make_shared<PGPeeringEvent>(ps->get_osdmap_epoch(),
                                               ps->get_osdmap_epoch(),
...
boost::statechart::result PeeringState::RepWaitBackfillReserved::react(
    const RemoteBackfillReserved &evt) {
  DECLARE_LOCALS;

  pl->send_cluster_message(
      ps->primary.osd,
      new MBackfillReserve(MBackfillReserve::GRANT,
                           spg_t(ps->info.pgid.pgid, ps->primary.shard),
                           ps->get_osdmap_epoch()),
      ps->get_osdmap_epoch());
  return transit<RepRecovering>();
...
PeeringState::RepRecovering::react(const RemoteRecoveryPreempted &) {
  ...
  pl->send_cluster_message(
      ps->primary.osd,
      new MRecoveryReserve(MRecoveryReserve::REVOKE,
                           spg_t(ps->info.pgid.pgid, ps->primary.shard),
                           ps->get_osdmap_epoch()),
      ps->get_osdmap_epoch());
  return discard_event();
...
PeeringState::RepRecovering::react(const BackfillTooFull &) {
  ...
  pl->send_cluster_message(
      ps->primary.osd,
      new MBackfillReserve(MBackfillReserve::REVOKE_TOOFULL,
                           spg_t(ps->info.pgid.pgid, ps->primary.shard),
                           ps->get_osdmap_epoch()),
      ps->get_osdmap_epoch());
  return discard_event();
...
PeeringState::RepRecovering::react(const RemoteBackfillPreempted &) {
  ...
  pl->send_cluster_message(
      ps->primary.osd,
      new MBackfillReserve(MBackfillReserve::REVOKE,
                           spg_t(ps->info.pgid.pgid, ps->primary.shard),
                           ps->get_osdmap_epoch()),
      ps->get_osdmap_epoch());
  return discard_event();
...
PeeringState::WaitLocalRecoveryReserved::WaitLocalRecoveryReserved(
    my_context ctx)
    : my_base(ctx),
      NamedState(context<PeeringMachine>().state_history,
  ...
  pl->request_local_background_io_reservation(
      ...
      std::make_shared<PGPeeringEvent>(
          ps->get_osdmap_epoch(), ps->get_osdmap_epoch(), DeferRecovery(0.0)));
  pl->publish_stats_to_osd();
}
...
PeeringState::WaitLocalRecoveryReserved::react(const RecoveryTooFull &evt) {
  ...
  pl->schedule_event_after(
      std::make_shared<PGPeeringEvent>(ps->get_osdmap_epoch(),
                                       ps->get_osdmap_epoch(), DoRecovery()),
...
PeeringState::WaitRemoteRecoveryReserved::WaitRemoteRecoveryReserved(
    my_context ctx)
    : my_base(ctx),
      NamedState(context<PeeringMachine>().state_history,
...
boost::statechart::result PeeringState::WaitRemoteRecoveryReserved::react(
    const RemoteRecoveryReserved &evt) {
  DECLARE_LOCALS;
...
void PeeringState::Recovering::release_reservations(bool cancel) {
  ...
  for (set<pg_shard_t>::const_iterator i =
           context<Active>().remote_shards_to_reserve_recovery.begin();
       i != context<Active>().remote_shards_to_reserve_recovery.end(); ++i) {
...
PeeringState::Active::Active(my_context ctx)
    : my_base(ctx), NamedState(context<PeeringMachine>().state_history,
      ...
      remote_shards_to_reserve_recovery(unique_osd_shard_set(
          context<PeeringMachine>().state->pg_whoami,
          context<PeeringMachine>().state->acting_recovery_backfill)),
      remote_shards_to_reserve_backfill(unique_osd_shard_set(
          context<PeeringMachine>().state->pg_whoami,
  ...
  for (set<pg_shard_t>::iterator p = ps->acting_recovery_backfill.begin();
       p != ps->acting_recovery_backfill.end(); ++p) {
...
boost::statechart::result PeeringState::Active::react(const AdvMap &advmap) {
  ...
  if (ps->info.stats.reported_epoch +
          ps->cct->_conf->osd_pg_stat_report_interval_max <
      advmap.osdmap->get_epoch()) {
    psdout(20) << "reporting stats to osd after "
               << (advmap.osdmap->get_epoch() - ps->info.stats.reported_epoch)
               << " epochs" << dendl;
    need_publish = true;
  }
...
boost::statechart::result PeeringState::Active::react(const ActMap &) {
  ...
  if (unfound > 0 && ps->all_unfound_are_queried_or_lost(ps->get_osdmap())) {
    if (ps->cct->_conf->osd_auto_mark_unfound_lost) {
      pl->get_clog_error()
          << context<PeeringMachine>().spgid.pgid << " has " << unfound
...
boost::statechart::result
PeeringState::Active::react(const MNotifyRec &notevt) {

  DECLARE_LOCALS;
  ceph_assert(ps->is_primary());
  if (ps->peer_info.count(notevt.from)) {
    psdout(10) << "Active: got notify from " << notevt.from
               << ", already have info from that osd, ignoring" << dendl;
  } else if (ps->peer_purged.count(notevt.from)) {
    psdout(10) << "Active: got notify from " << notevt.from
               << ", already purged that peer, ignoring" << dendl;
  } else {
    psdout(10) << "Active: got notify from " << notevt.from
    ...
    ps->proc_replica_info(notevt.from, notevt.notify.info,
                          notevt.notify.epoch_sent);
    if (ps->have_unfound() ||
        (ps->is_degraded() && ps->might_have_unfound.count(notevt.from))) {
...
boost::statechart::result PeeringState::Active::react(const MTrim &trim) {
  ...
  ldout(ps->cct, 10) << " replica osd." << trim.from << " lcod " << trim.trim_to
                     << dendl;
  ps->peer_last_complete_ondisk[pg_shard_t(trim.from, trim.shard)] =
      trim.trim_to;
...
boost::statechart::result PeeringState::Active::react(const MInfoRec &infoevt) {
  ...
  if (ps->is_acting_recovery_backfill(infoevt.from) &&
    ...
    psdout(10) << " peer osd." << infoevt.from << " activated and committed"
               << dendl;
...
boost::statechart::result PeeringState::Active::react(const MLogRec &logevt) {
  DECLARE_LOCALS;
  psdout(10) << "searching osd." << logevt.from << " log for unfound items"
             << dendl;
  ps->proc_replica_log(logevt.msg->info, logevt.msg->log, logevt.msg->missing,
                       logevt.from);
  bool got_missing = ps->search_for_missing(
      ps->peer_info[logevt.from], ps->peer_missing[logevt.from], logevt.from,
      context<PeeringMachine>().get_recovery_ctx());
...
boost::statechart::result
PeeringState::Active::react(const ActivateCommitted &evt) {
  DECLARE_LOCALS;
  ceph_assert(!ps->peer_activated.count(ps->pg_whoami));
  ps->peer_activated.insert(ps->pg_whoami);
  psdout(10) << "_activate_committed " << evt.epoch << " peer_activated now "
             << ps->peer_activated << " last_interval_started "
             << ps->info.history.last_interval_started << " last_epoch_started "
             << ps->info.history.last_epoch_started << " same_interval_since "
             << ps->info.history.same_interval_since << dendl;
  ceph_assert(!ps->acting_recovery_backfill.empty());
  if (ps->peer_activated.size() == ps->acting_recovery_backfill.size())
    all_activated_and_committed();
  return discard_event();
...
boost::statechart::result
PeeringState::Active::react(const AllReplicasActivated &evt) {
  ...
  if (ps->prior_readable_until_ub > mnow) {
    ...
    pl->queue_check_readable(ps->last_peering_reset,
...
boost::statechart::result
PeeringState::Active::react(const CheckReadable &evt) {
  DECLARE_LOCALS;
  pl->recheck_readable();
  return discard_event();
}
...
boost::statechart::result
PeeringState::ReplicaActive::react(const Activate &actevt) {
  DECLARE_LOCALS;
...
boost::statechart::result
PeeringState::ReplicaActive::react(const ActivateCommitted &evt) {
  DECLARE_LOCALS;
  ...
  rctx.send_info(ps->get_primary().osd,
                 spg_t(ps->info.pgid.pgid, ps->get_primary().shard), epoch,
                 epoch, i, {}, /* lease */
                 ps->get_lease_ack());
...
boost::statechart::result PeeringState::ReplicaActive::react(const MLease &l) {
  ...
  pl->send_cluster_message(
      ps->get_primary().osd,
      new MOSDPGLeaseAck(epoch, spg_t(spgid.pgid, ps->get_primary().shard),
                         ps->get_lease_ack()),
      epoch);
  return discard_event();
}
...
boost::statechart::result
PeeringState::ReplicaActive::react(const MInfoRec &infoevt) {
  DECLARE_LOCALS;
...
boost::statechart::result
PeeringState::ReplicaActive::react(const MLogRec &logevt) {
  DECLARE_LOCALS;
...
boost::statechart::result
PeeringState::ReplicaActive::react(const MTrim &trim) {
  DECLARE_LOCALS;
...
boost::statechart::result
PeeringState::ReplicaActive::react(const MQuery &query) {
  DECLARE_LOCALS;
...
boost::statechart::result
PeeringState::ReplicaActive::react(const QueryState &q) {
  q.f->open_object_section("state");
  q.f->dump_string("name", state_name);
  q.f->dump_stream("enter_time") << enter_time;
  q.f->close_section();
...
boost::statechart::result PeeringState::Stray::react(const MLogRec &logevt) {
  ...
  psdout(10) << "got info+log from osd." << logevt.from << " " << msg->info
             << " " << msg->log << dendl;
...
boost::statechart::result PeeringState::Stray::react(const MInfoRec &infoevt) {
  DECLARE_LOCALS;
  psdout(10) << "got info from osd." << infoevt.from << " " << infoevt.info
             << dendl;
...
PeeringState::WaitDeleteReserved::WaitDeleteReserved(my_context ctx)
  ...
  pl->request_local_background_io_reservation(
      ...
      std::make_shared<PGPeeringEvent>(
          ps->get_osdmap_epoch(), ps->get_osdmap_epoch(), DeleteReserved()),
      std::make_shared<PGPeeringEvent>(
          ps->get_osdmap_epoch(), ps->get_osdmap_epoch(), DeleteInterrupted()));
}
  ...
  for (set<pg_shard_t>::const_iterator it = prior_set.probe.begin();
    ...
    if (peer_info_requested.count(peer)) {
      ...
      context<PeeringMachine>().send_query(
          peer.osd, pg_query_t(pg_query_t::INFO, it->shard, ps->pg_whoami.shard,
...
boost::statechart::result
PeeringState::GetInfo::react(const MNotifyRec &infoevt) {
  ...
  if (ps->proc_replica_info(infoevt.from, infoevt.notify.info,
                            infoevt.notify.epoch_sent)) {
    // we got something new ...
    PastIntervals::PriorSet &prior_set = context<Peering>().prior_set;
    if (old_start < ps->info.history.last_epoch_started) {
      psdout(10) << " last_epoch_started moved forward, rebuilding prior"
                 << dendl;
      prior_set = ps->build_prior();
      ps->prior_readable_down_osds = prior_set.down;
    ...
    psdout(20) << "Adding osd: " << infoevt.from.osd
               << " peer features: " << hex << infoevt.features << dec << dendl;
    ps->apply_peer_features(infoevt.features);

    // are we done getting everything?
    if (peer_info_requested.empty() && !prior_set.pg_down) {
      psdout(20) << "Common peer features: " << hex
                 << ps->get_min_peer_features() << dec << dendl;
      psdout(20) << "Common acting features: " << hex
                 << ps->get_min_acting_features() << dec << dendl;
      psdout(20) << "Common upacting features: " << hex
                 << ps->get_min_upacting_features() << dec << dendl;
      post_event(GotInfo());
    }
  }
  return discard_event();
...
PeeringState::GetLog::GetLog(my_context ctx)
  ...
  if (ps->info.last_update < best.log_tail) {
    psdout(10) << " not contiguous with osd." << auth_log_shard << ", down"
               << dendl;
    post_event(IsIncomplete());
    return;
  }
  ...
  context<PeeringMachine>().send_query(
      auth_log_shard.osd,
      pg_query_t(pg_query_t::LOG, auth_log_shard.shard, ps->pg_whoami.shard,
                 request_log_from, ps->info.history, ps->get_osdmap_epoch()));
...
boost::statechart::result PeeringState::GetLog::react(const AdvMap &advmap) {
  ...
  if (!advmap.osdmap->is_up(auth_log_shard.osd)) {
    psdout(10) << "GetLog: auth_log_shard osd." << auth_log_shard.osd
               << " went down" << dendl;
    post_event(advmap);
    return transit<Reset>();
  }
...
boost::statechart::result
PeeringState::WaitActingChange::react(const AdvMap &advmap) {
  DECLARE_LOCALS;
  OSDMapRef osdmap = advmap.osdmap;

  psdout(10) << "verifying no want_acting " << ps->want_acting
             << " targets didn't go down" << dendl;
  for (vector<int>::iterator p = ps->want_acting.begin();
       p != ps->want_acting.end(); ++p) {
    if (!osdmap->is_up(*p)) {
      psdout(10) << " want_acting target osd." << *p << " went down, resetting"
                 << dendl;
      post_event(advmap);
      return transit<Reset>();
    }
...
boost::statechart::result
PeeringState::WaitActingChange::react(const MLogRec &logevt) {
  psdout(10) << "In WaitActingChange, ignoring MLocRec" << dendl;
  return discard_event();
}

boost::statechart::result
PeeringState::WaitActingChange::react(const MInfoRec &evt) {
  psdout(10) << "In WaitActingChange, ignoring MInfoRec" << dendl;
  return discard_event();
}

boost::statechart::result
PeeringState::WaitActingChange::react(const MNotifyRec &evt) {
  psdout(10) << "In WaitActingChange, ignoring MNotifyRec" << dendl;
  return discard_event();
}

boost::statechart::result
PeeringState::WaitActingChange::react(const QueryState &q) {
  q.f->open_object_section("state");
  q.f->dump_string("name", state_name);
  q.f->dump_stream("enter_time") << enter_time;
...
boost::statechart::result PeeringState::Down::react(const MNotifyRec &infoevt) {
  ...
  if (!ps->peer_info.count(infoevt.from) &&
      ps->get_osdmap()->has_been_up_since(infoevt.from.osd,
                                          infoevt.notify.epoch_sent)) {
  ...
  if (ps->info.history.last_epoch_started > old_start) {
    psdout(10) << " last_epoch_started moved forward, re-enter getinfo"
               << dendl;
    ps->state_clear(PG_STATE_DOWN);
    ps->state_set(PG_STATE_PEERING);
...
boost::statechart::result
PeeringState::Incomplete::react(const AdvMap &advmap) {
  DECLARE_LOCALS;
  int64_t poolnum = ps->info.pgid.pool();

  // Reset if min_size turn smaller than previous value, pg might now be able to
  // go active
...
boost::statechart::result
PeeringState::Incomplete::react(const MNotifyRec &notevt) {
  DECLARE_LOCALS;
  psdout(7) << "handle_pg_notify from osd." << notevt.from << dendl;
  if (ps->proc_replica_info(notevt.from, notevt.notify.info,
                            notevt.notify.epoch_sent)) {
...
PeeringState::GetMissing::GetMissing(my_context ctx)
  ...
  for (set<pg_shard_t>::iterator i = ps->acting_recovery_backfill.begin();
       i != ps->acting_recovery_backfill.end(); ++i) {
    if (*i == ps->get_primary())
      continue;
    ...
    ps->peer_missing[*i].may_include_deletes =
        !ps->perform_deletes_during_peering();

    if (pi.is_empty())
      continue; // no pg data, nothing divergent

    if (pi.last_update < ps->pg_log.get_tail()) {
      psdout(10) << " osd." << *i << " is not contiguous, will restart backfill"
                 << dendl;
    ...
    if (pi.last_backfill == hobject_t()) {
      psdout(10) << " osd." << *i
                 << " will fully backfill; can infer empty missing set"
                 << dendl;
      ps->peer_missing[*i].clear();
      continue;
    }
    ...
    if (pi.log_tail <= since) {
      psdout(10) << " requesting log+missing since " << since << " from osd."
                 << *i << dendl;
      context<PeeringMachine>().send_query(
          i->osd, pg_query_t(pg_query_t::LOG, i->shard, ps->pg_whoami.shard,
                             since, ps->info.history, ps->get_osdmap_epoch()));
    } else {
      psdout(10) << " requesting fulllog+missing from osd." << *i
                 << " (want since " << since << " < log.tail " << pi.log_tail
                 << ")" << dendl;
      context<PeeringMachine>().send_query(
          i->osd, pg_query_t(pg_query_t::FULLLOG, i->shard, ps->pg_whoami.shard,
                             ps->info.history, ps->get_osdmap_epoch()));
    }
...
boost::statechart::result
PeeringState::GetMissing::react(const MLogRec &logevt) {
  DECLARE_LOCALS;
...
boost::statechart::result PeeringState::GetMissing::react(const QueryState &q) {
  ...
  for (set<pg_shard_t>::iterator p = peer_missing_requested.begin();
       p != peer_missing_requested.end(); ++p) {
    q.f->open_object_section("osd");
    q.f->dump_stream("osd") << *p;
    if (ps->peer_missing.count(*p)) {
...
boost::statechart::result
PeeringState::WaitUpThru::react(const MLogRec &logevt) {
  DECLARE_LOCALS;
...
boost::statechart::result PeeringState::WaitUpThru::react(const QueryState &q) {
  ...
  q.f->dump_stream("enter_time") << enter_time;
  q.f->dump_string("comment",
                   "waiting for osdmap to reflect a new up_thru for this osd");
  q.f->close_section();
  return forward_event();
}
...
void PeeringState::PeeringMachine::log_exit(const char *state_name,
  ...
  DECLARE_LOCALS;
  utime_t dur = ceph_clock_now() - enter_time;
  psdout(5) << "exit " << state_name << " " << dur << " " << event_count << " "
            << event_time << dendl;
