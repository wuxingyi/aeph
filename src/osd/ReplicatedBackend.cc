...
public:
  PG_SendMessageOnConn(PGBackend::Listener *pg, Message *reply,
                       ConnectionRef conn)
      : pg(pg), reply(reply), conn(conn) {}
  void finish(int) override { pg->send_message_osd_cluster(reply, conn.get()); }
};
...
public:
  PG_RecoveryQueueAsync(PGBackend::Listener *pg,
...
ReplicatedBackend::ReplicatedBackend(PGBackend::Listener *pg,
                                     const coll_t &coll,
                                     ObjectStore::CollectionHandle &c,
                                     ObjectStore *store, CephContext *cct)
    : PGBackend(cct, pg, store, coll, c) {}

void ReplicatedBackend::run_recovery_op(PGBackend::RecoveryHandle *_h,
                                        int priority) {
...
int ReplicatedBackend::recover_object(const hobject_t &hoid, eversion_t v,
...
int ReplicatedBackend::objects_readv_sync(const hobject_t &hoid,
                                          map<uint64_t, uint64_t> &m,
                                          uint32_t op_flags, bufferlist *bl) {
...
public:
  C_OSD_OnOpCommit(ReplicatedBackend *pg,
                   ceph::ref_t<ReplicatedBackend::InProgressOp> op)
...
void generate_transaction(
    PGTransactionUPtr &pgt, const coll_t &coll,
    vector<pg_log_entry_t> &log_entries, ObjectStore::Transaction *t,
    set<hobject_t> *added, set<hobject_t> *removed,
      ...
      [&](pair<const hobject_t, PGTransaction::ObjectOperation> &obj_op) {
        ...
        match(
            ...
            [&](const PGTransaction::ObjectOperation::Init::Clone &op) {
              t->clone(coll,
                       ghobject_t(op.source, ghobject_t::NO_GEN,
                                  shard_id_t::NO_SHARD),
                       goid);
            },
        ...
        if (op.alloc_hint) {
          ...
          t->set_alloc_hint(coll, goid, hint.expected_object_size,
                            hint.expected_write_size, hint.flags);
        }
        ...
        for (auto &&extent : op.buffer_updates) {
          ...
          match(
              ...
              [&](const BufferUpdate::Write &op) {
                t->write(coll, goid, extent.get_off(), extent.get_len(),
                         op.buffer, op.fadvise_flags);
              },
              [&](const BufferUpdate::Zero &op) {
                t->zero(coll, goid, extent.get_off(), extent.get_len());
              },
              [&](const BufferUpdate::CloneRange &op) {
                ceph_assert(op.len == extent.get_len());
                t->clone_range(coll,
                               ghobject_t(op.from, ghobject_t::NO_GEN,
                                          shard_id_t::NO_SHARD),
                               goid, op.offset, extent.get_len(),
                               extent.get_off());
              });
        }
      });
...
void ReplicatedBackend::submit_transaction(
    const hobject_t &soid, const object_stat_sum_t &delta_stats,
  ...
  issue_op(soid, at_version, tid, reqid, trim_to, at_version,
           added.size() ? *(added.begin()) : hobject_t(),
           removed.size() ? *(removed.begin()) : hobject_t(), log_entries,
           hset_history, &op, op_t);

  add_temp_objs(added);
  clear_temp_objs(removed);

  parent->log_operation(log_entries, hset_history, trim_to, at_version, true,
                        op_t);
...
void ReplicatedBackend::op_commit(const ceph::ref_t<InProgressOp> &op) {
  ...
  FUNCTRACE(cct);
  OID_EVENT_TRACE_WITH_MSG((op && op->op) ? op->op->get_req() : NULL,
...
int ReplicatedBackend::be_deep_scrub(const hobject_t &poid, ScrubMap &map,
  ...
  if (!pos.data_done()) {
    ...
    bufferlist bl;
    r = store->read(ch,
                    ghobject_t(poid, ghobject_t::NO_GEN,
                               get_parent()->whoami_shard().shard),
                    pos.data_pos, cct->_conf->osd_deep_scrub_stride, bl,
                    fadvise_flags);
    if (r < 0) {
  ...
  while (iter->status() == 0 && iter->valid()) {
    ...
    if (iter->status() < 0) {
      dout(25) << __func__ << "  " << poid << " on omap scan, db status error"
               << dendl;
      o.read_error = true;
      return 0;
    }
  ...
  if (pos.omap_keys >
          cct->_conf->osd_deep_scrub_large_omap_object_key_threshold ||
      pos.omap_bytes >
          cct->_conf->osd_deep_scrub_large_omap_object_value_sum_threshold) {
  ...
  dout(20) << __func__ << " done with " << poid << " omap_digest " << std::hex
           << o.omap_digest << std::dec << dendl;
...
void ReplicatedBackend::_do_push(OpRequestRef op) {
  ...
  t.register_on_complete(
      new PG_SendMessageOnConn(get_parent(), reply, m->get_connection()));
  ...
  void finish(ThreadPool::TPHandle &handle) override {
    ...
    for (auto &&i : to_continue) {
      ...
      if (started < 0) {
        ...
        bc->get_parent()->on_failed_pull({bc->get_parent()->whoami_shard()},
...
void ReplicatedBackend::_do_pull_response(OpRequestRef op) {
  ...
  if (get_parent()->check_failsafe_full()) {
    dout(10) << __func__
             << " Out of space (failsafe) processing pull response (push)."
             << dendl;
    ceph_abort();
  }
  ...
  for (vector<PushOp>::const_iterator i = m->pushes.begin();
       i != m->pushes.end(); ++i) {
    bool more =
        handle_pull_response(from, *i, &(replies.back()), &to_continue, &t);
    if (more)
      replies.push_back(PullOp());
  }
  if (!to_continue.empty()) {
    C_ReplicatedBackend_OnPullComplete *c =
        new C_ReplicatedBackend_OnPullComplete(this, m->get_priority());
    c->to_continue.swap(to_continue);
    t.register_on_complete(new PG_RecoveryQueueAsync(
        get_parent(), get_parent()->bless_unlocked_gencontext(c)));
  }
...
Message *ReplicatedBackend::generate_subop(
    const hobject_t &soid, const eversion_t &at_version, ceph_tid_t tid,
    osd_reqid_t reqid, eversion_t pg_trim_to, eversion_t pg_roll_forward_to,
    hobject_t new_temp_oid, hobject_t discard_temp_oid,
  ...
  MOSDRepOp *wr = new MOSDRepOp(
      reqid, parent->whoami_shard(), spg_t(get_info().pgid.pgid, peer.shard),
      soid, acks_wanted, get_osdmap_epoch(),
      parent->get_last_peering_reset_epoch(), tid, at_version);
...
void ReplicatedBackend::issue_op(
  ...
  if (parent->get_acting_recovery_backfill_shards().size() > 1) {
    ...
    for (const auto &shard :
      ...
      Message *wr;
      wr = generate_subop(soid, at_version, tid, reqid, pg_trim_to,
                          pg_roll_forward_to, new_temp_oid, discard_temp_oid,
                          logs, hset_hist, op_t, shard, pinfo);
...
void ReplicatedBackend::do_repop(OpRequestRef op) {
  ...
  if (m->discard_temp_oid != hobject_t()) {
    dout(20) << __func__ << " stop tracking temp " << m->discard_temp_oid
             << dendl;
    if (rm->opt.empty()) {
  ...
  if (pmissing.is_missing(soid)) {
    async = true;
    dout(30) << __func__ << " is_missing " << pmissing.is_missing(soid)
             << dendl;
    for (auto &&e : log) {
  ...
  parent->log_operation(log, m->updated_hit_set_history, m->pg_trim_to,
                        m->pg_roll_forward_to, update_snaps, rm->localt, async);
...
void ReplicatedBackend::repop_commit(RepModifyRef rm) {
  ...
  dout(10) << __func__ << " on op " << *m << ", sending commit to osd."
           << rm->ackerosd << dendl;
  ...
  MOSDRepOpReply *reply =
      new MOSDRepOpReply(m, get_parent()->whoami_shard(), 0, get_osdmap_epoch(),
                         m->get_min_epoch(), CEPH_OSD_FLAG_ONDISK);
  reply->set_last_complete_ondisk(rm->last_complete);
  reply->set_priority(CEPH_MSG_PRIO_HIGH); // this better match ack priority!
  reply->trace = rm->op->pg_trace;
  get_parent()->send_message_osd_cluster(rm->ackerosd, reply,
                                         get_osdmap_epoch());
...
void ReplicatedBackend::calc_head_subsets(
  ...
  dout(10) << "calc_head_subsets " << head << " clone_overlap "
           << snapset.clone_overlap << dendl;
  ...
  if (get_parent()->get_pool().allow_incomplete_clones()) {
    dout(10) << __func__ << ": caching (was) enabled, skipping clone subsets"
             << dendl;
    return;
  }
...
void ReplicatedBackend::calc_clone_subsets(
    SnapSet &snapset, const hobject_t &soid, const pg_missing_t &missing,
    const hobject_t &last_backfill, interval_set<uint64_t> &data_subset,
    map<hobject_t, interval_set<uint64_t>> &clone_subsets,
    ObcLockManager &manager) {
  dout(10) << "calc_clone_subsets " << soid << " clone_overlap "
           << snapset.clone_overlap << dendl;
...
void ReplicatedBackend::prepare_pull(eversion_t v, const hobject_t &soid,
                                     ObjectContextRef headctx, RPGHandle *h) {
  const auto missing_iter =
      get_parent()->get_local_missing().get_items().find(soid);
  ceph_assert(missing_iter !=
  ...
  if (p == q->second.end()) {
    ...
    p = q->second.begin();
    std::advance(p, util::generate_random_number<int>(0, q->second.size() - 1));
  }
  ...
  if (pmissing.is_missing(soid, v)) {
    ...
    ceph_assert(
        get_parent()->get_log().get_log().objects.count(soid) &&
  ...
  if (soid.is_snap()) {
    ...
    calc_clone_subsets(ssc->snapset, soid, get_parent()->get_local_missing(),
                       get_info().last_backfill, recovery_info.copy_subset,
                       recovery_info.clone_subset, lock_manager);
    // FIXME: this may overestimate if we are pulling multiple clones in
    // parallel...
 ...
 */
int ReplicatedBackend::prep_push_to_replica(ObjectContextRef obc,
                                            const hobject_t &soid,
                                            pg_shard_t peer, PushOp *pop,
                                            bool cache_dont_need) {
  const object_info_t &oi = obc->obs.oi;
  uint64_t size = obc->obs.oi.size;

  dout(10) << __func__ << ": " << soid << " v" << oi.version << " size " << size
           << " to osd." << peer << dendl;
  ...
  if (soid.snap && soid.snap < CEPH_NOSNAP) {
    ...
    if (get_parent()->get_local_missing().is_missing(head)) {
      dout(15) << "push_to_replica missing head " << head
    ...
    calc_clone_subsets(ssc->snapset, soid, pm->second, pi->second.last_backfill,
                       data_subset, clone_subsets, lock_manager);
    ...
    dout(15) << "push_to_replica snapset is " << ssc->snapset << dendl;
    calc_head_subsets(
        obc, ssc->snapset, soid,
        get_parent()->get_shard_missing().find(peer)->second,
        get_parent()->get_shard_info().find(peer)->second.last_backfill,
        data_subset, clone_subsets, lock_manager);
  }

  return prep_push(obc, soid, peer, oi.version, data_subset, clone_subsets, pop,
                   cache_dont_need, std::move(lock_manager));
}

int ReplicatedBackend::prep_push(ObjectContextRef obc, const hobject_t &soid,
                                 pg_shard_t peer, PushOp *pop,
                                 bool cache_dont_need) {
  interval_set<uint64_t> data_subset;
  if (obc->obs.oi.size)
  ...
  return prep_push(obc, soid, peer, obc->obs.oi.version, data_subset,
                   clone_subsets, pop, cache_dont_need, ObcLockManager());
}

int ReplicatedBackend::prep_push(
    ObjectContextRef obc, const hobject_t &soid, pg_shard_t peer,
    eversion_t version, interval_set<uint64_t> &data_subset,
  ...
  int r = build_push_op(pi.recovery_info, pi.recovery_progress, &new_progress,
                        pop, &(pi.stat), cache_dont_need);
  if (r < 0)
    return r;
...
void ReplicatedBackend::submit_push_data(
    const ObjectRecoveryInfo &recovery_info, bool first, bool complete,
  ...
  if (first && complete) {
    ...
    if (first) {
      dout(10) << __func__ << ": Adding oid " << target_oid
  ...
  if (first) {
    if (!complete) {
      ...
      object_info_t oi(bv);
      t->set_alloc_hint(coll, ghobject_t(target_oid), oi.expected_object_size,
    ...
    if (get_parent()->pg_is_remote_backfilling()) {
      ...
      if (size != recovery_info.oi.size) {
        get_parent()->pg_add_local_num_bytes((int64_t)recovery_info.oi.size -
                                             (int64_t)size);
        get_parent()->pg_add_num_bytes((int64_t)recovery_info.oi.size -
                                       (int64_t)size);
        dout(10) << __func__ << " " << recovery_info.soid << " backfill size "
                 << recovery_info.oi.size << " previous size " << size
                 << " net size " << recovery_info.oi.size - size << dendl;
      }
    }
    if (!complete) {
      ...
      if (recovery_info.object_exist) {
        assert(r == 0);
        uint64_t local_size =
            std::min(recovery_info.size, (uint64_t)st.st_size);
        ...
        for (interval_set<uint64_t>::const_iterator q =
                 local_intervals_included.begin();
             q != local_intervals_included.end(); ++q) {
          dout(15) << " clone_range " << recovery_info.soid << " "
                   << q.get_start() << "~" << q.get_len() << dendl;
          t->clone_range(coll, ghobject_t(recovery_info.soid),
                         ghobject_t(target_oid), q.get_start(), q.get_len(),
                         q.get_start());
        }
      }
    }
  ...
  for (interval_set<uint64_t>::const_iterator p = intervals_included.begin();
       p != intervals_included.end(); ++p) {
    bufferlist bit;
    bit.substr_of(data_included, off, p.get_len());
    t->write(coll, ghobject_t(target_oid), p.get_start(), p.get_len(), bit,
             fadvise_flags);
    off += p.get_len();
  }
...
ObjectRecoveryInfo
ReplicatedBackend::recalc_subsets(const ObjectRecoveryInfo &recovery_info,
                                  SnapSetContext *ssc,
                                  ObcLockManager &manager) {
  ...
  calc_clone_subsets(ssc->snapset, new_info.soid,
                     get_parent()->get_local_missing(),
                     get_info().last_backfill, new_info.copy_subset,
                     new_info.clone_subset, manager);
  return new_info;
}
...
bool ReplicatedBackend::handle_pull_response(
  ...
  dout(10) << "handle_pull_response " << pop.recovery_info << pop.after_progress
           ...
           << " data_included: " << data_included << dendl;
  ...
  if (first) {
    ...
    pi.recovery_info =
        recalc_subsets(pi.recovery_info, pi.obc->ssc, pi.lock_manager);
  }

  interval_set<uint64_t> usable_intervals;
  bufferlist usable_data;
  trim_pushed_data(pi.recovery_info.copy_subset, data_included, data,
  ...
  submit_push_data(pi.recovery_info, first, complete, clear_omap,
                   pi.cache_dont_need, data_zeros, data_included, data,
                   pop.omap_header, pop.attrset, pop.omap_entries, t);

  pi.stat.num_keys_recovered += pop.omap_entries.size();
  pi.stat.num_bytes_recovered += data.length();
  get_parent()->get_logger()->inc(l_osd_rbytes,
                                  pop.omap_entries.size() + data.length());

  if (complete) {
    pi.stat.num_objects_recovered++;
    // XXX: This could overcount if regular recovery is needed right after a
    // repair
...
void ReplicatedBackend::handle_push(pg_shard_t from, const PushOp &pop,
                                    PushReplyOp *response,
                                    ObjectStore::Transaction *t,
                                    bool is_repair) {
  dout(10) << "handle_push " << pop.recovery_info << pop.after_progress
           << dendl;
  bufferlist data;
  data = pop.data;
  bool first = pop.before_progress.first;
  bool complete =
      pop.after_progress.data_complete && pop.after_progress.omap_complete;
  bool clear_omap = !pop.before_progress.omap_complete;
  interval_set<uint64_t> data_zeros;
  uint64_t z_offset = pop.before_progress.data_recovered_to;
  uint64_t z_length = pop.after_progress.data_recovered_to -
  ...
  submit_push_data(pop.recovery_info, first, complete, clear_omap,
                   true, // must be replicate
                   data_zeros, pop.data_included, data, pop.omap_header,
                   pop.attrset, pop.omap_entries, t);
  ...
  if (complete) {
    ...
    get_parent()->on_local_recover(pop.recovery_info.soid, pop.recovery_info,
                                   ObjectContextRef(), // ok, is replica
                                   false, t);
  }
}

void ReplicatedBackend::send_pushes(int prio,
                                    map<pg_shard_t, vector<PushOp>> &pushes) {
  for (map<pg_shard_t, vector<PushOp>>::iterator i = pushes.begin();
       i != pushes.end(); ++i) {
    ConnectionRef con =
        get_parent()->get_con_osd_cluster(i->first.osd, get_osdmap_epoch());
    if (!con)
      continue;
    ...
    while (j != i->second.end()) {
      ...
      for (; (j != i->second.end() && cost < cct->_conf->osd_max_push_cost &&
           ...
           ++j) {
        dout(20) << __func__ << ": sending push " << *j << " to osd."
                 << i->first << dendl;
        cost += j->cost(cct);
        pushes += 1;
...
void ReplicatedBackend::send_pulls(int prio,
  ...
  for (map<pg_shard_t, vector<PullOp>>::iterator i = pulls.begin();
    ...
    if (!con)
      continue;
    dout(20) << __func__ << ": sending pulls " << i->second << " to osd."
             << i->first << dendl;
...
int ReplicatedBackend::build_push_op(const ObjectRecoveryInfo &recovery_info,
                                     ...
                                     PushOp *out_op, object_stat_sum_t *stat,
                                     bool cache_dont_need) {
  ...
  dout(7) << __func__ << " " << recovery_info.soid << " v "
          << recovery_info.version << " size " << recovery_info.size
          << " recovery_info: " << recovery_info << dendl;

  eversion_t v = recovery_info.version;
  object_info_t oi;
  if (progress.first) {
    int r = store->omap_get_header(ch, ghobject_t(recovery_info.soid),
                                   &out_op->omap_header);
    if (r < 0) {
      dout(1) << __func__ << " get omap header failed: " << cpp_strerror(-r)
              << dendl;
      return r;
    }
    ...
    try {
      ...
      dout(0) << __func__ << ": bad object_info_t: " << recovery_info.soid
              << dendl;
      return -EINVAL;
    }
    ...
    if (v == eversion_t()) {
      ...
      get_parent()->clog_error()
          << get_info().pgid << " push " << recovery_info.soid << " v "
          << recovery_info.version << " failed because local copy is "
          << oi.version;
      return -EINVAL;
    }
  ...
  if (!progress.omap_complete) {
    ObjectMap::ObjectMapIterator iter =
        store->get_omap_iterator(ch, ghobject_t(recovery_info.soid));
    ceph_assert(iter);
    for (iter->lower_bound(progress.omap_recovered_to); iter->valid();
         iter->next()) {
      if (!out_op->omap_entries.empty() &&
          ((cct->_conf->osd_recovery_max_omap_entries_per_chunk > 0 &&
            out_op->omap_entries.size() >=
                cct->_conf->osd_recovery_max_omap_entries_per_chunk) ||
  ...
  if (available > 0) {
    if (!recovery_info.copy_subset.empty()) {
      ...
      if (r >= 0) {
        ...
        // intersection of copy_subset and empty interval_set would be empty
        // anyway
        copy_subset.clear();
      }
  ...
  if (cct->_conf->osd_debug_random_push_read_error &&
      (rand() % (int)(cct->_conf->osd_debug_random_push_read_error * 100.0)) ==
          0) {
  ...
  if (out_op->data_included.size() != origin_size) {
    dout(10) << __func__ << " some extents get pruned "
             << out_op->data_included.size() << "/" << origin_size << dendl;
  ...
  if (progress.first && !out_op->data_included.empty() &&
    ...
    if (oi.data_digest != crc) {
      dout(0) << __func__ << " " << coll << std::hex
              << " full-object read crc 0x" << crc << " != expected 0x"
              << oi.data_digest << std::dec << " on " << recovery_info.soid
              << dendl;
      return -EIO;
    }
  }
  ...
  if (new_progress.is_complete(recovery_info)) {
    ...
    // If omap is not changed, we need recovery omap when recovery cannot be
    // completed once
  ...
  if (stat) {
    ...
    get_parent()->get_logger()->inc(l_osd_rbytes, out_op->omap_entries.size() +
                                                      out_op->data.length());
  }
...
bool ReplicatedBackend::handle_push_reply(pg_shard_t peer,
                                          const PushReplyOp &op,
                                          PushOp *reply) {
  const hobject_t &soid = op.soid;
  if (pushing.count(soid) == 0) {
    dout(10) << "huh, i wasn't pushing " << soid << " to osd." << peer
             << ", or anybody else" << dendl;
    return false;
    ...
    if (!pi->recovery_progress.data_complete && !error) {
      dout(10) << " pushing more from, "
               << pi->recovery_progress.data_recovered_to << " of "
               << pi->recovery_info.copy_subset << dendl;
      ObjectRecoveryProgress new_progress;
      int r = build_push_op(pi->recovery_info, pi->recovery_progress,
                            &new_progress, reply, &(pi->stat));
      ...
      if (pushing[soid].empty()) {
        if (!error)
          ...
          get_parent()->on_failed_pull(
              std::set<pg_shard_t>{get_parent()->whoami_shard()}, soid, v);
        pushing.erase(soid);
      } else {
...
void ReplicatedBackend::handle_pull(pg_shard_t peer, PullOp &op,
                                    PushOp *reply) {
  const hobject_t &soid = op.soid;
  struct stat st;
  ...
  if (r != 0) {
    get_parent()->clog_error()
        << get_info().pgid << " " << peer << " tried to pull " << soid
...
void ReplicatedBackend::clear_pull(map<hobject_t, PullInfo>::iterator piter,
...
int ReplicatedBackend::start_pushes(const hobject_t &soid, ObjectContextRef obc,
                                    RPGHandle *h) {
  ...
  for (auto j : shards) {
    ...
    int r =
        prep_push_to_replica(obc, soid, peer, &(h->pushes[peer].back()), cache);
    if (r < 0) {
