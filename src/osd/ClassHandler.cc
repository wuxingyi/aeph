...
int ClassHandler::open_all_classes() {
  ...
  while ((pde = ::readdir(dir))) {
    ...
    if (strlen(pde->d_name) > sizeof(CLS_PREFIX) - 1 + sizeof(CLS_SUFFIX) - 1 &&
        ...
        strcmp(pde->d_name + strlen(pde->d_name) - (sizeof(CLS_SUFFIX) - 1),
               CLS_SUFFIX) == 0) {
...
ClassHandler::ClassData *ClassHandler::_get_class(const string &cname,
  ...
  if (iter != classes.end()) {
    ...
    if (check_allowed &&
        !in_class_list(cname, cct->_conf->osd_class_load_list)) {
      ldout(cct, 0) << "_get_class not permitted to load " << cname << dendl;
      return NULL;
    }
    cls = &classes[cname];
    ldout(cct, 10) << "_get_class adding new class name " << cname << " " << cls
                   << dendl;
    cls->name = cname;
    cls->handler = this;
...
int ClassHandler::_load_class(ClassData *cls) {
  ...
  if (cls->status == ClassData::CLASS_UNKNOWN ||
    ...
    snprintf(fname, sizeof(fname), "%s/" CLS_PREFIX "%s" CLS_SUFFIX,
             cct->_conf->osd_class_dir.c_str(), cls->name.c_str());
  ...
  while (p != cls->missing_dependencies.end()) {
    ...
    ldout(cct, 10) << "_load_class " << cls->name << " satisfied dependency "
                   << dc->name << dendl;
...
ClassHandler::ClassData *ClassHandler::register_class(const char *cname) {
  ...
  ldout(cct, 10) << "register_class " << cname << " status " << cls->status
                 << dendl;
...
ClassHandler::ClassData::register_method(const char *mname, int flags,
  ...
  ldout(handler->cct, 10) << "register_method " << name << "." << mname
                          << " flags " << flags << " " << (void *)func << dendl;
  [[maybe_unused]] auto [method, added] =
      methods_map.try_emplace(mname, mname, func, flags, this);
  return &method->second;
}

ClassHandler::ClassMethod *
ClassHandler::ClassData::register_cxx_method(const char *mname, int flags,
                                             cls_method_cxx_call_t func) {
  /* no need for locking, called under the class_init mutex */
  ldout(handler->cct, 10) << "register_cxx_method " << name << "." << mname
                          << " flags " << flags << " " << (void *)func << dendl;
  [[maybe_unused]] auto [method, added] =
      methods_map.try_emplace(mname, mname, func, flags, this);
  return &method->second;
...
ClassHandler::ClassData::register_cxx_filter(const std::string &filter_name,
...
ClassHandler::ClassMethod *
ClassHandler::ClassData::_get_method(const std::string &mname) {
...
void ClassHandler::ClassData::unregister_method(
...
void ClassHandler::ClassMethod::unregister() { cls->unregister_method(this); }

void ClassHandler::ClassData::unregister_filter(
    ClassHandler::ClassFilter *filter) {
  /* no need for locking, called under the class_init mutex */
...
void ClassHandler::ClassFilter::unregister() { cls->unregister_filter(this); }
