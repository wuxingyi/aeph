...
void PGBackend::send_recovery_deletes(
  ...
  for (const auto &p : deletes) {
    ...
    ConnectionRef con =
        get_parent()->get_con_osd_cluster(shard.osd, get_osdmap_epoch());
    if (!con)
      continue;
    ...
    while (it != objects.end()) {
      ...
      MOSDPGRecoveryDelete *msg =
          new MOSDPGRecoveryDelete(get_parent()->whoami_shard(), target_pg,
...
void PGBackend::rollback(const pg_log_entry_t &entry,
...
struct Trimmer : public ObjectModDesc::Visitor {
  ...
  Trimmer(const hobject_t &soid, PGBackend *pg, ObjectStore::Transaction *t)
      : soid(soid), pg(pg), t(t) {}
  void rmobject(version_t old_version) override {
    pg->trim_rollback_object(soid, old_version, t);
  }
...
void PGBackend::rollforward(const pg_log_entry_t &entry,
                            ObjectStore::Transaction *t) {
...
void PGBackend::try_stash(const hobject_t &hoid, version_t v,
                          ObjectStore::Transaction *t) {
  t->try_rename(
      coll,
...
void PGBackend::remove(const hobject_t &hoid, ObjectStore::Transaction *t) {
  ...
  t->remove(coll, ghobject_t(hoid, ghobject_t::NO_GEN,
...
void PGBackend::on_change_cleanup(ObjectStore::Transaction *t) {
  ...
  for (set<hobject_t>::iterator i = temp_contents.begin();
       i != temp_contents.end(); ++i) {
    dout(10) << __func__ << ": Removing oid " << *i
             << " from the temp collection" << dendl;
    t->remove(coll, ghobject_t(*i, ghobject_t::NO_GEN,
                               get_parent()->whoami_shard().shard));
  }
  temp_contents.clear();
}

int PGBackend::objects_list_partial(const hobject_t &begin, int min, int max,
  ...
  while (!_next.is_max() && ls->size() < (unsigned)min) {
    ...
    r = store->collection_list(ch, _next, ghobject_t::get_max(),
                               max - ls->size(), &objects, &_next);
    if (r != 0) {
      derr << __func__ << " list collection " << ch
...
int PGBackend::objects_list_range(const hobject_t &start, const hobject_t &end,
                                  vector<hobject_t> *ls,
...
int PGBackend::objects_get_attr(const hobject_t &hoid, const string &attr,
                                bufferlist *out) {
  bufferptr bp;
  int r = store->getattr(
      ch,
...
int PGBackend::objects_get_attrs(const hobject_t &hoid,
...
void PGBackend::rollback_setattrs(
    const hobject_t &hoid, map<string, std::optional<bufferlist>> &old_attrs,
    ObjectStore::Transaction *t) {
...
void PGBackend::rollback_append(const hobject_t &hoid, uint64_t old_size,
                                ObjectStore::Transaction *t) {
  ceph_assert(!hoid.is_temp());
  t->truncate(
      coll,
...
void PGBackend::rollback_stash(const hobject_t &hoid, version_t old_version,
                               ObjectStore::Transaction *t) {
  ceph_assert(!hoid.is_temp());
  t->remove(coll, ghobject_t(hoid, ghobject_t::NO_GEN,
                             get_parent()->whoami_shard().shard));
...
void PGBackend::rollback_try_stash(const hobject_t &hoid, version_t old_version,
                                   ObjectStore::Transaction *t) {
  ceph_assert(!hoid.is_temp());
  t->remove(coll, ghobject_t(hoid, ghobject_t::NO_GEN,
                             get_parent()->whoami_shard().shard));
...
void PGBackend::rollback_extents(
  ...
  for (auto &&extent : extents) {
    t->clone_range(coll, ghobject_t(hoid, gen, shard),
                   ghobject_t(hoid, ghobject_t::NO_GEN, shard), extent.first,
                   extent.second, extent.first);
  }
  t->remove(coll, ghobject_t(hoid, gen, shard));
}

void PGBackend::trim_rollback_object(const hobject_t &hoid,
...
PGBackend *PGBackend::build_pg_backend(const pg_pool_t &pool,
                                       const map<string, string> &profile,
                                       Listener *l, coll_t coll,
  ...
  case pg_pool_t::TYPE_ERASURE: {
    ...
    ceph::ErasureCodePluginRegistry::instance().factory(
        ...
        cct->_conf.get_val<std::string>("erasure_code_dir"), ec_profile,
        &ec_impl, &ss);
    ceph_assert(ec_impl);
...
int PGBackend::be_scan_list(ScrubMap &map, ScrubMapBuilder &pos) {
  ...
  if (r == 0) {
    ...
    dout(25) << __func__ << "  " << poid << " got " << r << ", skipping"
             << dendl;
  } else if (r == -EIO) {
    dout(25) << __func__ << "  " << poid << " got " << r << ", stat_error"
             << dendl;
    ScrubMap::object &o = map.objects[poid];
    o.stat_error = true;
  } else {
...
bool PGBackend::be_compare_scrub_objects(
  ...
  if (oi_size != candidate.size) {
    if (error != CLEAN)
      errorstream << ", ";
    error = FOUND_ERROR;
    errorstream << "size " << candidate.size << " != size " << oi_size
                << " from auth oi " << auth_oi;
  ...
  if (auth.size != candidate.size) {
    if (error != CLEAN)
      errorstream << ", ";
    error = FOUND_ERROR;
    errorstream << "size " << candidate.size << " != size " << auth.size
                << " from shard " << auth_shard;
  ...
  if (candidate.size > cct->_conf->osd_max_object_size &&
    ...
    errorstream << "size " << candidate.size << " > "
                << cct->_conf->osd_max_object_size << " is too large";
...
map<pg_shard_t, ScrubMap *>::const_iterator PGBackend::be_select_auth_object(
    const hobject_t &obj, const map<pg_shard_t, ScrubMap *> &maps,
    object_info_t *auth_oi, map<pg_shard_t, shard_info_wrapper> &shard_map,
    bool &digest_match, spg_t pgid, ostream &errorstream) {
  ...
  for (auto &l : shards) {
    ...
    if (auth_version != eversion_t() &&
        auth->second->objects[obj].digest_present && i->second.digest_present &&
        auth->second->objects[obj].digest != i->second.digest) {
      digest_match = false;
      dout(10) << __func__ << " digest_match = false, " << obj
               << " data_digest 0x" << std::hex << i->second.digest
               << " != data_digest 0x" << auth->second->objects[obj].digest
               << std::dec << dendl;
    }
  ...
  dout(10) << __func__ << ": selecting osd " << auth->first << " for obj "
           << obj << " with oi " << *auth_oi << dendl;
  return auth;
}

void PGBackend::be_compare_scrubmaps(
    const map<pg_shard_t, ScrubMap *> &maps, const set<hobject_t> &master_set,
    bool repair, map<hobject_t, set<pg_shard_t>> &missing,
    map<hobject_t, set<pg_shard_t>> &inconsistent,
    map<hobject_t, list<pg_shard_t>> &authoritative,
    map<hobject_t, pair<std::optional<uint32_t>, std::optional<uint32_t>>>
        &missing_digest,
    int &shallow_errors, int &deep_errors, Scrub::Store *store,
    const spg_t &pgid, const vector<int> &acting, ostream &errorstream) {
  ...
  for (set<hobject_t>::const_iterator k = master_set.begin();
    ...
    bool digest_match;
    map<pg_shard_t, ScrubMap *>::const_iterator auth = be_select_auth_object(
        *k, maps, &auth_oi, shard_map, digest_match, pgid, errorstream);
    ...
    for (auto j = maps.cbegin(); j != maps.cend(); ++j) {
      ...
      if (j->second->objects.count(*k)) {
        ...
        // Compare
        stringstream ss;
        bool found = be_compare_scrub_objects(
            auth->first, auth_object, auth_oi, j->second->objects[*k],
            shard_map[j->first], object_error, ss, k->has_snapset());
        ...
        if (shard_map[j->first].errors != 0) {
          ...
          if (shard_map[j->first].has_deep_errors())
            ...
            ++shallow_errors;
          // Only true if be_compare_scrub_objects() found errors and put
          // something in ss.
          if (found)
        ...
        shard_map[j->first].primary =
            (j->first == get_parent()->whoami_shard());
        // Can't have any other errors if there is no information available
        ++shallow_errors;
        errorstream << pgid << " shard " << j->first << " " << *k
                    << " : missing\n";
      }
    ...
    if (!cur_inconsistent.empty() || !cur_missing.empty()) {
      ...
      if (auth_oi.is_data_digest() && auth_object.digest_present &&
        ...
        errorstream << pgid << " recorded data digest 0x" << std::hex
                    << auth_oi.data_digest << " != on disk 0x"
      ...
      if (auth_oi.is_omap_digest() && auth_object.omap_digest_present &&
        ...
        errorstream << pgid << " recorded omap digest 0x" << std::hex
                    << auth_oi.omap_digest << " != on disk 0x"
                    << auth_object.omap_digest << std::dec << " on "
                    << auth_oi.soid << "\n";
        if (repair)
          update = FORCE;
      }
      ...
      if (update != NO) {
        ...
        if (update == FORCE ||
          ...
          if (auth_object.digest_present) {
            data_digest = auth_object.digest;
            dout(20) << __func__ << " will update data digest on " << *k
                     << dendl;
          }
          if (auth_object.omap_digest_present) {
            omap_digest = auth_object.omap_digest;
            dout(20) << __func__ << " will update omap digest on " << *k
                     << dendl;
          }
          missing_digest[*k] = make_pair(data_digest, omap_digest);
        } else {
          dout(20) << __func__ << " missing digest but age " << age << " < "
                   << cct->_conf->osd_deep_scrub_update_digest_min_age << " on "
                   << *k << dendl;
        }
      }
    }
