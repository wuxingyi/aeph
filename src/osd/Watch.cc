...
Notify::Notify(ConnectionRef client, uint64_t client_gid, bufferlist &payload,
               uint32_t timeout, uint64_t cookie, uint64_t notify_id,
               uint64_t version, OSDService *osd)
    : client(client), client_gid(client_gid), complete(false), discarded(false),
      timed_out(false), payload(payload), timeout(timeout), cookie(cookie),
      notify_id(notify_id), version(version), osd(osd), cb(NULL) {}

NotifyRef Notify::makeNotifyRef(ConnectionRef client, uint64_t client_gid,
                                bufferlist &payload, uint32_t timeout,
                                uint64_t cookie, uint64_t notify_id,
                                uint64_t version, OSDService *osd) {
  NotifyRef ret(new Notify(client, client_gid, payload, timeout, cookie,
                           notify_id, version, osd));
  ret->set_self(ret);
  return ret;
}
...
void Notify::do_timeout() {
  ...
  for (set<WatchRef>::iterator i = _watchers.begin(); i != _watchers.end();
       ++i) {
...
void Notify::complete_watcher(WatchRef watch, bufferlist &reply_bl) {
  ...
  notify_replies.insert(make_pair(
      make_pair(watch->get_watcher_gid(), watch->get_cookie()), reply_bl));
  maybe_complete_notify();
}
...
void Notify::maybe_complete_notify() {
  dout(10) << "maybe_complete_notify -- " << watchers.size()
  ...
  if (watchers.empty() || timed_out) {
    ...
    for (set<WatchRef>::iterator p = watchers.begin(); p != watchers.end();
         ++p) {
    ...
    MWatchNotify *reply(new MWatchNotify(
        cookie, version, notify_id, CEPH_WATCH_EVENT_NOTIFY_COMPLETE, empty));
...
std::ostream &Watch::gen_dbg_prefix(std::ostream &out) {
  return pg->gen_prefix(out)
         << " -- Watch(" << make_pair(cookie, entity) << ") ";
}

Watch::Watch(PrimaryLogPG *pg, OSDService *osd, ObjectContextRef obc,
             uint32_t timeout, uint64_t cookie, entity_name_t entity,
             const entity_addr_t &addr)
    : cb(NULL), osd(osd), pg(pg), obc(obc), timeout(timeout), cookie(cookie),
      addr(addr), will_ping(false), entity(entity), discarded(false) {
  dout(10) << "Watch()" << dendl;
}
...
bool Watch::is_discarded() const { return discarded; }
...
void Watch::remove(bool send_disconnect) {
  ...
  if (send_disconnect && conn) {
    bufferlist empty;
    MWatchNotify *reply(
        new MWatchNotify(cookie, 0, 0, CEPH_WATCH_EVENT_DISCONNECT, empty));
...
void Watch::start_notify(NotifyRef notif) {
  ...
  if (will_ping) {
    ...
    if (last_ping < cutoff) {
      dout(10) << __func__ << " " << notif->notify_id << " last_ping "
               << last_ping << " < cutoff " << cutoff << ", disconnecting"
               << dendl;
      disconnect();
      return;
    }
...
void Watch::send_notify(NotifyRef notif) {
  ...
  MWatchNotify *notify_msg =
      new MWatchNotify(cookie, notif->version, notif->notify_id,
...
WatchRef Watch::makeWatchRef(PrimaryLogPG *pg, OSDService *osd,
                             ObjectContextRef obc, uint32_t timeout,
  ...
  for (set<WatchRef>::iterator i = _watches.begin(); i != _watches.end(); ++i) {
    ...
    if (!(*i)->is_discarded()) {
      if ((*i)->is_connected(con)) {
        ...
        lgeneric_derr(cct) << __func__ << " not still connected to " << (*i)
                           << dendl;
      }
    }
    pg->unlock();
