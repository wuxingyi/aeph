...
void encode_and_write(pg_t pgid, const hobject_t &oid,
                      const ECUtil::stripe_info_t &sinfo,
                      ErasureCodeInterfaceRef &ecimpl, const set<int> &want,
                      uint64_t offset, bufferlist bl, uint32_t flags,
                      ECUtil::HashInfoRef hinfo, extent_map &written,
  ...
  if (offset >= before_size) {
    ...
    hinfo->append(sinfo.aligned_logical_offset_to_chunk_offset(offset),
                  buffers);
  }
  ...
  for (auto &&i : *transactions) {
    ...
    if (offset >= before_size) {
      i.second.set_alloc_hint(coll_t(spg_t(pgid, i.first)),
                              ghobject_t(oid, ghobject_t::NO_GEN, i.first), 0,
                              0,
...
void ECTransaction::generate_transactions(
  ...
  t.safe_create_traverse([&](pair<const hobject_t,
                                  PGTransaction::ObjectOperation> &opair) {
    ...
    ldpp_dout(dpp, 20) << "generate_transactions: " << opair.first
                       << ", current size is "
                       << hinfo->get_total_logical_size(sinfo)
                       << " buffers are " << op.buffer_updates << dendl;
    if (op.truncate) {
    ...
    if (op.delete_first) {
      ...
      if (entry) {
        ...
        for (auto &&st : *transactions) {
          st.second.remove(coll_t(spg_t(pgid, st.first)),
                           ghobject_t(oid, ghobject_t::NO_GEN, st.first));
        }
      }
      hinfo->clear();
        ...
        [&](const PGTransaction::ObjectOperation::Init::Create &op) {
          for (auto &&st : *transactions) {
            if (require_osd_release >= ceph_release_t::octopus) {
              st.second.create(coll_t(spg_t(pgid, st.first)),
                               ghobject_t(oid, ghobject_t::NO_GEN, st.first));
            } else {
    ...
    if (!op.attr_updates.empty()) {
      ...
      for (auto &&j : op.attr_updates) {
        if (j.second) {
          ...
          for (auto &&st : *transactions) {
            st.second.rmattr(coll_t(spg_t(pgid, st.first)),
                             ghobject_t(oid, ghobject_t::NO_GEN, st.first),
                             j.first);
          }
        }
          ...
          if (entry) {
            if (citer != obc->attr_cache.end()) {
              ...
              xattr_rollback.insert(
                  make_pair(j.first, std::optional<bufferlist>(citer->second)));
            } else {
              // won't overwrite anything we put in earlier
      ...
      for (auto &&st : *transactions) {
        st.second.setattrs(coll_t(spg_t(pgid, st.first)),
                           ghobject_t(oid, ghobject_t::NO_GEN, st.first),
                           to_set);
      }
    ...
    if (op.alloc_hint) {
      ...
      for (auto &&st : *transactions) {
        st.second.set_alloc_hint(coll_t(spg_t(pgid, st.first)),
                                 ghobject_t(oid, ghobject_t::NO_GEN, st.first),
                                 object_size, write_size, op.alloc_hint->flags);
      }
    }
    ...
    if (op.truncate && op.truncate->first < new_size) {
      ...
      ldpp_dout(dpp, 20) << __func__ << ": new_size truncate down " << new_size
                         << dendl;
      ...
      if (entry && !op.is_fresh_object()) {
        uint64_t restore_from =
            sinfo.logical_to_prev_chunk_offset(op.truncate->first);
        ...
        ldpp_dout(dpp, 20) << __func__ << ": saving extent "
                           << make_pair(restore_from, restore_len) << dendl;
        ldpp_dout(dpp, 20) << __func__ << ": truncating to " << new_size
                           << dendl;
        ...
        for (auto &&st : *transactions) {
          ...
          st.second.clone_range(
              coll_t(spg_t(pgid, st.first)),
              ghobject_t(oid, ghobject_t::NO_GEN, st.first),
              ghobject_t(oid, entry->version.version, st.first), restore_from,
              restore_len, restore_from);
        }
      } else {
    ...
    for (auto &&extent : op.buffer_updates) {
      ...
      match(
          ...
          [&](const BufferUpdate::CloneRange &) {
            ceph_assert(0 == "CloneRange is not allowed, do_op should have "
                             "returned ENOTSUPP");
          });
      ...
      ldpp_dout(dpp, 20) << __func__ << ": adding buffer_update "
                         << make_pair(off, len) << dendl;
      ceph_assert(len > 0);
      if (off > new_size) {
        ceph_assert(off > append_after);
        bl.prepend_zero(off - new_size);
        len += off - new_size;
        ldpp_dout(dpp, 20) << __func__ << ": prepending zeroes to align " << off
                           << "->" << new_size << dendl;
        off = new_size;
      }
      if (!sinfo.logical_offset_is_stripe_aligned(end) &&
          (end > append_after)) {
    ...
    if (op.truncate && op.truncate->second > new_size) {
      ...
      ldpp_dout(dpp, 20) << __func__ << ": truncating out to " << truncate_to
                         << dendl;
    }
    ...
    ldpp_dout(dpp, 20) << __func__ << ": to_overwrite: " << to_overwrite
                       << dendl;
    for (auto &&extent : to_overwrite) {
      ...
      if (entry) {
        uint64_t restore_from =
            sinfo.aligned_logical_offset_to_chunk_offset(extent.get_off());
        uint64_t restore_len =
            sinfo.aligned_logical_offset_to_chunk_offset(extent.get_len());
        ldpp_dout(dpp, 20) << __func__ << ": overwriting " << restore_from
                           << "~" << restore_len << dendl;
      ...
      encode_and_write(pgid, oid, sinfo, ecimpl, want, extent.get_off(),
                       extent.get_val(), fadvise_flags, hinfo, written,
                       transactions, dpp);
    }

    auto to_append = to_write.intersect(
        append_after, std::numeric_limits<uint64_t>::max() - append_after);
    ldpp_dout(dpp, 20) << __func__ << ": to_append: " << to_append << dendl;
    for (auto &&extent : to_append) {
      ceph_assert(sinfo.logical_offset_is_stripe_aligned(extent.get_off()));
      ceph_assert(sinfo.logical_offset_is_stripe_aligned(extent.get_len()));
      ldpp_dout(dpp, 20) << __func__ << ": appending " << extent.get_off()
                         << "~" << extent.get_len() << dendl;
      encode_and_write(pgid, oid, sinfo, ecimpl, want, extent.get_off(),
                       extent.get_val(), fadvise_flags, hinfo, written,
                       transactions, dpp);
    }
    ...
    ldpp_dout(dpp, 20) << __func__ << ": " << oid
                       << " resetting hinfo to logical size " << new_size
                       << dendl;
    if (!rollback_extents.empty() && entry) {
      if (entry) {
        ldpp_dout(dpp, 20) << __func__ << ": " << oid
                           ...
                           << dendl;
        entry->mod_desc.rollback_extents(entry->version.version,
                                         rollback_extents);
      }
