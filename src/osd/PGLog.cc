...
void PGLog::IndexedLog::split_out_child(pg_t child_pgid, unsigned split_bits,
...
void PGLog::IndexedLog::trim(CephContext *cct, eversion_t s,
  ...
  if (complete_to != log.end())
    lgeneric_subdout(cct, osd, 20)
        << " complete_to " << complete_to->version << dendl;

  auto earliest_dup_version =
      log.rbegin()->version.version < cct->_conf->osd_pg_log_dups_tracked
          ? 0u
          : log.rbegin()->version.version -
                cct->_conf->osd_pg_log_dups_tracked + 1;

  lgeneric_subdout(cct, osd, 20)
      << "earliest_dup_version = " << earliest_dup_version << dendl;
  while (!log.empty()) {
    ...
    if (e.version.version >= earliest_dup_version) {
      if (write_from_dups != nullptr && *write_from_dups > e.version) {
        lgeneric_subdout(cct, osd, 20)
            << "updating write_from_dups from " << *write_from_dups << " to "
            << e.version << dendl;
      ...
      for (const auto &extra : e.extra_reqids) {
        ...
        dups.push_back(
            pg_log_dup_t(e.version, extra.second, extra.first, return_code));
        index(dups.back());
      }
    }
    ...
    if (reset_complete_to) {
      ...
      if (complete_to != log.end()) {
        lgeneric_subdout(cct, osd, 20)
            << " moving complete_to to " << log.begin()->version << dendl;
      } else {
...
ostream &PGLog::IndexedLog::print(ostream &out) const {
  out << *this << std::endl;
  for (list<pg_log_entry_t>::const_iterator p = log.begin(); p != log.end();
       ++p) {
    out << *p << " " << (logged_object(p->soid) ? "indexed" : "NOT INDEXED")
        << std::endl;
    ceph_assert(!p->reqid_is_indexed() || logged_req(p->reqid));
  }

  for (list<pg_log_dup_t>::const_iterator p = dups.begin(); p != dups.end();
       ++p) {
    out << *p << std::endl;
  }
...
void PGLog::reset_backfill() { missing.clear(); }
...
void PGLog::trim(eversion_t trim_to, pg_info_t &info, bool transaction_applied,
                 bool async) {
  ...
  if (trim_to > log.tail) {
    ...
    if (log.complete_to != log.log.end())
      dout(10) << " after trim complete_to " << log.complete_to->version
               << dendl;
  }
}

void PGLog::proc_replica_log(pg_info_t &oinfo, const pg_log_t &olog,
                             pg_missing_t &omissing, pg_shard_t from) const {
  dout(10) << "proc_replica_log for osd." << from << ": " << oinfo << " "
           << olog << " " << omissing << dendl;

  if (olog.head < log.tail) {
    dout(10) << __func__ << ": osd." << from
  ...
  */
   ...
   * the event represented by log.tail.  Similarly, because log.head >=
   * olog.tail, we know that the even represented by olog.tail must be common to
   * both logs. Furthermore, the event represented by a log tail was necessarily
   * trimmed, thus neither olog.tail nor log.tail can be divergent. It's
  ...
  eversion_t lu = (first_non_divergent == log.log.rend() ||
                   first_non_divergent->version < limit)
                      ? limit
                      : first_non_divergent->version;

  // we merge and adjust the replica's log, rollback the rollbackable divergent
  // entry, remove the unrollbackable divergent entry and mark the according
  // object as missing. the rollback boundary must choose crt of the olog which
  // going to be merged. The replica log's(olog) crt will not be modified, so it
  // could get passed to _merge_divergent_entries() directly.
  IndexedLog folog(olog);
  auto divergent = folog.rewind_from_head(lu);
  _merge_divergent_entries(folog, divergent, oinfo, olog.get_can_rollback_to(),
                           omissing, 0, this);
 ...
 */
void PGLog::rewind_divergent_log(eversion_t newhead, pg_info_t &info,
                                 LogEntryHandler *rollbacker, bool &dirty_info,
                                 bool &dirty_big_info) {
  dout(10) << "rewind_divergent_log truncate divergent future " << newhead
           << dendl;

  // We need to preserve the original crt before it gets updated in
  // rewind_from_head(). Later, in merge_object_divergent_entries(), we use it
  // to check whether we can rollback a divergent entry or not.
  ...
  _merge_divergent_entries(log, divergent, info, original_crt, missing,
                           rollbacker, this);
...
void PGLog::merge_log(pg_info_t &oinfo, pg_log_t &olog, pg_shard_t fromosd,
  ...
  dout(10) << "merge_log " << olog << " from osd." << fromosd << " into " << log
           << dendl;
  ...
  if (olog.head < log.head) {
    rewind_divergent_log(olog.head, info, rollbacker, dirty_info,
                         dirty_big_info);
    changed = true;
  }
  ...
  if (olog.head > log.head) {
    ...
    dout(20) << "merge_log cut point (usually last shared) is " << lower_bound
             << dendl;
    ...
    append_log_entries_update_missing(info.last_backfill, new_entries, false,
                                      &log, missing, rollbacker, this);
...
bool PGLog::merge_log_dups(const pg_log_t &olog) {
    ...
    if (log.dups.empty()) {
      ...
      if (olog.dups.front().version < log.dups.front().version) {
        ...
        for (auto i = olog.dups.cbegin(); i != olog.dups.cend(); ++i) {
          if (i->version >= insert_cursor->version)
            break;
          log.dups.insert(insert_cursor, *i);
          last = i->version;
          auto prev = insert_cursor;
  ...
  if (!log.dups.empty() && log.dups.back().version > log.tail) {
    dout(10) << "merge_log removed dups overlapping log entries (" << log.tail
             << "," << log.dups.back().version << "]" << dendl;
    changed = true;
...
void PGLog::check() {
  ...
  if (log.log.size() != log_keys_debug.size()) {
    ...
    for (list<pg_log_entry_t>::iterator i = log.log.begin(); i != log.log.end();
         ++i) {
      derr << "    " << *i << dendl;
    }
    derr << "log_keys_debug:" << dendl;
    for (set<string>::const_iterator i = log_keys_debug.begin();
         i != log_keys_debug.end(); ++i) {
      derr << "    " << *i << dendl;
    }
  }
...
// non-static
void PGLog::write_log_and_missing(ObjectStore::Transaction &t,
                                  map<string, bufferlist> *km,
                                  const coll_t &coll, const ghobject_t &log_oid,
                                  bool require_rollback) {
  if (needs_write()) {
    dout(6) << "write_log_and_missing with: "
            << "dirty_to: " << dirty_to << ", dirty_from: " << dirty_from
            << ", writeout_from: " << writeout_from << ", trimmed: " << trimmed
            << ", trimmed_dups: " << trimmed_dups
            << ", clear_divergent_priors: " << clear_divergent_priors << dendl;
    _write_log_and_missing(
        t, km, log, coll, log_oid, dirty_to, dirty_from, writeout_from,
        std::move(trimmed), std::move(trimmed_dups), missing, !touched_log,
        require_rollback, clear_divergent_priors, dirty_to_dups,
        dirty_from_dups, write_from_dups, &may_include_deletes_in_missing_dirty,
...
void PGLog::write_log_and_missing_wo_missing(
    ObjectStore::Transaction &t, map<string, bufferlist> *km, pg_log_t &log,
    const coll_t &coll, const ghobject_t &log_oid,
    map<eversion_t, hobject_t> &divergent_priors, bool require_rollback) {
  _write_log_and_missing_wo_missing(
      t, km, log, coll, log_oid, divergent_priors, eversion_t::max(),
      eversion_t(), eversion_t(), true, true, require_rollback,
      eversion_t::max(), eversion_t(), eversion_t(), nullptr);
}

// static
void PGLog::write_log_and_missing(ObjectStore::Transaction &t,
                                  map<string, bufferlist> *km, pg_log_t &log,
                                  const coll_t &coll, const ghobject_t &log_oid,
                                  const pg_missing_tracker_t &missing,
                                  bool require_rollback,
                                  bool *may_include_deletes_in_missing_dirty) {
  _write_log_and_missing(t, km, log, coll, log_oid, eversion_t::max(),
                         eversion_t(), eversion_t(), set<eversion_t>(),
                         set<string>(), missing, true, require_rollback, false,
...
void PGLog::_write_log_and_missing_wo_missing(
  ...
  if (dirty_to != eversion_t()) {
    t.omap_rmkeyrange(coll, log_oid, eversion_t().get_key_name(),
  ...
  if (dirty_to != eversion_t::max() && dirty_from != eversion_t::max()) {
    // dout(10) << "write_log_and_missing, clearing from " << dirty_from <<
    // dendl;
    t.omap_rmkeyrange(coll, log_oid, dirty_from.get_key_name(),
                      eversion_t::max().get_key_name());
    clear_after(log_keys_debug, dirty_from.get_key_name());
  }

  for (list<pg_log_entry_t>::iterator p = log.log.begin();
       p != log.log.end() && p->version <= dirty_to; ++p) {
  ...
  if (log_keys_debug) {
    for (map<string, bufferlist>::iterator i = (*km).begin(); i != (*km).end();
         ++i) {
      if (i->first[0] == '_')
        continue;
  ...
  if (dirty_to_dups != eversion_t()) {
    ...
    t.omap_rmkeyrange(coll, log_oid, min.get_key_name(),
                      dirty_to_dup.get_key_name());
  }
  if (dirty_to_dups != eversion_t::max() &&
    ...
    t.omap_rmkeyrange(coll, log_oid, dirty_from_dup.get_key_name(),
                      max.get_key_name());
  }
  ...
  if (require_rollback) {
    ...
    encode(log.get_rollback_info_trimmed_to(),
...
void PGLog::_write_log_and_missing(
    ...
    set<string> &&trimmed_dups, const pg_missing_tracker_t &missing,
    bool touch_log, bool require_rollback, bool clear_divergent_priors,
    eversion_t dirty_to_dups, eversion_t dirty_from_dups,
    eversion_t write_from_dups,
  ...
  missing.get_changed([&](const hobject_t &obj) {
    ...
    if (!missing.is_missing(obj, &item)) {
      ...
      uint64_t features =
          missing.may_include_deletes ? CEPH_FEATURE_OSD_RECOVERY_DELETES : 0;
...
void PGLog::rebuild_missing_set_with_deletes(ObjectStore *store,
  ...
  for (list<pg_log_entry_t>::reverse_iterator i = log.log.rbegin();
    ...
    bufferlist bv;
    int r = store->getattr(
        ch, ghobject_t(i->soid, ghobject_t::NO_GEN, info.pgid.shard), OI_ATTR,
        bv);
    dout(20) << __func__ << " check for log entry: " << *i << " = " << r
             << dendl;
