...
ostream &operator<<(ostream &out, const osd_info_t &info) {
  out << "up_from " << info.up_from << " up_thru " << info.up_thru
      << " down_at " << info.down_at << " last_clean_interval ["
...
ostream &operator<<(ostream &out, const osd_xinfo_t &xi) {
  return out << "down_stamp " << xi.down_stamp << " laggy_probability "
             << xi.laggy_probability << " laggy_interval " << xi.laggy_interval
             << " old_weight " << xi.old_weight << " last_purged_snaps_scrub "
    ...
    if (!new_pool.second.tiers.empty()) {
      ...
      for (const auto &tier_pool : base.tiers) {
        ...
        if (tier->tier_of != new_pool.first) {
          lderr(cct) << __func__ << " " << r->first
                     << " tier_of != " << new_pool.first << dendl;
          return -EIO;
        }
...
bool OSDMap::containing_subtree_is_down(CephContext *cct, int id,
                                        int subtree_type,
                                        set<int> *down_cache) const {
  ...
  while (true) {
    ...
    if (!subtree_is_down(current, down_cache)) {
      ldout(cct, 30) << "containing_subtree_is_down(" << id << ") = false"
                     << dendl;
      return false;
    }
    ...
    if (type >= subtree_type) {
      ldout(cct, 30) << "containing_subtree_is_down(" << id << ") = true ... "
                     << type << " >= " << subtree_type << dendl;
      return true;
    }
...
bool OSDMap::subtree_type_is_down(
    CephContext *cct, int id, int subtree_type, set<int> *down_in_osds,
  ...
  crush->get_children(id, &children);
  for (const auto &child : children) {
    if (!subtree_type_is_down(cct, child, crush->get_bucket_type(child),
                              down_in_osds, up_in_osds, subtree_up,
                              subtree_type_down)) {
...
void OSDMap::Incremental::encode_classic(ceph::buffer::list &bl,
...
template <class T>
static void encode_addrvec_map_as_addr(const T &m, ceph::buffer::list &bl,
                                       uint64_t f) {
  uint32_t n = m.size();
  encode(n, bl);
  for (auto &i : m) {
...
template <class T>
static void encode_addrvec_pvec_as_addr(const T &m, ceph::buffer::list &bl,
                                        uint64_t f) {
  uint32_t n = m.size();
  encode(n, bl);
  for (auto &i : m) {
 ...
 */
void OSDMap::Incremental::encode(ceph::buffer::list &bl,
                                 uint64_t features) const {
  using ceph::encode;
...
void OSDMap::Incremental::decode_classic(
...
void OSDMap::Incremental::dump(Formatter *f) const {
  ...
  f->dump_int("new_require_min_compat_client",
              ceph::to_integer<int>(new_require_min_compat_client));
  f->dump_int("new_require_osd_release",
              ceph::to_integer<int>(new_require_osd_release));
...
void OSDMap::get_full_pools(CephContext *cct, set<int64_t> *full,
...
void OSDMap::adjust_osd_weights(const map<int, double> &weights,
                                Incremental &inc) const {
  float max = 0;
  ...
  for (const auto &weight : weights) {
    inc.new_weight[weight.first] =
        (unsigned)((weight.second / max) * CEPH_OSD_IN);
  }
}
...
int OSDMap::identify_osd(const entity_addr_t &addr) const {
  for (int i = 0; i < max_osd; i++)
    if (exists(i) &&
        (get_addrs(i).contains(addr) || get_cluster_addrs(i).contains(addr)))
      return i;
  return -1;
}
...
uint64_t OSDMap::get_features(int entity_type, uint64_t *pmask) const {
  ...
  for (auto &pool : pools) {
    ...
    int ruleid =
        crush->find_rule(pool.second.get_crush_rule(), pool.second.get_type(),
  ...
  if (entity_type == CEPH_ENTITY_TYPE_OSD) {
    ...
    const uint64_t kraken_features =
        CEPH_FEATUREMASK_SERVER_KRAKEN | CEPH_FEATURE_MSG_ADDR2;
...
uint64_t OSDMap::get_up_osd_features() const { return cached_up_osd_features; }
...
void OSDMap::clean_temps(CephContext *cct, const OSDMap &oldmap,
                         const OSDMap &nextmap, Incremental *pending_inc) {
  ...
  for (auto &pg : *nextmap.primary_temp) {
    ...
    if (real_primary == templess_primary) {
      ldout(cct, 10) << __func__ << "  removing primary_temp " << pgid << " -> "
                     << real_primary << " (unnecessary/redundant)" << dendl;
...
bool OSDMap::check_pg_upmaps(
    CephContext *cct, const vector<pg_t> &to_check, vector<pg_t> *to_cancel,
  ...
  for (auto &pg : to_check) {
    ...
    if (r < 0) {
      ldout(cct, 0) << __func__ << " verify_upmap of pg " << pg << " returning "
                    << r << dendl;
      to_cancel->push_back(pg);
      continue;
    }
    ...
    ldout(cct, 10) << __func__ << " pg " << pg << " weight_map " << weight_map
                   << dendl;
    for (auto osd : up) {
...
void OSDMap::clean_pg_upmaps(
    CephContext *cct, Incremental *pending_inc, const vector<pg_t> &to_cancel,
  ...
  for (auto &pg : to_cancel) {
    ...
    if (i != pending_inc->new_pg_upmap.end()) {
      ldout(cct, 10) << __func__ << " cancel invalid pending "
                     << "pg_upmap entry " << i->first << "->" << i->second
                     << dendl;
    ...
    if (q != pg_upmap_items.end()) {
      ldout(cct, 10) << __func__ << " cancel invalid "
                     << "pg_upmap_items entry " << q->first << "->" << q->second
                     << dendl;
...
// mapping
int OSDMap::map_to_pg(int64_t poolid, const string &name, const string &key,
                      const string &nspace, pg_t *pg) const {
...
int OSDMap::object_locator_to_pg(const object_t &oid,
                                 const object_locator_t &loc, pg_t &pg) const {
  if (loc.hash >= 0) {
...
ceph_object_layout OSDMap::make_object_layout(object_t oid, int pg_pool,
                                              string nspace) const {
...
void OSDMap::_pg_to_raw_osds(const pg_pool_t &pool, pg_t pg, vector<int> *osds,
                             ps_t *ppps) const {
  // map to osds[]
...
void OSDMap::_apply_upmap(const pg_pool_t &pi, pg_t raw_pg,
                          vector<int> *raw) const {
...
void OSDMap::_apply_primary_affinity(ps_t seed, const pg_pool_t &pool,
  ...
  for (unsigned i = 0; i < osds->size(); ++i) {
    ...
    if (a < CEPH_OSD_MAX_PRIMARY_AFFINITY &&
        (crush_hash32_2(CRUSH_HASH_RJENKINS1, seed, o) >> 16) >= a) {
...
void OSDMap::_pg_to_up_acting_osds(const pg_t &pg, vector<int> *up,
                                   int *up_primary, vector<int> *acting,
                                   int *acting_primary,
...
bool OSDMap::primary_changed(int oldprimary, const vector<int> &oldacting,
                             int newprimary, const vector<int> &newacting) {
...
uint64_t OSDMap::get_encoding_features() const {
  ...
  if (require_osd_release < ceph_release_t::jewel) {
    f &= ~(CEPH_FEATURE_SERVER_JEWEL | CEPH_FEATURE_NEW_OSDOP_ENCODING |
...
void OSDMap::dump(Formatter *f) const {
  ...
  f->dump_string("min_compat_client", ceph::to_string(get_min_compat_client()));
  f->dump_string("require_osd_release", ceph::to_string(require_osd_release));
...
string OSDMap::get_flag_string() const { return get_flag_string(flags); }
...
void OSDMap::print_pools(ostream &out) const {
  for (const auto &pool : pools) {
    ...
    for (const auto &snap : pool.second.snaps)
      out << "\tsnap " << snap.second.snapid << " '" << snap.second.name << "' "
          << snap.second.stamp << "\n";
    ...
    *tbl << name.str();
    ...
    if (!qi.is_bucket()) {
      if (!osdmap->exists(qi.id)) {
        ...
        *tbl << s << weightf_t(osdmap->get_weightf(qi.id))
...
protected:
  void dump_item_fields(const CrushTreeDumper::Item &qi,
...
void OSDMap::print_tree(Formatter *f, ostream *out, unsigned filter,
                        string bucket) const {
  if (f) {
...
void OSDMap::print_summary(Formatter *f, ostream &out, const string &prefix,
                           bool extra) const {
  if (f) {
    f->dump_int("epoch", get_epoch());
...
void OSDMap::print_oneline_summary(ostream &out) const {
  out << "e" << get_epoch() << ": " << get_num_osds() << " total, "
      << get_num_up_osds() << " up, " << get_num_in_osds() << " in";
}
  ...
  for (auto &i : pools) {
    ...
    if (poolsize < newcrush->get_rule_mask_min_size(ruleno) ||
      ...
      *ss << "pool " << i.first << " size " << poolsize << " does not"
          << " fall within rule " << ruleno << " min_size "
          << newcrush->get_rule_mask_min_size(ruleno) << " and max_size "
          << newcrush->get_rule_mask_max_size(ruleno);
      return -EINVAL;
    }
  }
  return 0;
...
int OSDMap::build_simple_optioned(CephContext *cct, epoch_t e, uuid_d &fsid,
  ...
  if (nosd >= 0) {
    ...
    for (auto &section : sections) {
      ...
      if (o > cct->_conf->mon_max_osd) {
        lderr(cct) << "[osd." << o << "] in config has id > mon_max_osd "
                   << cct->_conf->mon_max_osd << dendl;
        return -ERANGE;
      }
  ...
  const int default_replicated_rule =
  ...
  if (default_pool) {
    ...
    for (auto &plname : pool_names) {
      ...
      pools[pool].min_size =
          cct->_conf.get_osd_pool_default_min_size(pools[pool].size);
...
int OSDMap::get_erasure_code_profile_default(CephContext *cct,
                                             map<string, string> &profile_map,
                                             ostream *ss) {
  int r = get_json_str_map(
      cct->_conf.get_val<string>("osd_pool_default_erasure_code_profile"), *ss,
      &profile_map);
  return r;
}
...
int OSDMap::build_simple_crush_map(CephContext *cct, CrushWrapper &crush,
  ...
  int rootid;
  int r = crush.add_bucket(0, 0, CRUSH_HASH_DEFAULT, root_type, 0, NULL, NULL,
                           &rootid);
  ceph_assert(r == 0);
...
int OSDMap::build_simple_crush_rules(CephContext *cct, CrushWrapper &crush,
                                     const string &root, ostream *ss) {
  ...
  int r;
  r = crush.add_simple_rule_at("replicated_rule", root, failure_domain, "",
                               "firstn", pg_pool_t::TYPE_REPLICATED, crush_rule,
                               ss);
  if (r < 0)
    return r;
...
int OSDMap::summarize_mapping_stats(OSDMap *newmap, const set<int64_t> *pools,
                                    std::string *out, Formatter *f) const {
  set<int64_t> ls;
  if (pools) {
    ls = *pools;
  } else {
    ...
    vector<int> *out) ///< resulting alternative mapping
{
  ...
  int r = crush->try_remap_rule(cct, rule, pool->get_size(), overfull,
                                underfull, more_underfull, *orig, out);
  if (r < 0)
    return false;
  if (*out == *orig)
    return false;
...
int OSDMap::calc_pg_upmaps(CephContext *cct, uint32_t max_deviation, int max,
  ...
  for (auto &i : pools) {
    ...
    int ruleno = tmp.crush->find_rule(i.second.get_crush_rule(),
                                      i.second.get_type(), i.second.get_size());
    tmp.crush->get_rule_weight_osd_map(ruleno, &pmap);
    ldout(cct, 20) << __func__ << " pool " << i.first << " ruleno " << ruleno
                   << " weight-map " << pmap << dendl;
    for (auto p : pmap) {
  ...
  for (auto &i : osd_weight) {
    ...
    ldout(cct, 20) << " osd." << i.first << " weight " << i.second << " pgs "
                   << pgs << dendl;
  }
  ...
  for (auto &i : pgs_by_osd) {
    ...
    ldout(cct, 20) << " osd." << i.first << "\tpgs " << i.second.size()
  ...
  ldout(cct, 20) << " stdev " << stddev << " max_deviation "
                 << cur_max_deviation << dendl;
  ...
  while (max--) {
    ...
    for (auto i = deviation_osd.rbegin(); i != deviation_osd.rend(); i++) {
      ldout(cct, 30) << " check " << i->first << " <= " << max_deviation
                     << dendl;
      if (i->first <= 0)
        break;
    ...
    for (auto i = deviation_osd.begin(); i != deviation_osd.end(); i++) {
      ldout(cct, 30) << " check " << i->first << " >= " << -(int)max_deviation
                     << dendl;
      if (i->first >= 0)
        break;
    ...
    if (underfull.empty() && overfull.empty()) {
      ldout(cct, 20) << __func__ << " failed to build overfull and underfull"
                     << dendl;
      break;
    }
    if (overfull.empty() && !underfull.empty()) {
      ldout(cct, 20) << __func__
                     << " Using more_overfull since we still have underfull"
                     << dendl;
      overfull = more_overfull;
      using_more_overfull = true;
    }

    ldout(cct, 10) << " overfull " << overfull << " underfull " << underfull
                   << dendl;
    set<pg_t> to_skip;
  ...
  retry:
    ...
    for (auto p = deviation_osd.rbegin(); p != deviation_osd.rend(); ++p) {
      ...
      for (auto pg : pgs) {
        ...
        for (auto q : p->second) {
          if (q.second == osd) {
            ldout(cct, 10) << " will try dropping existing"
                           << " remapping pair " << q.first << " -> "
                           << q.second << " which remapped " << pg
        ...
        if (new_upmap_items.empty()) {
          ...
          ldout(cct, 10) << " existing pg_upmap_items " << p->second
                         ...
                         << ", will try cancelling it entirely" << dendl;
          to_unmap.insert(pg);
          goto test_change;
      ...
      for (auto pg : pgs) {
        ...
        if (!try_pg_upmap(cct, pg, overfull, underfull, more_underfull, &orig,
                          &out)) {
          continue;
        }
        ...
        for (unsigned i = 0; i < out.size(); ++i) {
          ...
          if (osd_deviation[orig[i]] > max_dev) {
            ...
            pos = i;
            ldout(cct, 30) << "Max osd." << orig[i] << " pos " << i << " dev "
                           << osd_deviation[orig[i]] << dendl;
          }
        }
        if (pos != -1) {
          int i = pos;
          ldout(cct, 10) << " will try adding new remapping pair " << orig[i]
                         << " -> " << out[i] << " for " << pg
    ...
    for (auto &p : deviation_osd) {
      ...
      if (fabsf(deviation) < max_deviation) {
        ...
        ldout(cct, 10) << " osd." << osd << " target " << target
                       << " deviation " << deviation << " -> absolute "
                       << fabsf(deviation) << " < max " << max_deviation
                       << dendl;
        break;
      }
      // look for remaps we can un-remap
      vector<pair<pg_t, mempool::osdmap::vector<pair<int32_t, int32_t>>>>
          candidates;
      ...
      for (auto &i : candidates) {
        ...
        for (auto &j : i.second) {
          if (j.first == osd) {
            ldout(cct, 10) << " will try dropping existing"
                           << " remapping pair " << j.first << " -> "
                           << j.second << " which remapped " << pg
        ...
        if (new_upmap_items.empty()) {
          ...
          ldout(cct, 10) << " existing pg_upmap_items " << i.second
                         << " remapped " << pg << " out from underfull osd."
                         << osd << ", will try cancelling it entirely" << dendl;
          to_unmap.insert(pg);
          goto test_change;
  ...
  test_change:
    ...
    for (auto &i : to_upmap) {
      ldout(cct, 10) << " upmap pg " << i.first << " new pg_upmap_items "
                     << i.second << dendl;
...
void OSDMap::get_pool_ids_by_osd(CephContext *cct, int osd,
...
public:
  ...
  OSDUtilizationDumper(const CrushWrapper *crush, const OSDMap *osdmap_,
                       const PGMap &pgmap_, bool tree_, const string &filter)
      : Parent(crush, osdmap_->get_pool_names()), osdmap(osdmap_),
        pgmap(pgmap_), tree(tree_), min_var(-1), max_var(-1), stddev(0),
        sum(0) {
...
protected:
  ...
  void dump_item(const CrushTreeDumper::Item &qi, F *f) override {
    ...
    dump_item(qi, reweight, kb, kb_used, kb_used_data, kb_used_omap,
              kb_used_meta, kb_avail, util, var, num_pgs, f);
  ...
  virtual void dump_item(const CrushTreeDumper::Item &qi, float &reweight,
                         int64_t kb, int64_t kb_used, int64_t kb_used_data,
                         int64_t kb_used_omap, int64_t kb_used_meta,
                         int64_t kb_avail, double &util, double &var,
                         const size_t num_pgs, F *f) = 0;
  ...
  double average_utilization() {
    ...
    for (int i = 0; i < osdmap->get_max_osd(); i++) {
      if (!osdmap->exists(i) || osdmap->get_weight(i) == 0 || !should_dump(i))
        continue;
  ...
  bool get_osd_utilization(int id, int64_t *kb, int64_t *kb_used,
                           int64_t *kb_used_data, int64_t *kb_used_omap,
  ...
  bool get_bucket_utilization(int id, int64_t *kb, int64_t *kb_used,
    ...
    if (id >= 0) {
      ...
      return get_osd_utilization(id, kb, kb_used, kb_used_data, kb_used_omap,
                                 kb_used_meta, kb_avail);
    }
    ...
    *kb_avail = 0;
    ...
    for (int k = osdmap->crush->get_bucket_size(id) - 1; k >= 0; k--) {
      ...
      int64_t kb_i = 0, kb_used_i = 0, kb_used_data_i = 0, kb_used_omap_i = 0,
              kb_used_meta_i = 0, kb_avail_i = 0;
      if (!get_bucket_utilization(item, &kb_i, &kb_used_i, &kb_used_data_i,
                                  &kb_used_omap_i, &kb_used_meta_i,
                                  &kb_avail_i))
        return false;
      *kb += kb_i;
      *kb_used += kb_used_i;
...
public:
  ...
  OSDUtilizationPlainDumper(const CrushWrapper *crush, const OSDMap *osdmap,
                            const PGMap &pgmap, bool tree, const string &filter)
  ...
  void dump(TextTable *tbl) {
    ...
    *tbl << ""
         << ""
         << ""
         << "TOTAL" << byte_u_t(sum.statfs.total)
         ...
         << byte_u_t(sum.statfs.available) << lowprecision_t(average_util) << ""
         << TextTable::endrow;
  }
...
protected:
  ...
  void dump_item(const CrushTreeDumper::Item &qi, float &reweight, int64_t kb,
                 int64_t kb_used, int64_t kb_used_data, int64_t kb_used_omap,
                 int64_t kb_used_meta, int64_t kb_avail, double &util,
                 double &var, const size_t num_pgs, TextTable *tbl) override {
    const char *c = crush->get_item_class(qi.id);
    if (!c)
      c = "";
    *tbl << qi.id << c << weightf_t(qi.weight) << weightf_t(reweight)
         << byte_u_t(kb << 10) << byte_u_t(kb_used << 10)
         << byte_u_t(kb_used_data << 10) << byte_u_t(kb_used_omap << 10)
         << byte_u_t(kb_used_meta << 10) << byte_u_t(kb_avail << 10)
         << lowprecision_t(util) << lowprecision_t(var);
...
ostream &operator<<(ostream &out,
  ...
  if (v.v < -0.01) {
    ...
    return out << std::fixed << std::setprecision(2) << v.v
               << std::setprecision(p);
  }
}
...
protected:
  ...
  void dump_item(const CrushTreeDumper::Item &qi, float &reweight, int64_t kb,
                 int64_t kb_used, int64_t kb_used_data, int64_t kb_used_omap,
                 int64_t kb_used_meta, int64_t kb_avail, double &util,
                 double &var, const size_t num_pgs, Formatter *f) override {
...
void print_osd_utilization(const OSDMap &osdmap, const PGMap &pgmap,
                           ostream &out, Formatter *f, bool tree,
                           const string &filter) {
...
void OSDMap::check_health(CephContext *cct, health_check_map_t *checks) const {
  ...
  if (num_osds >= 0) {
    ...
    for (int i = 0; i < get_max_osd(); i++) {
      ...
      if (!is_up(i)) {
        ...
        for (int type = 0; type <= max_type; type++) {
          ...
          if (!subtree_type_is_down(cct, parent_id, type, &down_in_osds,
                                    &up_in_osds, &subtree_up,
                                    &subtree_type_down))
            break;
          current = parent_id;
        }
    ...
    for (int type = 1; type <= max_type; type++) {
      ...
      for (auto j = subtree_type_down[type].begin();
           j != subtree_type_down[type].end(); ++j) {
        list<int> children;
        int num = 0;
    ...
    if (num_down_in_osds > 0) {
      ...
      auto &d =
          checks->add("OSD_DOWN", HEALTH_WARN, ss.str(), down_in_osds.size());
  ...
  // OSD_OUT_OF_ORDER_FULL
  {
    ...
    if (br < nr) {
      ostringstream ss;
      ss << "backfillfull_ratio (" << br << ") < nearfull_ratio (" << nr
         << "), increased";
  ...
  // OSD_FAILSAFE_FULL
  {
    ...
    if (nearfull.size()) {
      ...
      auto &d =
          checks->add("OSD_NEARFULL", HEALTH_WARN, ss.str(), nearfull.size());
      for (auto &i : nearfull) {
        ostringstream ss;
  ...
  {
    // warn about flags
    uint64_t warn_flags = CEPH_OSDMAP_PAUSERD | CEPH_OSDMAP_PAUSEWR |
                          CEPH_OSDMAP_PAUSEREC | CEPH_OSDMAP_NOUP |
                          CEPH_OSDMAP_NODOWN | CEPH_OSDMAP_NOIN |
                          CEPH_OSDMAP_NOOUT | CEPH_OSDMAP_NOBACKFILL |
                          CEPH_OSDMAP_NORECOVER | CEPH_OSDMAP_NOSCRUB |
                          CEPH_OSDMAP_NODEEP_SCRUB | CEPH_OSDMAP_NOTIERAGENT |
                          CEPH_OSDMAP_NOSNAPTRIM | CEPH_OSDMAP_NOREBALANCE;
  ...
  // OSD_FLAGS
  {
    ...
    const unsigned flags =
        CEPH_OSD_NOUP | CEPH_OSD_NOIN | CEPH_OSD_NODOWN | CEPH_OSD_NOOUT;
    for (int i = 0; i < max_osd; ++i) {
    ...
    for (auto &i : crush_node_flags) {
      if (i.second && crush->item_exists(i.first)) {
        ...
        ss << (tn ? tn : "node") << " " << crush->get_item_name(i.first)
           << " has flags " << states;
        detail.push_back(ss.str());
      }
    }
    ...
    if (!detail.empty()) {
      ostringstream ss;
      ss << detail.size()
         << " OSDs or CRUSH {nodes, device-classes} have "
  ...
  if (cct->_conf->mon_warn_on_crush_straw_calc_version_zero) {
    if (crush->get_straw_calc_version() == 0) {
      ...
      auto &d =
          checks->add("OLD_CRUSH_STRAW_CALC_VERSION", HEALTH_WARN, ss.str(), 0);
      d.detail.push_back("see "
                         "http://docs.ceph.com/docs/master/rados/operations/"
                         "crush-map/#tunables");
    }
  ...
  if (cct->_conf->mon_warn_on_cache_pools_without_hit_sets) {
    ...
    for (auto p = pools.cbegin(); p != pools.cend(); ++p) {
      ...
      if (info.cache_mode_requires_hit_set() &&
        ...
        ostringstream ss;
        ss << "pool '" << get_pool_name(p->first) << "' with cache_mode "
           << info.get_cache_mode_name()
  ...
  // POOL_NEARFULL/BACKFILLFULL/FULL
  {
    ...
    if (!full_detail.empty()) {
      ...
      auto &d =
          checks->add("POOL_FULL", HEALTH_WARN, ss.str(), full_detail.size());
  ...
  if (cct->_conf.get_val<bool>("mon_warn_on_pool_pg_num_not_power_of_two")) {
    ...
    for (auto it : get_pools()) {
      if (!isp2(it.second.get_pg_num_target())) {
        ostringstream ss;
        ss << "pool '" << get_pool_name(it.first) << "' pg_num "
           << it.second.get_pg_num_target() << " is not a power of two";
        detail.push_back(ss.str());
      }
    }
    if (!detail.empty()) {
