...
static const char *config_keys[] = {"crush_location", NULL};
...
public:
  ...
  int call(std::string_view command, const cmdmap_t &cmdmap, Formatter *f,
           std::ostream &ss, ceph::buffer::list &out) override;
};
...
Objecter::OSDSession::unique_completion_lock
Objecter::OSDSession::get_lock(object_t &oid) {
  if (oid.name.empty())
    return unique_completion_lock();

  static constexpr uint32_t HASH_PRIME = 1021;
  uint32_t h =
      ceph_str_hash_linux(oid.name.c_str(), oid.name.size()) % HASH_PRIME;

  return unique_completion_lock(completion_locks[h % num_locks],
                                std::defer_lock);
}

const char **Objecter::get_tracked_conf_keys() const { return config_keys; }
...
void Objecter::_send_linger(LingerOp *info, shunique_lock &sul) {
  ...
  watchl.unlock();
  Op *o =
      new Op(info->target.base_oid, info->target.base_oloc, opv,
...
void Objecter::_linger_ping(LingerOp *info, int r, ceph::coarse_mono_time sent,
  ...
  ldout(cct, 10) << __func__ << " " << info->linger_id << " sent " << sent
                 << " gen " << register_gen << " = " << r << " (last_error "
                 << info->last_error << " register_gen " << info->register_gen
                 << ")" << dendl;
...
int Objecter::linger_check(LingerOp *info) {
  ...
  ldout(cct, 10) << __func__ << " " << info->linger_id << " err "
                 << info->last_error << " age " << age << dendl;
  if (info->last_error)
    return info->last_error;
  // return a safe upper bound (we are truncating to ms)
  return 1 + std::chrono::duration_cast<std::chrono::milliseconds>(age).count();
}
...
Objecter::LingerOp *Objecter::linger_register(const object_t &oid,
  ...
  ldout(cct, 10) << __func__ << " info " << info << " linger_id "
                 << info->linger_id << " cookie " << info->get_cookie()
                 << dendl;
...
ceph_tid_t Objecter::linger_watch(LingerOp *info, ObjectOperation &op,
                                  const SnapContext &snapc, real_time mtime,
                                  ceph::buffer::list &inbl, Context *oncommit,
                                  version_t *objver) {
  info->is_watch = true;
  info->snapc = snapc;
  info->mtime = mtime;
...
ceph_tid_t Objecter::linger_notify(LingerOp *info, ObjectOperation &op,
...
struct C_DoWatchNotify : public Context {
  ...
  void finish(int r) override { objecter->_do_watch_notify(info, msg); }
};
...
void Objecter::_do_watch_notify(LingerOp *info, MWatchNotify *m) {
  ...
  case CEPH_WATCH_EVENT_NOTIFY:
    info->watch_context->handle_notify(m->notify_id, m->cookie, m->notifier_gid,
                                       m->bl);
    break;
  }
...
void Objecter::_scan_requests(OSDSession *s, bool skipped_map,
                              bool cluster_full,
  ...
  while (lp != s->linger_ops.end()) {
    ...
    if (pool_full_map)
      force_resend_writes =
          force_resend_writes || (*pool_full_map)[op->target.base_oloc.pool];
    switch (r) {
    ...
    case RECALC_OP_TARGET_POOL_DNE:
      ...
      if (unregister) {
        ldout(cct, 10) << " need to unregister linger op " << op->linger_id
                       << dendl;
        op->get();
  ...
  while (cp != s->command_ops.end()) {
    ...
    if (pool_full_map)
      force_resend_writes =
          force_resend_writes || (*pool_full_map)[c->target_pg.pool()];
  ...
  for (list<LingerOp *>::iterator iter = unregister_lingers.begin();
       iter != unregister_lingers.end(); ++iter) {
    _linger_cancel(*iter);
    (*iter)->put();
  }
}
...
void Objecter::C_Op_Map_Latest::finish(int r) {
  ...
  lgeneric_subdout(objecter->cct, objecter, 10)
      << "op_map_latest r=" << r << " tid=" << tid << " latest " << latest
      << dendl;
...
void Objecter::_check_op_pool_dne(Op *op, unique_lock *sl) {
  ...
  if (op->target.pool_ever_existed) {
    ...
    ldout(cct, 10) << "check_op_pool_dne tid " << op->tid
                   << " pool previously exists but now does not" << dendl;
  } else {
    ldout(cct, 10) << "check_op_pool_dne tid " << op->tid << " current "
                   << osdmap->get_epoch() << " map_dne_bound "
...
void Objecter::_op_cancel_map_check(Op *op) {
  // rwlock is locked unique
  map<ceph_tid_t, Op *>::iterator iter = check_latest_map_ops.find(op->tid);
...
void Objecter::_check_linger_pool_dne(LingerOp *op, bool *need_unregister) {
  ...
  *need_unregister = false;
  ...
  if (op->register_gen > 0) {
    ...
    ldout(cct, 10) << "_check_linger_pool_dne linger_id " << op->linger_id
                   << " current " << osdmap->get_epoch() << " map_dne_bound "
                   << op->map_dne_bound << dendl;
  }
  if (op->map_dne_bound > 0) {
    if (osdmap->get_epoch() >= op->map_dne_bound) {
...
void Objecter::_check_command_map_dne(CommandOp *c) {
  ...
  ldout(cct, 10) << "_check_command_map_dne tid " << c->tid << " current "
                 << osdmap->get_epoch() << " map_dne_bound " << c->map_dne_bound
                 << dendl;
  if (c->map_dne_bound > 0) {
...
void Objecter::_maybe_request_map() {
  // rwlock is locked
  int flag = 0;
  if (_osdmap_full_flag() || osdmap->test_flag(CEPH_OSDMAP_PAUSERD) ||
      osdmap->test_flag(CEPH_OSDMAP_PAUSEWR)) {
    ldout(cct, 10) << "_maybe_request_map subscribing (continuous) to next "
                      "osd map (FULL flag is set)"
                   << dendl;
  } else {
    ldout(cct, 10) << "_maybe_request_map subscribing (onetime) to next osd map"
                   << dendl;
...
void Objecter::tick() {
  ...
  for (map<int, OSDSession *>::iterator siter = osd_sessions.begin();
    ...
    bool found = false;
    for (map<ceph_tid_t, Op *>::iterator p = s->ops.begin(); p != s->ops.end();
         ++p) {
      Op *op = p->second;
      ceph_assert(op->session);
      if (op->stamp < cutoff) {
  ...
  if (initialized) {
    tick_event = timer.reschedule_me(
        ceph::make_timespan(cct->_conf->objecter_tick_interval));
  }
}
...
void Objecter::resend_mon_ops() {
  ...
  for (map<uint64_t, LingerOp *>::iterator p = check_latest_map_lingers.begin();
       p != check_latest_map_lingers.end(); ++p) {
  ...
  for (map<uint64_t, CommandOp *>::iterator p =
           check_latest_map_commands.begin();
       p != check_latest_map_commands.end(); ++p) {
...
void Objecter::_op_submit_with_budget(Op *op, shunique_lock &sul,
                                      ceph_tid_t *ptid, int *ctx_budget) {
  ceph_assert(initialized);
...
void Objecter::_op_submit(Op *op, shunique_lock &sul, ceph_tid_t *ptid) {
  ...
  bool check_for_latest_map =
      _calc_target(&op->target, nullptr) == RECALC_OP_TARGET_POOL_DNE;
  ...
  if (osdmap->get_epoch() < epoch_barrier) {
    ldout(cct, 10) << " barrier, paused " << op << " tid " << op->tid << dendl;
    ...
    ldout(cct, 0) << " FULL, paused modify " << op << " tid " << op->tid
                  << dendl;
    op->target.paused = true;
    _maybe_request_map();
...
int Objecter::op_cancel(OSDSession *s, ceph_tid_t tid, int r) {
  ...
  if (p == s->ops.end()) {
    ldout(cct, 10) << __func__ << " tid " << tid << " dne in session " << s->osd
                   << dendl;
    return -ENOENT;
  }
...
start:
  ...
  ldout(cct, 5) << __func__ << ": tid " << tid << " not found in live sessions"
                << dendl;
...
bool Objecter::is_pg_changed(int oldprimary, const vector<int> &oldacting,
                             int newprimary, const vector<int> &newacting,
                             bool any_change) {
  if (OSDMap::primary_changed(oldprimary, oldacting, newprimary, newacting))
    return true;
  ...
  for (map<int64_t, pg_pool_t>::const_iterator it = osdmap->get_pools().begin();
    ...
    if (pool_full_map.find(it->first) == pool_full_map.end()) {
      ...
      pool_full_map[it->first] =
          _osdmap_pool_full(it->second) || pool_full_map[it->first];
    }
  }
}
...
int Objecter::_calc_target(op_target_t *t, Connection *con, bool any_change) {
  ...
  ldout(cct, 20) << __func__ << " epoch " << t->epoch << " base " << t->base_oid
                 << " " << t->base_oloc << " precalc_pgid "
                 << (int)t->precalc_pgid << " pgid " << t->base_pgid
  ...
  ldout(cct, 30) << __func__ << "  base pi " << pi << " pg_num "
                 << pi->get_pg_num() << dendl;
  ...
  ldout(cct, 30) << __func__ << "  target pi " << pi << " pg_num "
                 << pi->get_pg_num() << dendl;
  ...
  if (lookup_pg_mapping(actual_pgid, &pg_mapping)) {
    ...
    osdmap->pg_to_up_acting_osds(actual_pgid, &up, &up_primary, &acting,
                                 &acting_primary);
    pg_mapping_t pg_mapping(osdmap->get_epoch(), up, up_primary, acting,
                            acting_primary);
  ...
  if (any_change &&
      PastIntervals::is_new_interval(
          t->acting_primary, acting_primary, t->acting, acting, t->up_primary,
          up_primary, t->up, up, t->size, size, t->min_size, min_size,
          t->pg_num, pg_num, t->pg_num_pending, pg_num_pending, t->sort_bitwise,
          sort_bitwise, t->recovery_deletes, recovery_deletes, prev_pgid)) {
    force_resend = true;
  }
  ...
  bool legacy_change =
      t->pgid != pgid ||
      is_pg_changed(t->acting_primary, t->acting, acting_primary, acting,
  ...
  if (legacy_change || split_or_merge || force_resend) {
    ...
    ldout(cct, 10) << __func__ << " "
                   ...
                   << " acting " << acting << " primary " << acting_primary
                   << dendl;
    ...
    if (acting_primary == -1) {
      ...
      if (read && (t->flags & CEPH_OSD_FLAG_BALANCE_READS)) {
        ...
        for (unsigned i = 0; i < acting.size(); ++i) {
          ...
          ldout(cct, 20) << __func__ << " localize: rank " << i << " osd."
                         << acting[i] << " locality " << locality << dendl;
          if (i == 0 ||
...
int Objecter::_recalc_linger_op_target(LingerOp *linger_op,
  ...
  if (r == RECALC_OP_TARGET_NEED_RESEND) {
    ldout(cct, 10) << "recalc_linger_op_target tid " << linger_op->linger_id
                   << " pgid " << linger_op->target.pgid << " acting "
                   << linger_op->target.acting << dendl;
...
MOSDOp *Objecter::_prepare_osd_op(Op *op) {
  ...
  MOSDOp *m = new MOSDOp(client_inc, op->tid, hobj, op->target.actual_pgid,
                         osdmap->get_epoch(), flags, op->features);
...
void Objecter::_send_op(Op *op) {
  ...
  ldout(cct, 15) << "_send_op " << op->tid << " to " << op->target.actual_pgid
                 << " on osd." << op->session->osd << dendl;
...
int Objecter::calc_op_budget(const vector<OSDOp> &ops) {
  int op_budget = 0;
  for (vector<OSDOp>::const_iterator i = ops.begin(); i != ops.end(); ++i) {
...
int Objecter::take_linger_budget(LingerOp *info) { return 1; }
...
void Objecter::handle_osd_op_reply(MOSDOpReply *m) {
  ...
  ldout(cct, 7) << "handle_osd_op_reply " << tid
                << (m->is_ondisk() ? " ondisk"
                                   : (m->is_onnvram() ? " onnvram" : " ack"))
                << " uv " << m->get_user_version() << " in " << m->get_pg()
                << " attempt " << m->get_retry_attempt() << dendl;
  Op *op = iter->second;
  ...
  if (m->get_retry_attempt() >= 0) {
    if (m->get_retry_attempt() != (op->attempts - 1)) {
      ldout(cct, 7) << " ignoring reply from attempt " << m->get_retry_attempt()
                    << " from " << m->get_source_inst() << "; last attempt "
                    << (op->attempts - 1) << " sent to "
  ...
  if (m->is_redirect_reply()) {
    ...
    op->target.flags |= (CEPH_OSD_FLAG_REDIRECTED | CEPH_OSD_FLAG_IGNORE_CACHE |
  ...
  if (op->outbl) {
    ...
    if (op->outbl->length() == bl.length() && bl.get_num_buffers() <= 1) {
      ...
      ldout(cct, 10) << __func__ << " copying resulting " << bl.length()
                     << " into existing ceph::buffer of length "
                     << op->outbl->length() << dendl;
      ceph::buffer::list t;
      t.claim(*op->outbl);
  ...
  for (unsigned i = 0; p != out_ops.end() && pb != op->out_bl.end();
       ++i, ++p, ++pb, ++pr, ++ph) {
    ldout(cct, 10) << " op " << i << " rval " << p->rval << " len "
                   << p->outdata.length() << dendl;
    if (*pb)
      **pb = p->outdata;
...
void Objecter::handle_osd_backoff(MOSDBackoff *m) {
  ...
  case CEPH_OSD_BACKOFF_OP_BLOCK: {
    ...
    Message *r =
        new MOSDBackoff(m->pgid, m->map_epoch, CEPH_OSD_BACKOFF_OP_ACK_BLOCK,
                        m->id, m->begin, m->end);
    ...
    if (p != s->backoffs_by_id.end()) {
      ...
      if (b->begin != m->begin && b->end != m->end) {
        lderr(cct) << __func__ << " got " << m->pgid << " id " << m->id
                   << " unblock on [" << m->begin << "," << m->end
                   << ") but backoff is [" << b->begin << "," << b->end << ")"
                   << dendl;
        // hrmpf, unblock it anyway.
      }
      ldout(cct, 10) << __func__ << " unblock backoff " << b->pgid << " id "
                     << b->id << " [" << b->begin << "," << b->end << ")"
                     << dendl;
...
uint32_t Objecter::list_nobjects_seek(NListContext *list_context,
                                      uint32_t pos) {
  shared_lock rl(rwlock);
  list_context->pos = hobject_t(object_t(), string(), CEPH_NOSNAP, pos,
                                list_context->pool_id, string());
  ldout(cct, 10) << __func__ << " " << list_context << " pos " << pos << " -> "
...
uint32_t Objecter::list_nobjects_seek(NListContext *list_context,
  ...
  list_context->at_end_of_pool = false;
  pg_t actual =
      osdmap->raw_pg_to_pg(pg_t(cursor.get_hash(), list_context->pool_id));
  list_context->current_pg = actual.ps();
...
void Objecter::list_nobjects_get_cursor(NListContext *list_context,
  ...
  if (list_context->list.empty()) {
    *cursor = list_context->pos;
    ...
    uint32_t h = osdmap->get_pg_pool(list_context->pool_id)
                     ->hash_key(*key, entry.nspace);
    *cursor = hobject_t(entry.oid, entry.locator, list_context->pool_snap_seq,
                        h, list_context->pool_id, entry.nspace);
  }
}
...
void Objecter::list_nobjects(NListContext *list_context, Context *onfinish) {
  ldout(cct, 10) << __func__ << " pool_id " << list_context->pool_id
                 ...
                 << " list_context " << list_context << " onfinish " << onfinish
                 << " current_pg " << list_context->current_pg << " pos "
                 << list_context->pos << dendl;
  ...
  if (list_context->sort_bitwise != sort_bitwise) {
    list_context->pos =
        hobject_t(object_t(), string(), CEPH_NOSNAP, list_context->current_pg,
                  list_context->pool_id, string());
    list_context->sort_bitwise = sort_bitwise;
  ...
  if (list_context->pos.is_max()) {
    ldout(cct, 20) << __func__ << " end of pool, list " << list_context->list
                   << dendl;
    if (list_context->list.empty()) {
  ...
  ObjectOperation op;
  op.pg_nls(list_context->max_entries, list_context->filter, list_context->pos,
            osdmap->get_epoch());
...
void Objecter::_nlist_reply(NListContext *list_context, int r,
  ...
  ldout(cct, 20) << " response.entries.size " << response_size
                 << ", response.entries " << response.entries << ", handle "
                 << response.handle << ", tentative new pos "
                 << list_context->pos << dendl;
  if (response_size) {
...
int Objecter::create_pool_snap(int64_t pool, string &snap_name,
                               Context *onfinish) {
  unique_lock wl(rwlock);
  ldout(cct, 10) << "create_pool_snap; pool: " << pool
                 << "; snap: " << snap_name << dendl;
...
int Objecter::delete_pool_snap(int64_t pool, string &snap_name,
                               Context *onfinish) {
  unique_lock wl(rwlock);
  ldout(cct, 10) << "delete_pool_snap; pool: " << pool
                 << "; snap: " << snap_name << dendl;
...
int Objecter::delete_selfmanaged_snap(int64_t pool, snapid_t snap,
                                      Context *onfinish) {
  unique_lock wl(rwlock);
  ldout(cct, 10) << "delete_selfmanaged_snap; pool: " << pool
                 << "; snap: " << snap << dendl;
  PoolOp *op = new PoolOp;
  if (!op)
    return -ENOMEM;
...
void Objecter::pool_op_submit(PoolOp *op) {
  ...
  if (mon_timeout > timespan(0)) {
    op->ontimeout = timer.add_event(
        mon_timeout, [this, op]() { pool_op_cancel(op->tid, -ETIMEDOUT); });
  }
  _pool_op_submit(op);
}
...
void Objecter::_pool_op_submit(PoolOp *op) {
  ...
  MPoolOp *m = new MPoolOp(monc->get_fsid(), op->tid, op->pool, op->name,
                           op->pool_op, last_seen_osdmap_version);
  if (op->snapid)
    m->snapid = op->snapid;
  if (op->crush_rule)
    m->crush_rule = op->crush_rule;
...
void Objecter::handle_pool_op_reply(MPoolOpReply *m) {
  ...
  if (iter != pool_ops.end()) {
    ...
    ldout(cct, 10) << "have request " << tid << " at " << op
                   << " Op: " << ceph_pool_op_name(op->pool_op) << dendl;
    if (op->blp)
...
void Objecter::get_pool_stats(list<string> &pools,
                              map<string, pool_stat_t> *result, bool *per_pool,
                              Context *onfinish) {
  ...
  op->onfinish = onfinish;
  if (mon_timeout > timespan(0)) {
    op->ontimeout = timer.add_event(mon_timeout, [this, op]() {
      pool_stat_op_cancel(op->tid, -ETIMEDOUT);
    });
  } else {
    op->ontimeout = 0;
  }
...
void Objecter::_poolstat_submit(PoolStatOp *op) {
  ...
  monc->send_mon_message(new MGetPoolStats(monc->get_fsid(), op->tid, op->pools,
...
void Objecter::get_fs_stats(ceph_statfs &result,
  ...
  if (mon_timeout > timespan(0)) {
    op->ontimeout = timer.add_event(
        mon_timeout, [this, op]() { statfs_op_cancel(op->tid, -ETIMEDOUT); });
  } else {
    op->ontimeout = 0;
  }
...
void Objecter::_fs_stats_submit(StatfsOp *op) {
  ...
  monc->send_mon_message(new MStatfs(monc->get_fsid(), op->tid, op->data_pool,
                                     last_seen_pgmap_version));
  op->last_submit = ceph::coarse_mono_clock::now();
  ...
  if (con->get_peer_type() == CEPH_ENTITY_TYPE_OSD) {
    ...
    if (session) {
      ...
      if (!(initialized && osdmap->is_up(session->osd))) {
        ldout(cct, 1) << "ms_handle_reset aborted,initialized=" << initialized
                      << dendl;
        wl.unlock();
        return false;
      }
...
void Objecter::_dump_active(OSDSession *s) {
  for (map<ceph_tid_t, Op *>::iterator p = s->ops.begin(); p != s->ops.end();
       ++p) {
    Op *op = p->second;
    ldout(cct, 20) << op->tid << "\t" << op->target.pgid << "\tosd."
...
void Objecter::_dump_ops(const OSDSession *s, Formatter *fmt) {
  for (map<ceph_tid_t, Op *>::const_iterator p = s->ops.begin();
    ...
    Op *op = p->second;
    auto age =
        std::chrono::duration<double>(coarse_mono_clock::now() - op->stamp);
...
void Objecter::_dump_command_ops(const OSDSession *s, Formatter *fmt) {
  for (map<uint64_t, CommandOp *>::const_iterator p = s->command_ops.begin();
    ...
    for (vector<string>::const_iterator q = op->cmd.begin(); q != op->cmd.end();
         ++q)
...
int Objecter::RequestStateHook::call(std::string_view command,
                                     const cmdmap_t &cmdmap, Formatter *f,
                                     std::ostream &ss,
...
void Objecter::handle_command_reply(MCommandReply *m) {
  ...
  if (!c->session || m->get_connection() != c->session->con) {
    ldout(cct, 10) << "handle_command_reply tid " << m->get_tid()
                   << " got reply from wrong connection " << m->get_connection()
                   << " " << m->get_source_inst() << dendl;
    m->put();
    sl.unlock();
    return;
...
void Objecter::submit_command(CommandOp *c, ceph_tid_t *ptid) {
  ...
  _assign_command_session(c, sul);
  if (osd_timeout > timespan(0)) {
    c->ontimeout = timer.add_event(osd_timeout, [this, c, tid]() {
      command_op_cancel(c->session, tid, -ETIMEDOUT);
    });
  }
...
int Objecter::_calc_command_target(CommandOp *c, shunique_lock &sul) {
  ...
  ceph_assert(r !=
              -EAGAIN); /* shouldn't happen as we're holding the write lock */
...
void Objecter::_assign_command_session(CommandOp *c, shunique_lock &sul) {
  ceph_assert(sul.owns_lock() && sul.mutex() == &rwlock);
...
void Objecter::_finish_command(CommandOp *c, int r, string rs) {
  ...
  ldout(cct, 10) << "_finish_command " << c->tid << " = " << r << " " << rs
                 << dendl;
  if (c->prs)
    *c->prs = rs;
  if (c->onfinish)
...
Objecter::Objecter(CephContext *cct_, Messenger *m, MonClient *mc,
                   Finisher *fin, double mon_timeout, double osd_timeout)
    : Dispatcher(cct_), messenger(m), monc(mc), finisher(fin),
      ...
      retry_writes_after_first_reply(
...
hobject_t Objecter::enumerate_objects_begin() { return hobject_t(); }

hobject_t Objecter::enumerate_objects_end() { return hobject_t::get_max(); }
...
struct C_EnumerateReply : public Context {
  ...
  C_EnumerateReply(Objecter *objecter_, hobject_t *next_,
                   ...
                   const hobject_t end_, const int64_t pool_id_,
                   Context *on_finish_)
      : objecter(objecter_), next(next_), result(result_), end(end_),
        pool_id(pool_id_), on_finish(on_finish_), epoch(0), budget(-1) {}

  void finish(int r) override {
    objecter->_enumerate_reply(bl, r, end, pool_id, budget, epoch, result, next,
                               on_finish);
  }
};

void Objecter::enumerate_objects(int64_t pool_id, const std::string &ns,
                                 const hobject_t &start, const hobject_t &end,
                                 const uint32_t max,
                                 const ceph::buffer::list &filter_bl,
                                 std::list<librados::ListObjectImpl> *result,
                                 hobject_t *next, Context *on_finish) {
  ceph_assert(result);
  ...
  C_EnumerateReply *on_ack =
      new C_EnumerateReply(this, next, result, end, pool_id, on_finish);
  ...
  pg_read(start.get_hash(), oloc, op, &on_ack->bl, 0, on_ack, &on_ack->epoch,
          &on_ack->budget);
}

void Objecter::_enumerate_reply(ceph::buffer::list &bl, int r,
                                const hobject_t &end, const int64_t pool_id,
                                int budget, epoch_t reply_epoch,
                                std::list<librados::ListObjectImpl> *result,
                                hobject_t *next, Context *on_finish) {
  if (budget >= 0) {
  ...
  ldout(cct, 10) << __func__ << ": got " << response.entries.size()
                 << " handle " << response.handle << " reply_epoch "
                 << reply_epoch << dendl;
    ...
    *next = end;
    ...
    while (!response.entries.empty()) {
      ...
      hobject_t last(response.entries.back().oid,
                     response.entries.back().locator, CEPH_NOSNAP, hash,
                     pool_id, response.entries.back().nspace);
      if (last < end)
        break;
      ldout(cct, 20) << __func__ << " dropping item " << last << " >= end "
                     << end << dendl;
...
template <typename T>
void do_scrub_ls(::ObjectOperation *op, const scrub_ls_arg_t &arg,
                 std::vector<T> *items, uint32_t *interval, int *rval) {
...
void ::ObjectOperation::scrub_ls(
    const librados::object_id_t &start_after, uint64_t max_to_get,
    std::vector<librados::inconsistent_obj_t> *objects, uint32_t *interval,
    int *rval) {
...
void ::ObjectOperation::scrub_ls(
    const librados::object_id_t &start_after, uint64_t max_to_get,
    std::vector<librados::inconsistent_snapset_t> *snapsets, uint32_t *interval,
    int *rval) {
